...
class Anything2Images:
    def __init__(
        self, llm: byzerllm.ByzerLLM, args: AutoCoderArgs, 
    ...
    def run_python_code(self, code: str) -> str:
        ...
        try:
            result, error = jupyter_client.add_and_run(code)
            print(result)
            print(error)
        finally:
    ...
    def to_pdf(self, origin_file_path: str, target_file_path: str) -> str:
        ...
        if not os.path.exists(os.path.join(self.output_dir, "output.html")):
            ...
            with open(
                os.path.join(self.output_dir, "output.html"), "r", encoding="utf-8"
            ) as f:
                html = f.read()
