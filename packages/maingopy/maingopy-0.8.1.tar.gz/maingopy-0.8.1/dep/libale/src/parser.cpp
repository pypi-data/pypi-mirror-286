...
parser::parser(std::istream &input, symbol_table &symbols) :
    ...
    lex.reserve_keywords(
      { // fundamentals
        ...
        "sum_div", "xlog_sum", "pos", "neg", "lb_func", "ub_func",
  ...
  std::unique_ptr<value_node<ResultType>> &result) {
    if constexpr(std::is_same_v<ResultType, real<0>>) {
        return match_internal_function<index_to_real_node>(result, "as_real")
               ...
               // nary functions
               || match_internal_function<min_node>(result, "min") || match_internal_function<max_node>(result, "max") || match_internal_function<sum_div_node>(result, "sum_div") || match_internal_function<xlog_sum_node>(result, "xlog_sum")
