...
using real_node_types = typename combined_pack<
  pack<constant_node<real<0>> *, parameter_node<real<0>> *,
    ...
    erfc_node *, norm2_node *, sum_div_node *, xlog_sum_node *, pos_node *,
...
using boolean_node_types = typename combined_pack<
  pack<constant_node<boolean<0>> *, parameter_node<boolean<0>> *,
    ...
    element_node<real<0>> *, element_node<index<0>> *,
...
struct kary_node {
    kary_node(value_node<TTypes> *...args) :
        children(args...) {};
    template <size_t I>
...
struct iterator_node : kary_node<set<IteratorType, 0>, TType> {
    iterator_node(const std::string &name,
      value_node<set<IteratorType, 0>> *first,
      value_node<TType> *second) :
...
template <typename TType>
struct nary_node {
    void add_child(value_node<TType> *node) { children.emplace_back(node); };
