...
class BabNode {
  ...
  public:
    ...
    BabNode(const BabNode& babNodeIn, const std::vector<double>& lbdsIn, const std::vector<double>& ubdsIn):
        _pruningScore(babNodeIn.get_pruning_score()), _lowerBounds(lbdsIn), _upperBounds(ubdsIn), _parentidNumber(babNodeIn.get_parent_ID()), _idNumber(babNodeIn.get_ID()), _depth(babNodeIn.get_depth()), _augmentData(babNodeIn.get_augment_data()) {}
