...
class PdfUtils:
    ...
    def extract_text_from_pdf(cls, file_content, direct_convert=False):
        if direct_convert:
            ...
            pdf_text, _ = GoogleVisionAPI().extract_text_vision(image_file)
...
class DocumentUtils:
    ...
    def extract_text_from_image(cls, file):
        ...
        file.seek(0)
        return GoogleVisionAPI().extract_text_vision(buffer)

    @classmethod
    def extract_text_from_image_url(cls, url):
        return GoogleVisionAPI().extract_text_from_url(url)
