...
class GetFeatureStep(Step):
    def execute(self, context: DeviceRequestContext, result: StepResult):
        ...
        feature: str = finger.get_finger_feature()
        if feature:
            self.__set_ret_value(context, "featureID", feature)
            context.get_response().ret_code = 0
            result.status = StepStatus.SUCCESS
        else:
            result.status = StepStatus.FAIL
            result.code = -1
            result.data = "读取指纹特征失败"
            return
