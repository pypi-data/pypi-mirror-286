# author: haoliqing
# date: 2023/9/22 16:33
# desc:
import json

from model.task_model.step_model import Step, StepStatus, StepResult
from model.task_model.context.device_request_context import DeviceRequestContext
from driver.finger_scanner import FingerScanner, TemplatesInfo


class GetTemplateStep(Step):
    def execute(self, context: DeviceRequestContext, result: StepResult):
        finger: FingerScanner = context.get_device()
        if not finger:
            result.status = StepStatus.FAIL
            result.data = "无法获取到设备驱动"
            return

        finger_num = context.get_request_param_data("fingerNum")
        if not finger_num:
            finger_num = 1

        templates: TemplatesInfo = finger.get_finger_template(int(finger_num))
        if templates and len(templates.data) >= 0:
            template_str = json.dumps(templates.data, ensure_ascii=False)
            self.__set_ret_value(context, "templateID", template_str)
            context.get_response().ret_code = templates.error_code
            result.status = StepStatus.SUCCESS
        else:
            result.status = StepStatus.FAIL
            result.code = templates.error_code
            result.data = "读取指纹模板失败"
            return

    def __set_ret_value(self, context: DeviceRequestContext, param_id, value):
        if context.get_request_param_data(param_id):
            context.get_response().add_response_value(context.get_request_param_data(param_id), value)
