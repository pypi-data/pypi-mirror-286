...
class MK500FinanceIcCard(FinanceICCardRWer):
    def __init__(self):
        super().__init__()

        self.pszComNo = b"1001"
        """ MK500 端口号: “1001”表示 USB 通信，自动识别 HID 和 CCID 协议,串口 2 表示为“COM2” """

        self.BpNo = b""
        """ BP盒的转口，长度 1 字节，取值：A、B、C、K ,输入空指针的时候，表示不支持 BP 盒转口"""

        self.icFlag = b"3"
        """ 卡类型 1-接触式IC卡 2-非接触式IC卡 3-自动判断（超时时间需为0）"""

        self.nBaud = None
        """ 波特率 0-9600bps、1-115200bps，默认使用 9600bps"""
    ...
    def init(self, device_cfg: DeviceConfig) -> int:
        ...
        try:
            ...
            if port_type == PortType.USB:
                ...
                self.nBaud = device_cfg.port.get_port_param("baudRate")
    ...
    def power_on(self) -> int:
        ATRLen = c_int(1024)
        ATR = (c_ubyte * 1024)()
        on_status = self.dll_ICReader.CT_PowerOn(self.pszComNo, self.BpNo, self.icFlag, byref(ATRLen), ATR)
        atrLen = ATRLen.value  # ATR数据实际长度
        atrData = ATR[:atrLen]  # 返回的 ATR 数据
    ...
    def power_off(self) -> int:
        return self.dll_ICReader.CT_PowerOff(self.pszComNo, self.BpNo, self.icFlag)
    ...
    def read_finance_ic_info(self, tran_info: TranInfo) -> FinanceICInfo:
        ...
        self.pszTxData = (f"P{self.fbz(authAmount)}{authAmount}R{self.fbz(tranCcy)}{tranCcy}"
                          f"S{self.fbz(tranDate)}{tranDate}T{self.fbz(tranType)}{tranType}"
                          f"U{self.fbz(tranTime)}{tranTime}W{self.fbz(orgName)}{orgName}")
        pszAidList: bytes = bytes(inputAid, "gbk")
        flag = self.getICInfo(financeICInfo, pszAidList)
        if flag < 0:
            financeICInfo.err_code = flag
            return financeICInfo
        flag = self.getARQC(financeICInfo, pszAidList, tranDate, authAmount)
        if flag < 0:
            financeICInfo.err_code = flag
            return financeICInfo
    ...
    def getICInfo(self, financeICInfo: FinanceICInfo, pszAidList: bytes) -> int:
        """
        读取基本卡信息
        @param financeICInfo: 结果存储对象
        @param pszAidList: 交易使用的Aid
        @return:
        """
        pszTaglist = b"ABCDEFGHIJKMNOPQST"
        pnUsrInfoLen = c_int(1024)
        pszUserInfo = create_string_buffer(1024)
        read_card_status = self.dll_ICReader.CT_GetIccInfo(self.pszComNo, self.BpNo, self.icFlag,
                                                           pszAidList,
                                                           pszTaglist,
                                                           byref(pnUsrInfoLen),
                                                           pszUserInfo)
        ...
        try:
            result = pszUserInfo.value.decode("gbk")
    ...
    def getARQC(self, financeICInfo: FinanceICInfo, pszAidList: bytes, tranDate: str, authAmount: str) -> int:
        """
        获取ARQC
        @param authAmount:
        @param tranDate:
        ...
        @return:
        """
        pnARQCLen = c_int(1024)
        pszARQC = create_string_buffer(1024)
        logger.info(f"读取ARQC输入交易数据是: {self.pszTxData}")
        read_arqc_status = self.dll_ICReader.CT_GenerateARQC(self.pszComNo, self.BpNo, self.icFlag,
                                                             pszAidList,
        ...
        try:
            result = pszARQC.value.decode("gbk")
            logger.info(f"读取ARQC返回数据：{result}")
            financeICInfo.arqc = str(pnARQCLen.value).zfill(3) + result
    ...
    def getTranDetail(self, financeICInfo: FinanceICInfo, pszAidList: bytes) -> int:
        ...
        read_detail_status = self.dll_ICReader.CT_GetTransActionLog(self.pszComNo, self.BpNo, self.icFlag,
                                                                    pszAidList,
                                                                    byref(pnTxDetailLen),
                                                                    detail)
    ...
    def getCreditLoadLog(self, financeICInfo: FinanceICInfo, pszAidList: bytes) -> int:
        ...
        read_credit_result = self.dll_ICReader.CT_GetCreditLoadLog(self.pszComNo, self.BpNo, self.icFlag, pszAidList,
                                                                   byref(pnTxDetailLen),
                                                                   txDetail)
    ...
    def write_finance_ic_info(self, data: str) -> WriteICResult:
        ...
        write_status = self.dll_ICReader.CT_CloseTransAction(self.pszComNo,
                                                             self.BpNo,
                                                             self.icFlag,
                                                             bytes(self.pszTxData, "gbk"),
                                                             bytes(data, "gbk"),
