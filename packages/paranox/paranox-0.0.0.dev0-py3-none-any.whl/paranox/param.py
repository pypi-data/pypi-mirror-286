...
from .grammar import retrieve_address
from .util import (
    PyTree,
    Tensor,
    where_weight,
...
class MappedParameter(eqx.Module):
    ...
    def __init__(self, model: PyTree, *, where: Callable = where_weight):
        if where is None:
            self.original = self.preimage_map(model)
        else:
    ...
    def __jax_array__(self):
        return self.image_map(jnp.asarray(self.original))
...
class PhaseLikeParameter(DomainMappedParameter):
    """
    Parameter representing a phase-like tensor, whose value wraps around when
    it exceeds a maximum or minimum value.
    """
    ...
    def image_map_impl(self, param: Tensor) -> Tensor:
        max_val = self.period / 2
        min_val = -max_val
        return (param - min_val) % self.period + min_val
