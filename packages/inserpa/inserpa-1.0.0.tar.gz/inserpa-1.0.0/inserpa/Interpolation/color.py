...
from inserpa.Utility.color import hex_
...
@lru_cache(30)
def color(color1: str | tuple[int, int, int], color2: str | tuple[int, int, int], steps: int = 10) -> list[
    str]:
...
def gradient(colors: list[str | tuple[int, int, int, int]], steps: int = 10) -> list[str]:
    gradient = []
...
def gradient_gradient_as_trilent(gradient1: str, gradient2: str, steps: int, color_steps: int) -> list[str]:
    gradient1_colors = [hex_(color) for color in gradient1.split(' -> ')]
    gradient2_colors = [hex_(color) for color in gradient2.split(' -> ')]

    grad1 = gradient(gradient1_colors, color_steps)
    grad2 = gradient(gradient2_colors, color_steps)
