...
def name(string):
    return colour_names[string.lower()]
...
def longify(value, long):
    if len(value) == 7 and long:
        return 'FF'
    elif len(value) == 9 and not long:
        return value[0:7]
...
def hex_(value, long=False):
    if isinstance(value, str) and value.startswith('#'):
        return longify(value, long)
    elif isinstance(value, str) and value.isidentifier():
        return longify(name(value), long)
    elif isinstance(value, str) and value.count(',') > 0:
        return longify(to_hex(value.split(', ')), long)
