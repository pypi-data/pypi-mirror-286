...
class FindDupRowsDialog(object):
	def __init__(self, parent, table_rows, column_specs, prohibited_columns):
		...
		self.help_btn.grid(row=0, column=0, sticky=tk.W, padx=(6,3))
		self.dlg.bind("<Alt-h>", self.do_help)
		self.add_columns_btn = ttk.Button(btn_frame, text="Add Column", command=self.add_column, state=tk.DISABLED)
		self.add_columns_btn.grid(row=0, column=1, sticky=tk.W, padx=(3,6))
		close_btn = ttk.Button(btn_frame, text="Close", command=self.do_close, underline=0)
	...
	def redraw(self):
		...
		if self.plot_data is not None and len(self.plot_data) > 0 and len(self.plot_data[0]) > 0:
			if plot_type == "Count by category":
				...
				self.plot_axes.set_xlabel(self.xlabel or self.x_var.get()+" to "+self.y_var.get())
...
class PctStackedAreaDialog(object):
	...
	def redraw(self):
		...
		self.plot_axes.stackplot(np.array(self.plot_data[0]), np.array(rows_to_columns(yprop)), labels=y_lbls, colors=custpal)
		...
		self.plot_axes.set_xlabel(self.xlabel or self.x_var.get())
		self.plot_axes.set_ylabel(self.ylabel or "Percent of total")
		if self.plot_title is not None:
			self.plot_axes.set_title(self.plot_title)
			self.plot_axes.title.set_visible(True)
		else:
...
class MultiVariableLinePlotDialog(object):
	...
	def redraw(self):
		...
		plot_fn = self.plot_axes.plot if self.scatter_var.get() == '0' else self.plot_axes.scatter
		...
		self.plot_axes.set_xlabel(self.xlabel or self.x_var.get())
		self.plot_axes.set_ylabel(self.ylabel or "Value")
		if self.plot_title is not None:
			self.plot_axes.set_title(self.plot_title)
			self.plot_axes.title.set_visible(True)
		else:
...
class PCADialog(object):
	def __init__(self, parent, column_specs, id_columns):
		...
		self.screeplot_page.rowconfigure(0, weight=1)
		self.screeplot_page.columnconfigure(0, weight=1)
		self.screeplot_page.grid(row=0, column=0, sticky=tk.NSEW)
		self.screeplotfig = Figure(dpi=100)
		self.screeplotfig.set_figheight(2)
		self.screeplotfig.set_figwidth(2)
		self.screeplotfig_canvas = FigureCanvasTkAgg(self.screeplotfig, self.screeplot_page)
		self.screeplot_nav = NavigationToolbar2Tk(self.screeplotfig_canvas, self.screeplot_page)
		self.screeplotfig_canvas.get_tk_widget().pack(side=tk.TOP, fill=tk.BOTH, expand=1)
		self.screeplot_nav.update()
		...
		self.pcplotfig = Figure(dpi=100)
		self.pcplotfig.set_figheight(2)
		self.pcplotfig.set_figwidth(2)
		self.pcplotfig_canvas = FigureCanvasTkAgg(self.pcplotfig, pcpfig)
		self.pcplot_nav = NavigationToolbar2Tk(self.pcplotfig_canvas, pcpfig)
		self.pcplotfig_canvas.get_tk_widget().pack(side=tk.TOP, fill=tk.BOTH, expand=1)
		self.pcplot_nav.update()
	...
	def recalc(self):
		...
		if self.dataset is not None and len(self.dataset[0]) > 2:
			...
			self.screeplot_axes.plot(vartbl_v[0], vartbl_v[3], marker=".")
			self.screeplot_axes.set_xlabel("Principal Component")
			self.screeplot_axes.set_ylabel("Total Variance")
			self.screeplotfig_canvas.draw()
	...
	def pcplot_redraw(self, *args):
		...
		if self.scores is not None and xvar != '' and yvar != '':
			self.pcplotfig.clear()
			self.pcplot_axes = self.pcplotfig.add_subplot(111)
			xvals = self.scores[self.scores_labels.index(xvar)]
			yvals = self.scores[self.scores_labels.index(yvar)]
			splot = self.pcplot_axes.scatter(xvals, yvals, alpha=self.alpha)
			self.pcplot_axes.set_xlabel(xvar + " score")
			self.pcplot_axes.set_ylabel(yvar + " score")
			self.pcplotfig_canvas.draw()
			# Hover annotation
			def update_annot(ind):
				pos = splot.get_offsets()[ind["ind"][0]]
				self.annot.xy = pos
				text = ", ".join([str(self.scores[0][n]) for n in ind["ind"]])
				self.annot.set_text(text)
				self.annot.get_bbox_patch().set_facecolor("wheat")
				self.annot.get_bbox_patch().set_alpha(0.8)
			def hover(event):
				vis = self.annot.get_visible()
				if event.inaxes == self.pcplot_axes:
					cont, ind = splot.contains(event)
					if cont:
						update_annot(ind)
						self.annot.set_visible(True)
						...
						if vis:
							self.annot.set_visible(False)
					self.pcplotfig_canvas.draw_idle()
			self.annot = self.pcplot_axes.annotate("", xy=(0,0), xytext=(8,12), textcoords="offset points", \
					bbox={"boxstyle": "round", "fc": "w"}, arrowprops={"arrowstyle": "->"})
			self.annot.set_visible(False)
			self.canvas_conn_id = self.pcplotfig_canvas.mpl_connect("motion_notify_event", hover)
	...
	def clear_output(self):
		...
		self.screeplotfig.clear()
		self.screeplot_axes = self.screeplotfig.add_subplot(111)
		self.screeplotfig_canvas.draw()
		self.pcp_x_var.set("")
		self.pcp_y_var.set("")
		self.pcplotfig.clear()
		...
		if ok:
			...
			for col_idx in range(len(col_names)):
				...
				for i in range(len(key_col_vals)):
					cur.execute(f"update mapdata set {db_col_name} = '{col_table[col_idx][i]}' where {key_col_id} = '{key_col_vals[i]}';")
				# Update the Treeview
				if new_column:
