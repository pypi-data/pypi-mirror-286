...
class Plot(object):
	def __init__(self, frame, height=2, width=2):
...
class PairPlotDialog(object):
	def __init__(self, parent, column_specs):
		...
		self.content_frame.columnconfigure(0, weight=1)
		frame_panes.add(self.content_frame, weight=12)
		self.pairplot = Plot(self.content_frame, 4, 4)
...
class VariableDistDialog(object):
	...
	def set_xlabel(self, *args):
		...
		if xlabel is not None:
			self.xlabel = xlabel
			self.plotaxes.set_xlabel(xlabel)
			self.plot.canvas.draw()
	def set_ylabel(self, *args):
...
class PCADialog(object):
	...
	def pcplot_redraw(self, *args):
		...
		if self.scores is not None and xvar != '' and yvar != '':
			...
			def hover(event):
				vis = self.annot.get_visible()
				if event.inaxes == self.pcplot.axes:
					cont, ind = splot.contains(event)
					if cont:
						update_annot(ind)
						self.annot.set_visible(True)
						...
						if vis:
							self.annot.set_visible(False)
					self.pcplot.canvas.draw_idle()
			self.annot = self.pcplot.axes.annotate("", xy=(0,0), xytext=(8,12), textcoords="offset points", \
					bbox={"boxstyle": "round", "fc": "w"}, arrowprops={"arrowstyle": "->"})
			self.annot.set_visible(False)
