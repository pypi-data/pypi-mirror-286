...
def _extract_matching_columns_from_schema(model: type[Base], schema: type[BaseModel] | list | None) -> list[Any]:
    ...
    if schema is not None:
        ...
        column_list = [getattr(model, column_name) for column_name in schema_fields if hasattr(model, column_name)]
    return column_list
...
def _extract_matching_columns_from_kwargs(model: type[Base], kwargs: dict) -> list[Any]:
    if kwargs is not None:
        ...
        column_list = [getattr(model, column_name) for column_name in kwargs_fields if hasattr(model, column_name)]
    return column_list


def _extract_matching_columns_from_column_names(model: type[Base], column_names: list) -> list[Any]:
    return [getattr(model, column_name) for column_name in column_names if hasattr(model, column_name)]
