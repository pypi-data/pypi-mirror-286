...
# DRD - AI Powered Command Line Assistant

DRD is an AI-powered command line assistant that helps you with various tasks directly from your terminal.
...
To use DRD, simply type `drd` followed by your request:

```
drd 'What's the weather like today?'
```

For more complex tasks or to execute commands, use the `--do` flag:

```
drd --do 'Create a Python script that prints "Hello, World!"'
```
...
You can configure DRD by creating a `.drdrc` file in your home directory. Here's an example configuration:
