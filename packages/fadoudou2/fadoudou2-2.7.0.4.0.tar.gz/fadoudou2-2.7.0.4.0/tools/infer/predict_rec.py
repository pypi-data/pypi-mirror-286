...
import os
import re
def extract_image_filenames_from_text(text):
    # 使用正则表达式从文本中提取以.jpg结尾的文件名
    pattern = r'\b(\w+\.jpg)\b'
    image_filenames = re.findall(pattern, text)
    return image_filenames
def main(args):
    file_path = 'Z:\伟皓\OCR样本/rec_gt_U0FoT0Madu.txt'
    with open(file_path, 'r', encoding='utf-8') as file:
        content = file.read()
        txt_image_list = extract_image_filenames_from_text(content)
    image_file_list=[os.path.join(args.image_dir, filename) for filename in txt_image_list]
