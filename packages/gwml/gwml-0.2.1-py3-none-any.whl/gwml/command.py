...
def downloadWhlFile(downloadList):
...
def handle_push():
    ...
    # whl 파일이 있는지 검사-없으면 requirements.txt 보고 자동 다운로드
    if "whlFileAutoDownload" not in opt or opt["whlFileAutoDownload"] == False:
    ...
    if whlFileAutoDownloadQ == "y" or opt["whlFileAutoDownload"]:
        with open("./requirements.txt", 'r') as file:
            downloadList = file.readlines()
        downloadList = downloadWhlFile(downloadList)
        
    # 쉘 스크립트 작성
    shName = 'run.sh'
    content = """#!/bin/bash
        pip install ./download_packages/*
        """
