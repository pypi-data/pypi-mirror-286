...
def filter_dataframe(options: LoadOptions, df: pl.DataFrame) -> pl.DataFrame:
    ...
    if options.relation_filter:
        ...
        if isinstance(f, FilterGroup) and len(f.filters) > 0 and isinstance(f.filters[0], FilterGroup):
            ...
            rel = [{prop_dict[i.key]: __get_converted_value(df, prop_dict[i.key], i.value) for i in f.filters} for f in group.filters]
...
def __get_converted_value(df: pl.DataFrame | pl.LazyFrame, alias: str, value: any):
    """
    Liefert den konvertierten Wert für die Spalte mit dem Alias
    :param df: DataFrame
    :param alias: str
    :param value: any
    :return: any
    ...
    data_type = df.collect_schema()[alias]
