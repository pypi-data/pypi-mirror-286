...
from maitai_gen.chat import ChatCompletionParams, ChatMessage, EvaluationContentType
        ...
        *,
        ...
        if server_side_inference:
            ...
            response = maitai.Inference.infer(session_id, reference_id, intent, application_ref_name, completion_params, apply_corrections, evaluation_enabled, callback, timeout=response_timeout,
                                              context_retrieval_enabled=context_retrieval_enabled, context_query=context_query, return_request=return_request, fallback_model=fallback_model, user_id=user_id)
            if stream:
                return _process_inference_stream(response)
            # ChatCompletion only
            chat_completion: Optional[MaitaiCompletion] = None
