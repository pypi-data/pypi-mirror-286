...
class _MaterPreProcess:
    ...
    def _inputs_setup(self, input_file: str):
        ...
        for input in input_dict:
            ...
            if input == "vectors":
                ...
                os.makedirs(
                    os.path.join(self.output_dir_path, self.run_name, input),
                    exist_ok=True,
                )

                # save the inputs into parquet format
                df2.stack(future_stack=True).to_frame("value").to_parquet(
                    os.path.join(self.output_dir_path, self.run_name, input, input + ".mater"),
                    engine="pyarrow",
                    compression="snappy",
                )
    ...
    def _control_flow_trade_inverse_computation(self):
        ...
        pseudoinverses["location_production"] = [self.vector["location"]] * len(pseudoinverses)
