...
class Mater(_MaterProcessNonPhysical):
    ...
    def run(
        ...
        input_file: str = "MATER_v0-3-0.xls",
    ):
        """Run the MATER model.

        Execute a run of the MATER model from `simulation_start_time` to `simulation_end_time` using the input from the `input_file`.
        ...
        :param run_name: Name of the scenario to run. It will create a new directory in your working directory containing all the output data in `Parquet <https://parquet.apache.org/docs/overview/>`_ files.
        :type run_name: str
        ...
        :param input_file: The name of the input Excel file, defaults to "MATER_v0-1-2.xls"
    ...
    def _extraneous_flows_computation(self, k: int):
        ...
        .. math::
            F^e_{l}(k) = F^{thP}_{l}(k) + \sum_{e_s}[E^{eff}_{l,o_s}(k) \odot F^{thP}_{l,o_s}(k)]
