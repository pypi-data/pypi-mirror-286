...
# ----------------------------------------------------------------------------------------------------------------------------------------  
    async def doRunServer(self):
        try:
            ...
            ssl_keyfile_password = CSetting.getInstance().doGet("CYBORGAI_SSL_KEY_PASSWORD")
            ssl_keyfilePem = CSetting.getInstance().doGet("CYBORGAI_SSL_KEY")
            ssl_certfilePem = CSetting.getInstance().doGet("CYBORGAI_SSL_CERT")
           
            IuLog.doInfo(__name__, f"SSL key password: {not IuText.StringEmpty(ssl_keyfile_password)}")
            IuLog.doInfo(__name__, f"SSL key: {not IuText.StringEmpty(ssl_keyfilePem)}")
  
            if not IuText.StringEmpty(ssl_keyfilePem) and not IuText.StringEmpty(ssl_certfilePem):
                
                ssl_keyfilePem = ssl_keyfilePem.replace("\\n", "\n")
                ssl_certfilePem= ssl_certfilePem.replace("\\n", "\n")
                
                IuLog.doInfo(__name__, f"SSL Cert:\n{ssl_certfilePem}")
                
                ssl_keyfile = "/tmp/cyborgai_key.pem"
                ssl_certfile = "/tmp/cyborgai_cert.pem"

                await IuFile.doWrite(ssl_keyfile, ssl_keyfilePem.encode())
                await IuFile.doWrite(ssl_certfile, ssl_certfilePem.encode())
                
                print(IuCryptHash.toSha256(ssl_keyfilePem.encode()))
                print(IuCryptHash.toSha256(ssl_certfilePem.encode()))
                
                IuLog.doWarning(__name__, f"ssl_keyfile_password:{ssl_keyfile_password}")
                 # Log file content for debugging
                with open(ssl_keyfile, 'r') as keyfile:
                    IuLog.doWarning(__name__, f"Key file content:\n{keyfile.read()}")
                with open(ssl_certfile, 'r') as certfile:
                    certTmp = certfile.read()
                    hash = IuCryptHash.toSha256(certTmp)
                    IuLog.doWarning(__name__, f"Cert file content:\n{hash}")
                
                context = ssl.SSLContext(ssl.PROTOCOL_TLS_SERVER)
                context.load_cert_chain(certfile=ssl_certfile, keyfile=ssl_keyfile, password=ssl_keyfile_password)
