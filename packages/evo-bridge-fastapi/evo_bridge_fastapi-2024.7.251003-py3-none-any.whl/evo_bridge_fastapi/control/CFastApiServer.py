    ...
    async def doRunServer(self):
        try:
            ...
            if not IuText.StringEmpty(ssl_keyfilePem) and not IuText.StringEmpty(ssl_certfilePem):
                ...
                ssl_keyPem = ssl_keyfilePem.replace('\\n', '\n').removesuffix("\n") 

                ssl_certPem= ssl_certfilePem.replace('\\n', '\n').removesuffix("\n") 

                
                IuLog.doInfo(__name__, f"SSL Cert:\n{ssl_certPem}")
                
                ssl_keyFile = "./cyborgai_key.pem"
                ssl_certFile = "./cyborgai_cert.pem"
                ...
                with open(ssl_certFile, 'r') as certfile:
                    ...
                    print(f"Cert file hash:\n{hash}")
                
                context = ssl.SSLContext(ssl.PROTOCOL_TLS_SERVER)
                context.load_cert_chain(certfile=ssl_keyFile, keyfile=ssl_certFile, password=ssl_keyfile_password)
