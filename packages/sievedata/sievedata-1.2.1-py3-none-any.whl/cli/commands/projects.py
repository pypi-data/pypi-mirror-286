...
@click.option(
    "--limit", "-l", type=int, default=100, help="maximum number of results to return"
)
...
@click.option(
    ...
    help="workflow to use for project",
)
@click.option("--fps", type=int, default=30, help="frames per second")
@click.option(
    "--store-data", type=bool, default=True, help="store data in Sieve's system"
)
@click.option(
    "--source-name", type=str, help="name of resource to initialize project with"
)
@click.option("--source-url", type=str, help="url of source to initialize project with")
@click.option(
    "--source-path",
    type=click.Path(exists=True),
    help="path to local source to initialize project with",
)
@click.option(
    "--last-layer", type=int, default=-1, help="last layer to use for project"
)
def main(
    project_name,
    command,
