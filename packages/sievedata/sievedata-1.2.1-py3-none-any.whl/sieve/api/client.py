...
class SieveJob:
    ...
    def from_project_and_id(cls, project_name: str, job_id: str) -> SieveJob:
        ...
        if job_json.status_code != 200:
            raise Exception(f"Error getting job {job_id} from project {project_name}")
...
class SieveProject:
    ...
    def from_json(cls, project_json) -> SieveProject:
        ...
        return SieveProject(
            ...
            workflow=SieveWorkflow(
                [
                    SieveLayer.from_json(layer_json)
    ...
    def __init__(
        self, name: str, workflow: SieveWorkflow, fps: int, store_data: bool
    ) -> None:
        self.name = name
    ...
    def create(self):
        ...
        res = sieve_post(
            ...
            data={PROJECT_CREATE_CONFIG: self.dict, PROJECT_NAME: self.name},
        )
        if res.status_code == 200:
            res_json = res.json()
...
class SieveClient:
    ...
    def list_projects(self) -> List[SieveProject]:
        ...
        if res.status_code == 200:
            res_json = res.json()
            return [
                SieveProject.from_json(project_json)
                for project_json in res_json["data"]
            ]
        else:
            try:
                res_json = res.json()
    ...
    def list_jobs(
        ...
        Returns:
            a list of SieveJob objects
        """
        return SieveProject.from_name(project_name, load_project_info=False).list_jobs(
    ...
    def push(
        ...
        Returns:
            a SieveJob object
        """
        return SieveProject.from_name(project_name, load_project_info=False).push(
            source_name, source_url, local_upload=local_upload
        )
    ...
    def search_feedback(
        ...
        print(
            f"{API_URL}/{API_BASE}/feedback/search?offset={offset}&limit={limit}", query
        )
        res = sieve_post(
    ...
    def _paginate(self, url: str, data: dict, limit: int = 1000):
        ...
        res = sieve_post(f"{url}?offset={offset}&limit={limit}", data=data)
        res_json = res.json()
    ...
    def query_intervals(
        self, query: Dict, limit: int = 7500, offset: int = 0, paginate: bool = False
    ):
    ...
    def dict(self) -> Dict:
        ...
        """
        if hasattr(self, "_name"):
            return {USER_NAME: self._name, USER_API_KEY: self.api_key}
        return {USER_NAME: self.name, USER_API_KEY: self.api_key}
    ...
    def from_json(cls, json: Dict) -> SieveClient:
        ...
        """
        return cls(name=json[USER_NAME], api_key=json[USER_API_KEY])
