...
#Like get schema but for the output to unwrap it into a dict
def get_env_list_dict(env_list):
    try:
        return [i.dict() for i in env_list]
    except:
        raise TypeError("Badly supplied env vars, make sure you supply a list of sieve.Env config variables", env_list)