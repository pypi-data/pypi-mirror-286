...
from model.net.base_blocks import ResBlock, StyleConv, ToRGB


class ENetPitch(nn.Module):
    def __init__(
            self,
            num_style_feat=512,
            lnet=None,
    ...
    def forward(self, audio_sequences, f0, face_sequences, gt_sequences):
        ...
        feat = F.leaky_relu_(self.conv_body_first(F.interpolate(ref, size=(256, 256), mode='bilinear')),
                             negative_slope=0.2)
        ...
        if self.concat:
            low_res_img, low_res_feat = self.low_res(audio_sequences, f0, LNet_input)
            low_res_img.detach()
            low_res_feat.detach()
            out = torch.cat([low_res_img, low_res_feat], dim=1)

        else:
            low_res_img = self.low_res(audio_sequences, f0, LNet_input)
            low_res_img.detach()
            # 96 x 96
            out = low_res_img
        p2d = (2, 2, 2, 2)
