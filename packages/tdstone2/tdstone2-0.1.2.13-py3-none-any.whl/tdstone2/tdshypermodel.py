...
class HyperModel():
    """
    Handles the lifecycle of hyperparameterized models, including initialization, model registration, training, scoring, and model management.

    Attributes:
        id (str): Unique identifier for the hypermodel process.
        tdstone (TDStone): Instance of the TDStone class for database operations.
        mapper_training (Mapper): Mapper instance for training data.
        mapper_scoring (Mapper): Mapper instance for scoring data.
        id_model (str): Identifier for the associated model.
        fold_training (Any): Specifies the fold for training, can be a specific fold or a strategy for fold selection.
        metadata (dict): Metadata associated with the hypermodel process.
