...
class JelverTestException(Exception):
    def __init__(self, message, failed_cases):
        self.message = message
    ...
    def __str__(self):
        ...
        if len(self.failed_cases):
            message += f"Number of failed tests {len(self.failed_cases)}. Please check the logs for more information"
        ...
        for case in self.failed_cases:
            ...
            case_error = '\n'.join([error for error in case['errors']])
            message_error += '\n\n' + \
               Style.DIM + \
               f'Case ID: {case_id}\n' + \
