...
class SuffixAutomaton:
    ...
    def lcs1(self, t: List[str], min_len: int = -1):
        """
        return [(start, length, cand_start)]
        ...
        for endpos, length, cand_start in cands:
            if length >= min_len:
                start = self.nodes[endpos].position+1-length
                if ans and start <= ans[-1][1]:
                    ans[-1] = (start, length, cand_start)
                else:
                    ans.append((start, length, cand_start))
        return ans
