...
class sequence_processor():
    ...
    def read_inputs(self, input_p):
        ...
        if input_p.name.endswith(".h5"): # if the embedding file already exists
        ...
        elif input_p.name.endswith(".fasta") or input_p.name.endswith(".faa"): # compute new embeddings if only FASTA available
            ...
            with open(emb_p,'wb') as f:
                ...
                #pickle.dump({i: embeddings_[idx] for idx,i in enumerate(ids)},f)
