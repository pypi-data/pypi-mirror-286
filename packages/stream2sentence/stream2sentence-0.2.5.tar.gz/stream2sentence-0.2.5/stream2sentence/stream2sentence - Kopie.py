    ...
    for char in _generate_characters(generator, log_characters):
        ...
        if char:
            ...
            if (is_first_sentence
                ...
                if (buffer[-1] in sentence_fragment_delimiters
                    ...
                    yield_text = _clean_text(
                        ...
                        cleanup_text_emojis)
                    if debug_logging:
                        print(f"s2s: yield 1st {yield_text}, set buffer='', is_first_sentence = False")
                    yield yield_text
                    buffer = ""
            ...
            if delimiter_char in sentence_fragment_delimiters:
                ...
                if debug_logging:
                    print(f"s2s: sentences tokenizer {sentences}")
                if len(sentences) > 1:
                    ...
                    # print(f"DBG # total_length_except_last: {total_length_except_last}, len(buffer): {len(buffer)}, context_size: {context_size}")
                    # print(f"DBG # total_length_except_last: {total_length_except_last}, len(buffer) - context_size + 1: {len(buffer) - context_size + 1}")

                    # # print(f"DBG # len(sentences[0]): {len(sentences[0])}, len(buffer): {len(buffer)}, context_size: {context_size}")
                    # # print(f"DBG # len(sentences[0]): {len(sentences[0])}, len(buffer) - context_size + 1: {len(buffer) - context_size + 1}")
                    # if total_length_except_last == len(buffer) - context_size + 1:
                    #     if debug_logging:
                    #         print(f"s2s: yield tokenizer {buffer[:-context_size + 1]}")
                    #     yield_text = _clean_text(
                    ...
                    #     if debug_logging:
                    #         print(f"s2s: AFTER yield tokenizer buffer {buffer}")
                    #     is_first_sentence = False
    ...
    if buffer:
        ...
        for sentence in sentences:
            ...
            yield_text = _clean_text(
                ...
                cleanup_text_emojis)
            if debug_logging:
                print(f"s2s: yield remaining {yield_text}, set sentence_buffer=''")
            yield yield_text
        ...
        if sentence_buffer:
            yield_text = _clean_text(
                ...
                cleanup_text_emojis)
            if debug_logging:
                print(f"s2s: yield last sentence_buffer {yield_text}")
            yield yield_text
...
#         return _clean_text(text, cleanup_text_links, cleanup_text_emojis)
...
#             # Handle first sentence fragment
#             if is_first_sentence and len(buffer) >= minimum_first_fragment_length:
#                 if quick_yield_single_sentence_fragment:
#                     if (buffer[-1] in sentence_fragment_delimiters
#                             or word_count >= force_first_fragment_after_words):
#                         yield yield_sentence(buffer)
#                         buffer = ""
#                         is_first_sentence = False
#                         word_count = 0
#                         continue
#                 elif len(buffer) >= minimum_sentence_length:
#                     sentences = _tokenize_sentences(buffer, tokenize_sentences)
#                     if sentences:
#                         yield yield_sentence(sentences[0])
#                         buffer = buffer[len(sentences[0]):].lstrip()
#                         is_first_sentence = False
#                         word_count = 0
#                         continue
...
#                 sentences = _tokenize_sentences(buffer[:-context_size], tokenize_sentences)
#                 if sentences:
#                     complete_sentences = sentences[:-1]
#                     for sentence in complete_sentences:
#                         if len(sentence) >= minimum_sentence_length:
#                             yield yield_sentence(sentence)
#                         else:
#                             buffer = sentence + ' ' + buffer[len(sentence):].lstrip()
#                             break
#                     else:
#                         buffer = sentences[-1] + buffer[-context_size:]

#     # Handle remaining buffer
#     if buffer:
#         sentences = _tokenize_sentences(buffer, tokenize_sentences)
#         for sentence in sentences:
#             if len(sentence) >= minimum_sentence_length or sentence == buffer:
#                 yield yield_sentence(sentence)
#             elif sentences[-1] == sentence:
#                 yield yield_sentence(sentence)