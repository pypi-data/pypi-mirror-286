...
def generate_sentences(generator: Iterator[str],
    ...
    Args:
        ...
        quick_yield_for_all_sentences (bool): If set to True, the
          generator will yield every sentence fragment as quickly as
          possible. 
    ...
    for char in _generate_characters(generator, log_characters):
        ...
        if char:
            ...
            outputstr = f" is_first_sentence: {is_first_sentence}, len(buffer) > minimum_first_fragment_length : {len(buffer) > minimum_first_fragment_length}, quick_yield_single_sentence_fragment: {quick_yield_single_sentence_fragment}"
            ...
            # context_size_look_overhead = 10
            # max_look_behind = len(buffer) - context_size - 1 - context_size_look_overhead
            # if max_look_behind < 0:
            #     max_look_behind = 0
            # look_behind = min(max_look_behind, len(buffer) - context_size - 1)
            # if (last_delimiter_position >= 0 and 
            #     0 <= len(buffer) - last_delimiter_position - 1 <= look_behind):
            ...
            # if delimiter_char in full_sentence_delimiters:
            #     sentences = _tokenize_sentences(buffer, tokenize_sentences)
            #     if len(sentences) > 1:
