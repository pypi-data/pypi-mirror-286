...
@dataclass
class CommandMassiveInvoiceReceptionElectronica:
    nit: int
    cuis: str
    cufd: str
...
@dataclass
class ResponseMassiveInvoiceReceptionElectronica:
    reception_code: str
    raw_response: Any


@feature(CommandMassiveInvoiceReceptionElectronica)
class MassiveReceptionElectronica(UseCase):
    def wsdl(self) -> str:
        return SIAT_WSDL.SERVICIOS_ELECTRONICA

    def execute(
        self, param_object: CommandMassiveInvoiceReceptionElectronica
