...
class CommandRevertCancelledCreditNote:
    sector_document: int
    emission_code: int
    point_of_sale: int
    system_code: str
...
@dataclass
class ResponseRevertCancelledCreditNote:
    reception_code: str
    literal_status: type_verified_status
    raw_response: dict


@feature(CommandRevertCancelledCreditNote)
class RevertCancelledCreditNote(UseCase):
    def wsdl(self) -> str:
        return SIAT_WSDL.NOTA_DE_CREDITO

    def execute(
        self, param_object: CommandRevertCancelledCreditNote
    ) -> ResponseRevertCancelledCreditNote:
        response = self.client.service.reversionAnulacionDocumentoAjuste(
