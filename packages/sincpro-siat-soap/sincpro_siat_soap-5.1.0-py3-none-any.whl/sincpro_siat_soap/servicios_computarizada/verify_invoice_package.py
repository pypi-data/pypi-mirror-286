...
@dataclass
class CommandVerifyInvoicePackageComputarizada:
    document_type: int
    emission_code: int
    point_of_sale: int
    system_code: str
...
@dataclass
class ResponseVerifyInvoicePackageComputarizada:
    reception_code: str
    raw_response: dict


@feature(CommandVerifyInvoicePackageComputarizada)
class VerifyInvoicePackageSiat(UseCase):
    def wsdl(self) -> str:
        return SIAT_WSDL.SERVICIOS_COMPUTARIZADA

    def execute(
        self, param_object: CommandVerifyInvoicePackageComputarizada
    ) -> ResponseVerifyInvoicePackageComputarizada:
        response = self.client.service.validacionRecepcionPaqueteFactura(
