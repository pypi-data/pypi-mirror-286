...
@dataclass
class CommandVerifyInvoiceStateComputarizada:
    document_type: int
    emission_code: int
    point_of_sale: int
    system_code: str
...
@dataclass
class ResponseVerifyInvoiceStateComputarizada:
    reception_code: str
    literal_status: type_verified_status
    raw_response: dict


@feature(CommandVerifyInvoiceStateComputarizada)
class VerifyInvoiceStateSiat(UseCase):
    def wsdl(self) -> str:
        return SIAT_WSDL.SERVICIOS_COMPUTARIZADA

    def execute(
        self, param_object: CommandVerifyInvoiceStateComputarizada
