...
@dataclass
class CommandCompressFile:
    string_file: Union[str, List[str]] = field(repr=False)


@dataclass
class ResponseCompressFile:
    zip_file: Any = field(repr=False)


@feature(CommandCompressFile)
class CompressFile(UseCase):
    def execute(self, param_object: CommandCompressFile) -> ResponseCompressFile:
        ...
        response = None
        if isinstance(param_object.string_file, str):
            response = CompressFile.compress_one(param_object.string_file)

        if isinstance(param_object.string_file, list):
            response = CompressFile.compress_many_to_targz(param_object.string_file)
    ...
    def compress_one(string_file: str):
        with tempfile.NamedTemporaryFile(suffix=".xml", prefix="") as file:
            logger.info(f"Creating XML file: {file.name}")
            logger.debug(f"Dumping the string into a temporary XML file")
            file.write(string_file.encode("utf-8"))
            file.seek(0)
            xml_file = file.read()

        temporary_file_for_zip = tempfile.NamedTemporaryFile(suffix=".zip", prefix="")
        logger.debug(f"Creating zip file: {temporary_file_for_zip.name}")

        with temporary_file_for_zip as zip_file:
            with gzip.GzipFile(fileobj=zip_file, mode="w") as compress:
                logger.info("Compressing the XML file in [ZIP]")
                compress.write(xml_file)

            logger.info("The compression was executed SUCCESSFULL")
            zip_file.seek(0)
    ...
    def compress_many(string_files: List[str]):
        with tempfile.NamedTemporaryFile(suffix=".zip", prefix="") as zipped_files:
            logger.info(f"Creating zip file: {zipped_files.name}")
    ...
    def compress_many_to_targz(string_files: List[str]):
        ...
        with tempfile.TemporaryDirectory(
            ...
            logger.debug(f"Creating temporary folder for invioces: {temporary_folder}")
            ...
            for index, string_file in enumerate(string_files):
                with tempfile.NamedTemporaryFile(
                    ...
                    logger.info(f"Creating temporary xml: {temp_xml.name}")
            ...
            with tarfile.open(path_tar_file, mode="w:gz") as compress_tar_funcs:
                for xml_file in xml_files:
                    logger.debug(f"Adding xml file: [{xml_file[0]}-{xml_file[1]}]")
                    compress_tar_funcs.add(xml_file[0], xml_file[1])

            with open(path_tar_file, "rb") as tmp_targz_file:
                logger.info(f"Read/get binary from temporary file [{path_tar_file}]")
