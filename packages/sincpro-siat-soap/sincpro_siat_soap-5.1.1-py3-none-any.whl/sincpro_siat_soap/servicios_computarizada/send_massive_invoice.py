...
@dataclass
class CommandMassiveInvoiceReceptionComputarizada:
    nit: int
    cuis: str
    cufd: str
...
@dataclass
class ResponseMassiveInvoiceReceptionComputarizada:
    reception_code: str
    raw_response: Any


@feature(CommandMassiveInvoiceReceptionComputarizada)
class MassiveReceptionComputarizada(UseCase):
    def wsdl(self) -> str:
        return SIAT_WSDL.SERVICIOS_COMPUTARIZADA

    def execute(
        self, param_object: CommandMassiveInvoiceReceptionComputarizada
    ) -> ResponseMassiveInvoiceReceptionComputarizada:
        response = self.client.service.recepcionMasivaFactura(
