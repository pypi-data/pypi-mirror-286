...
@dataclass
class CommandVerifyMassiveInvoiceComputarizada:
    document_type: int
    emission_code: int
    point_of_sale: int
    system_code: str
...
@dataclass
class ResponseVerifyMassiveInvoiceComputarizada:
    reception_code: str
    raw_response: dict


@feature(CommandVerifyMassiveInvoiceComputarizada)
class VerifyMassiveInvoiceSiatComputarizada(UseCase):
    def wsdl(self) -> str:
        return SIAT_WSDL.SERVICIOS_COMPUTARIZADA

    def execute(
        self, param_object: CommandVerifyMassiveInvoiceComputarizada
