...
@dataclass
class CommandRequestPurchaseConfirmation:
    point_of_sale: int
    system_code: str
    branch_office: int
    cufd: str
...
@dataclass
class ResponsePurchaseConfirmation:
    raw_response: Any
    # success: bool


@feature(CommandRequestPurchaseConfirmation)
class RequestPurchaseConfirmation(UseCase):
    def wsdl(self) -> str:
        return SIAT_WSDL.COMPRAS

    def execute(
        self, param_object: CommandRequestPurchaseConfirmation
    ) -> ResponsePurchaseConfirmation:
        response = self.client.service.confirmacionCompras(
        ...
        return ResponsePurchaseConfirmation(raw_response=response)
