...
@dataclass
class CommandReceptionCreditDebitNote:
    nit: int
    cuis: str
    cufd: str
...
@dataclass
class ResponseReceptionCreditDebitNote:
    reception_code: str
    raw_response: Any


@feature(CommandReceptionCreditDebitNote)
class ReceptionCreditDebitNote(UseCase):
    def wsdl(self) -> str:
        return SIAT_WSDL.NOTA_DE_CREDITO

    def execute(
        self, param_object: CommandReceptionCreditDebitNote
    ) -> ResponseReceptionCreditDebitNote:
        response = self.client.service.recepcionDocumentoAjuste(
