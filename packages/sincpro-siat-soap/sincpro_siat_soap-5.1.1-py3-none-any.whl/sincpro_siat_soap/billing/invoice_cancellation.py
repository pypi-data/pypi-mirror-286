...
@dataclass
class CommandInvoiceCancellationRequest:
    nit: int
    cuis: str
    cufd: str
    cuf: str
...
@dataclass
class ResponseInvoiceCancellation:
    transaction: bool
    raw_response: Any


@feature(CommandInvoiceCancellationRequest)
class InvoiceCancellationRequest(UseCase):
    def wsdl(self) -> str:
        return SIAT_WSDL.FACTURA_COMPRA_VENTA

    def execute(
        self, param_object: CommandInvoiceCancellationRequest
