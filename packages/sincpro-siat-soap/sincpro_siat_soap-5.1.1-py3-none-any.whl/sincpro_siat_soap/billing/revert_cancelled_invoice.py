...
class CommandRevertCancelledInvoice:
    sector_document: int
    emission_code: int
    point_of_sale: int
    system_code: str
...
@dataclass
class ResponseRevertCancelledInvoice:
    reception_code: str
    literal_status: type_verified_status
    raw_response: dict


@feature(CommandRevertCancelledInvoice)
class RevertCancelledInvoice(UseCase):
    def wsdl(self) -> str:
        return SIAT_WSDL.FACTURA_COMPRA_VENTA

    def execute(
        self, param_object: CommandRevertCancelledInvoice
