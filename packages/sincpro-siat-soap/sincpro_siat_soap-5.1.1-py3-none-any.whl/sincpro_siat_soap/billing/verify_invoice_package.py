...
@dataclass
class CommandVerifyInvoicePackage:
    document_type: int
    emission_code: int
    point_of_sale: int
    system_code: str
...
@dataclass
class ResponseVerifyInvoicePackage:
    reception_code: str
    raw_response: dict


@feature(CommandVerifyInvoicePackage)
class VerifyInvoicePackageSiat(UseCase):
    def wsdl(self) -> str:
        return SIAT_WSDL.FACTURA_COMPRA_VENTA

    def execute(
        self, param_object: CommandVerifyInvoicePackage
    ) -> ResponseVerifyInvoicePackage:
