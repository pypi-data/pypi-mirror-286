...
@dataclass
class CommandVerifyInvoicePackageElectronica:
    document_type: int
    emission_code: int
    point_of_sale: int
    system_code: str
...
@dataclass
class ResponseVerifyInvoicePackageElectronica:
    reception_code: str
    raw_response: dict


@feature(CommandVerifyInvoicePackageElectronica)
class VerifyInvoicePackageSiat(UseCase):
    def wsdl(self) -> str:
        return SIAT_WSDL.SERVICIOS_ELECTRONICA

    def execute(
        self, param_object: CommandVerifyInvoicePackageElectronica
