...
async def autodownload_handler(message: Message, command: CommandObject) -> None:
    print('✈️ autodownload():')
    print()
    print('🗿 Sender chat: ', message.sender_chat.id, ' : ', message.sender_chat.title)

    if check_chat_id_in_dict(message.sender_chat.id):
        del autodownload_chat_ids_hashed[get_hash_salted(message.sender_chat.id)]
        await message.reply(f'Remove from Dict: {get_hash_salted(message.sender_chat.id)}')
    else:
        autodownload_chat_ids_hashed[get_hash_salted(message.sender_chat.id)] = None
        await message.reply(f'Add to Dict: {get_hash_salted(message.sender_chat.id)}')
