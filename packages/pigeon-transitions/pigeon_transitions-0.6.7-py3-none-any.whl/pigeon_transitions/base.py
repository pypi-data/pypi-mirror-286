...
class BaseMachine(Machine):
    ...
    def __init__(
        ...
        * States haves callbacks transformed using _to_callable and _get_callable.
        * Transitions have callbacks and conditions transformed using _to_callable and _get_callable.
    ...
    def _to_callable(self, data, callbacks):
        """Returns transformed lists of state or transition data with the specified
        callbacks transformed from strings to references to class member functions
        when available."""
        if data is None:
            return data
        for el in data:
...
class RootMachine(BaseMachine):
    ...
    def __init__(self, logger=None, **kwargs):
        ...
        * A logger is created if not provided.
        ...
        self.parent = None
        self._collected = {}
        self.style_attributes["node"]["active"] = self.style_attributes["node"]["inactive"]
        self.style_attributes["node"]["previous"] = self.style_attributes["node"]["inactive"]
        super().__init__(
