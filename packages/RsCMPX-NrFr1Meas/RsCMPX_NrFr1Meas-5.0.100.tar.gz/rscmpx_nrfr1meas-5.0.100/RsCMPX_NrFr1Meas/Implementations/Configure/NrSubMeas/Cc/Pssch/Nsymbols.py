...
class NsymbolsCls:
	...
	def set(self, no_symbols: int, carrierComponentOne=repcap.CarrierComponentOne.Nr1) -> None:
		"""SCPI: CONFigure:NRSub:MEASurement<Instance>[:CC<no>]:PSSCh:NSYMbols \n
		Snippet: driver.configure.nrSubMeas.cc.pssch.nsymbols.set(no_symbols = 1, carrierComponentOne = repcap.CarrierComponentOne.Nr1) \n
		...
		self._core.io.write(f'CONFigure:NRSub:MEASurement<Instance>:CC{carrierComponentOne_cmd_val}:PSSCh:NSYMbols {param}')

	def get(self, carrierComponentOne=repcap.CarrierComponentOne.Nr1) -> int:
		"""SCPI: CONFigure:NRSub:MEASurement<Instance>[:CC<no>]:PSSCh:NSYMbols \n
		Snippet: value: int = driver.configure.nrSubMeas.cc.pssch.nsymbols.get(carrierComponentOne = repcap.CarrierComponentOne.Nr1) \n
		No command help available \n
			:param carrierComponentOne: optional repeated capability selector. Default value: Nr1
			:return: no_symbols: No help available"""
		carrierComponentOne_cmd_val = self._cmd_group.get_repcap_cmd_value(carrierComponentOne, repcap.CarrierComponentOne)
		response = self._core.io.query_str(f'CONFigure:NRSub:MEASurement<Instance>:CC{carrierComponentOne_cmd_val}:PSSCh:NSYMbols?')
