...
class AllCls:
	...
	def fetch(self, timeout: float = None, target_main_state: enums.TargetStateA = None, target_sync_state: enums.TargetSyncState = None) -> List[enums.ResourceState]:
		...
		Snippet: value: List[enums.ResourceState] = driver.nrSubMeas.srs.state.all.fetch(timeout = 1.0, target_main_state = enums.TargetStateA.OFF, target_sync_state = enums.TargetSyncState.ADJusted) \n
		...
		response = self._core.io.query_str(f'FETCh:NRSub:MEASurement<Instance>:SRS:STATe:ALL? {param}'.rstrip())
