...
class PreferenceData(BaseModel):
    ...
    def has_download_link(self):
        download_link = _get_download_link(self.data_name)
        return download_link is not None

    def get_download_link(self):
        while (status := self.get_job_status()) == 'PENDING':
            print(f'Job {self.data_name} status is {status}...')
            sleep(5)
        print(f'Job {self.data_name} status is {status}')

        url = _get_download_link(self.data_name)
        return url
...
def _get_download_link(data_name):
    download_link = DataClient().get(f'/v1/big_data/get_download_link/{data_name}') if data_name else None
