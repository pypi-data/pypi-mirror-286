...
class GenerationParams(BaseModel, extra=Extra.allow, title=''):
    ...
    reward_max_tokens: Optional[int] = Field(
        default=config.DEFAULT_REWARD_MAX_TOKENS,
    ...
    def _validate_reward_max_tokens(self):
        ...
        is_not_none = self.reward_max_tokens is not None
        is_not_in_correct_range = not (min_allowed <= self.reward_max_tokens <= max_allowed)

        if is_not_none and is_not_in_correct_range:
            msg = f"`reward_max_tokens` must be in [{min_allowed}, {max_allowed}], got {self.reward_max_tokens}"
            self._validation_errors.append(msg)
