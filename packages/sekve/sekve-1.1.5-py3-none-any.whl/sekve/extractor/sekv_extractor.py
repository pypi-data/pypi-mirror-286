...
class Extractor(Params):
    ...
    def extract_sigmad_lambdad(self, df: pd.DataFrame,
        ...
        ----------
        df: pd.DataFrame
            A DataFrame containing IDVD with various gate voltages, from weak to strong inversion.
            The index is main sweep, i.e., drain voltage. The column is the secondary sweep, i.e., gate voltage.
            Cell values are the drain current.
        gds_ext_dict: dict, optional
            Dict with keywords passed to the :func:`~sekve.extractor.gds_extractor.extract_Gds`
            used to parameters of :func:`~sekve.extractor.gds_extractor.extract_Gds`.
...
def extract_idvg(
    ...
    i: numpy.ndarray
        ...
        source-to-drain current for pMOS.
    return_detail: bool
        If True, it returns the :class:`Extractor` containing extraction
        results and :class:`sekve.Model`
    ext_dict: dict, optional
        Dict with keywords passed to the :class:`Extractor`
