...
class Grouped_GEMM_MoE(torch.nn.Module):
    ...
    def __init__(
        ...
        if self.implementer_type is MOEImplmenterType.MegaBlocks and (
            ...
            if rank() is None or rank() == 0:
                logger.warning(
                    "AllGather token_dispatcher_type not supported yet for MegaBlocks implementation_type,\
        ...
        if self.use_swiglu and not self.merge_w1_v1:
            self.w1 = torch.nn.Parameter(
                torch.empty(self.num_local_experts, self.hidden_size, self.expert_intermediate_size)
            )
    ...
    def _forward_megatron(
        ...
        dispatched_input, tokens_per_expert = self.token_dispatcher.token_permutation(
            hidden_states, expert_weights, top_experts
        )

        # TODO: overlap with permutation all2all
        ...
        # TODO: overlap with unpermutation all2all
    ...
    def compute_moe(self, hidden_states, tokens_per_expert, bin_ids=None):
        ...
        if self.use_swiglu and not self.merge_w1_v1:
            x1 = gmm(hidden_states, self.w1, tokens_per_expert, trans_b=False)
            x2 = gmm(hidden_states, self.v1, tokens_per_expert, trans_b=False)
    ...
    def compute_single_expert(self, hidden_states):
        if self.use_swiglu and not self.merge_w1_v1:
            x1 = F.linear(hidden_states, self.w1.squeeze(0).transpose(0, 1), None)
            x2 = F.linear(hidden_states, self.v1.squeeze(0).transpose(0, 1), None)
            intermediate_states = F.silu(x1) * x2
        else:
            fc1_output = F.linear(
