...
from .aap import get_organizations, get_projects, create_organization, create_project
...
def login_aap_basicauth(url, user, password):
  ...
  resp = session.get(pingurl, verify=False)
...
def main():
    ...
    while True:
      ...
      projects = get_projects(url, session)
