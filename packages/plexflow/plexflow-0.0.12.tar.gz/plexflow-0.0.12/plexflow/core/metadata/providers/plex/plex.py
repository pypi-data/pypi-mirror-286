...
def search_movie_by_rating_key(key: str) -> Union[PlexMovieMetadata, None]:
    ...
    """
    hook = PlexAuthorizedHttpHook(http_conn_id="plex_discover_hook", config_folder="config")
    
    response = hook.run(
        endpoint=f"/library/metadata/{key}",
    )
...
def search_seasons_by_show_rating_key(key: str) -> Union[List[PlexSeasonMetadata], None]:
    ...
    response = hook.run(
        endpoint=f"/library/metadata/{key}/children",
    )
...
def get_watch_state_by_rating_key(key: str) -> Union[PlexUserState, None]:
    ...
    response = hook.run(
        endpoint=f"/library/metadata/{key}/userState",
    )
