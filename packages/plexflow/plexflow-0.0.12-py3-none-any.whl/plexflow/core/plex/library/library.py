...
from plexflow.core.plex.watchlist.watchlist import get_watchlist
...
def get_library_id(library_name: str) -> int:
    ...
    2
    """
    hook = PlexAuthorizedHttpHook(http_conn_id="plex_local_hook", config_folder="config")
    response = hook.run(endpoint="/library/sections")
...
def is_media_in_library(guid: str, library_name: str, media_type: str) -> bool:
    ...
    hook = PlexAuthorizedHttpHook(http_conn_id="plex_local_hook", config_folder="config")
    response = hook.run(endpoint=f"/library/sections/{library_id}/all", query_params={
        "guid": guid
    })
