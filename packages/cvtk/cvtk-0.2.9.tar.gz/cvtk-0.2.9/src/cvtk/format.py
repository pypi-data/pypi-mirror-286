...
class ImageAnnotation:
    ...
    Class :class:`ImageAnnotation <cvtk.format.ImageAnnotation>` is a container that stores image annotations such as file paths,
    ...
    Class :class:`ImageAnnotation <cvtk.format.ImageAnnotation>` mainly implements methods
    :func:`format <cvtk.format.ImageAnnotation.format>`
    and :func:`draw <cvtk.format.ImageAnnotation.draw>`
    for format conversion and image visualization, respectively.
    By using method :func:`format <cvtk.format.ImageAnnotation.format>`,
    the annotations can be converted to any format,
    such as the COCO format and Pascal VOC format.
    By using method :func:`draw <cvtk.format.ImageAnnotation.draw>`,
    the image is plotted with annotations.
    
       
    Examples:
        >>> from cvtk.format import ImageAnnotation
        >>> 
        ...
        if isinstance(ann, str):
            if ann.lower().endswith(('.png', '.jpg', '.jpeg', '.tiff', '.bmp', '.gif')):
                ...
                if os.path.exists(ann):
                    fdat = ''.join(open(ann, 'r').readlines())
    ...
    def format(self, annotation_format, file_path=None):
        ...
        Method :func:`format <cvtk.format.ImageAnnotation.format>` converts
        class object :class:`ImageAnnotation <cvtk.format.ImageAnnotation>`
        ...
        :class:`ImageAnnotation <cvtk.format.ImageAnnotation>`
...
'''
        ...
        if output_fpath is None:
            ...
            if os.path.basename(output_fpath) == 'md5':
                with open(os.path.join(os.path.dirname(output_fpath), tmpl['asset']['id'] + '-asset.json'), 'w', encoding='utf-8') as fh:
                    json.dump(tmpl, fh, cls=__JsonEncoder, ensure_ascii=False, indent=4)
    ...
    def draw(self, fig_type='mask', file_path=None, label=False, score=False, alpha=1.0, class2rgb=None):
        ...
        Method :func:`draw <cvtk.format.ImageAnnotation.draw>`
        ...
        Examples:
            ...
            >>> ann_fpath = './path/to/image.json'
            >>> 
            >>> ann = cvtk.format.ImageAnnotation(image_fpath, ann_fpath)
            >>>
            >>> ann.draw('bbox')
...
class ImageAnnotations:
    """List of :class:`ImageAnnocation <cvtk.format.ImageAnnotation>` objects
    
    :class:`ImageAnnotations <cvtk.format.ImageAnnotations>` stores
    :class:`ImageAnnotation <cvtk.format.ImageAnnotation>` class objects as a list object.
    To retrieve elements from an :class:`ImageAnnotations <cvtk.format.ImageAnnotationss>` object,
    indexing ``[i]`` and a ``for`` loop can be used.
    To add new elements to the end of :class:`ImageAnnotations <cvtk.format.ImageAnnotations>`,
    methods :func:`append <cvtk.format.ImageAnnotations.append>`
    and :func:`extend <cvtk.format.ImageAnnotations.extend>` can be used.
    In addition, this class implements method :func:`format <cvtk.format.ImageAnnotations.format>`
    to convert annotations into files in a specific format,
    such as COCO or Pascal VOC.
    Compared with :func:`format <cvtk.format.ImageAnnotation.format>` implemented in
    class :class:`ImageAnnotation <cvtk.format.ImageAnnotation>`
    that generates annotations in a specific format for a single image,
    :class:`format <cvtk.format.ImageAnnotations.format>` in this class
    can generate annotations for multiple images.

    Args:
        x: If ``x`` is ``None``, generate an empty
           :class:`ImageAnnocations <cvtk.format.ImageAnnotations>` object.
           If ``x`` is :class:`ImageAnnocation <cvtk.format.ImageAnnotation>`
           or :class:`ImageAnnocations <cvtk.format.ImageAnnotations>` object,
           convert them to :class:`ImageAnnocations <cvtk.format.ImageAnnotations>` object.
    
    Examples:
        >>> from cvtk.format import ImageAnnotation, ImageAnnotations
        ...
        >>> ann = cvtk.format.ImageAnnotations([ann_1, ann_2, ann_3])
    """
    ...
    def append(self, image_annotation):
        """Add an :class:`ImageAnnotation <cvtk.format.ImageAnnotation>` object to the end of the :class:`ImageAnnotations <cvtk.format.ImageAnnotations>` object
        
        Add an :class:`ImageAnnotation <cvtk.format.ImageAnnotation>` object
        to the end of the :class:`ImageAnnotations <cvtk.format.ImageAnnotations>` object.
        
        Args:
            image_annotation (ImageAnnotation): An :class:`ImageAnnotation <cvtk.format.ImageAnnotation>` object.
    ...
    def extend(self, image_annotations):
        """Concatenate :class:`ImageAnnotations <cvtk.format.ImageAnnotations>` object to the end of the :class:`ImageAnnotations <cvtk.format.ImageAnnotations>` object
        
        Concatenate :class:`ImageAnnotations <cvtk.format.ImageAnnotations>` object
        or a list of :class:`ImageAnnotation <cvtk.format.ImageAnnotation>` objects
        to the end of the :class:`ImageAnnotations <cvtk.format.ImageAnnotations>` object.
        
        Args:
            image_annotations (ImageAnnotations, list): An :class:`ImageAnnotations <cvtk.format.ImageAnnotations>`
                                    object or a list of
                                    :class:`ImageAnnotation <cvtk.format.ImageAnnotation>` objects.
    ...
    def format(self, annotation_format, file_path=None, dataset_name='dataset'):
        ...
        Rearrange :class:`ImageAnnotations <cvtk.format.ImageAnnotations>` object
        to a specific format.
        COCO (.json) and Pascal VOC (.xml) are supported in current version.
        Different to :func:`format <cvtk.format.ImageAnnotation.format>`
        implemented in :class:`ImageAnnotation <cvtk.format.ImageAnnotation>`,
