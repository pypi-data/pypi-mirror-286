...
class MessagePrinter:
    @typechecked
    def __init__(
        self, output_format: str, msg_limit_received: Optional[Event] = None, num_msgs: Optional[int] = None
    ):
...
def _watch_process(
    ...
    try:
        msg_printer = MessagePrinter(output_format, done, num_msgs)
...
def _watch(
    ...
    with timeout_process(
        ...
        args=(output_format, exchanges, exchange_routes, alert_exchanges, done, config, msg_limit),
    ):
        done.wait()
...
def watch(
    ...
    _watch(output_format, exchanges, exchange_routes, alert_exchanges, msg_limit)
