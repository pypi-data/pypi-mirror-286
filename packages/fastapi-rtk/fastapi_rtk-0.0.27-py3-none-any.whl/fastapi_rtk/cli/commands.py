...
async def _check_roles(name: str, create: bool = False):
    async with session_manager.session() as session:
        ...
        if not role:
            ...
            await g.current_app.create_role(name=name, session=session)
...
async def create_user(
    ...
    return await g.current_app.create_user(
        email=email,
        username=username,
        password=password,
...
async def reset_password(email_or_username: str, password: str):
    ...
    Reset user password.
    """
    user = await g.current_app.get_user(email_or_username)
    return await g.current_app.reset_password(user, password)
...
async def export_data(
    ...
    Export data.
    """
    data = await g.current_app.export_data(data, type)
