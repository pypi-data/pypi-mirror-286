...
class SelfDepends:
    ...
    ```python
        class MyApi:
            @expose("/my_endpoint")
            def my_endpoint(self, something: Something = SelfDepends(dependency_that_require_self)):
                # Do something with the dependency
                pass

            def dependency_that_require_self(self):
                # Do something that requires the class instance, and should return a function that can be used with Depends()
                return get_db(self.datamodel.obj.__bind_key__)
    ```
    """
    ...
    def __call__(self, *args: Any, **kwds: Any) -> Any:
        return Depends(self.f(*args, **kwds))


class SelfType:
    def __init__(self, attr: str, with_depends=False) -> None:
        self.attr = attr
        self.with_depends = with_depends
...
def update_signature(cls, f):
    ...
    if old_first_parameter.name == "self":
        ...
        for parameter in old_parameters:
            ...
            if isinstance(parameter.default, SelfType):
                parameter = parameter.replace(
                    annotation=parameter.default(cls),
                    default=(
                        Depends()
                        if parameter.default.with_depends
