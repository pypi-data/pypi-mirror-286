        ...
        for attr_name in dir(self):
            ...
            if hasattr(attr, "_permission_name"):
                ...
                base_endpoint = getattr(
                    self,
                    f"{permission_name}_headless",
                    getattr(self, f"{permission_name}_list_headless", None),
                )

                if (
                    base_permissions
                    and permission_name_with_prefix not in base_permissions
                ) or (base_endpoint and permission_name in exclude_routes):
                    continue
                    ...
                    for method in methods:
                        ...
                        if priority is not None:
                            priority_routes.append(
                                (priority, router_func, rest_data, attr)
                            )
                            continue
