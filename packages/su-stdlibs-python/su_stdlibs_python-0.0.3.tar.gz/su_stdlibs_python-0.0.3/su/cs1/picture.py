    ...
    def __init__(self, arg1=None, arg2=None):
        ...
        if (arg1 is None) and (arg2 is None):
            ...
            if (not os.path.isfile(fileName)):
                # Correct hardcoded paths for this operating system
                import posixpath
                import ntpath
                if (os.name == 'posix'):
                    fileName = fileName.replace(ntpath.sep, posixpath.sep)
                elif (os.name == 'nt'):
                    fileName = fileName.replace(posixpath.sep, ntpath.sep)
                # Correct cases for this operating system
                directory, filename = os.path.split(fileName)
                directory, filename = (directory or '.'), filename.lower()
                name = next((x for x in os.listdir(directory) if x.lower() ==
                             filename), None)
                if (name is not None):
