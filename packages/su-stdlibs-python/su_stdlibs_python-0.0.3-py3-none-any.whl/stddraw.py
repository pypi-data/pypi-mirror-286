...
def picture(pic, x=None, y=None, w=None, h=None):
    """
    Draw pic on the background canvas centered at (x, y) with size (w, h).
    pic is an object of class picture.Picture. x and y default to the midpoint
    ...
    xs = _scaleX(x)
    ys = _scaleY(y)
    ws = _factorX(w) if w else pic.width()
    hs = _factorY(h) if h else pic.height()
    picSurface = pic._surface # violates encapsulation
    picSurface = pygame.transform.scale(picSurface, (ws, hs))
    _surface.blit(picSurface, pygame.Rect(xs-ws/2.0, ys-hs/2.0, ws, hs))
