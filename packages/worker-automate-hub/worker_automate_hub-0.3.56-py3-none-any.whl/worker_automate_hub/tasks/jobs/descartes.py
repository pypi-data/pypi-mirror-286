...
async def descartes(task):
    ...
    try:
        ...
        if field_validade is not None:
            ...
            await api_simplifica(task['configEntrada']['urlRetorno'], "ERRO", observacao, task['configEntrada']['uuidSimplifica'], '', '')
        ...
        for item in itens:
            ...
            if button_almoxarifado is not None:
                ...
                pyautogui.hotkey('del')
                pyautogui.write(task['configEntrada']['filialEmpresaOrigem']+"99")
                pyautogui.hotkey('tab')
                time.sleep(2)
                console.print(f"\nDigitou almoxarifado {task['configEntrada']['filialEmpresaOrigem']+"99"}", style='bold green')            
            else:
