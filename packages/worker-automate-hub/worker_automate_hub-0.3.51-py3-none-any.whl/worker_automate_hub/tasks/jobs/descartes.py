...
async def descartes(task):
    ...
    try:
        ...
        if condicao_field is not None:
            ...
            pyautogui.sleep(1)
            transferencia = await find_element_center(ASSETS_BASE_PATH + "a_vista_descartes.png", (977, 281, 232, 171), 10)
       ...
       #Verifica mensagem de "Pré-Venda incluida com número: xxxxx"
        ...
        if message_confirma_transferencia is not None:
            ...
            pyautogui.moveTo(1200, 300)
            message_primeira_parcela = await find_element_center(ASSETS_BASE_PATH + "message_vencimento_primeira_parcela.png", (581, 426, 757, 193), 10)
            if message_primeira_parcela is not None:
                button_yes = await find_element_center(ASSETS_BASE_PATH + "warning_button_yes.png", (581, 426, 757, 193), 10)
            pyautogui.click(button_yes.x , button_yes.y)
        ...
        warning_substituicao_trib = await find_element_center(ASSETS_BASE_PATH + "warning_valor_substituicao_tributaria.png", (734, 450, 458, 189), 10)
        if warning_substituicao_trib is not None:
            console.log("Vai clicar em não")
            button_no = await find_element_center(ASSETS_BASE_PATH + "button_nao.png", (734, 450, 458, 189), 10)
            pyautogui.click(button_no.x, button_no.y)
            console.log("Cliclou em 'No' na mensagem de Substiucao tributaria", style="bold green")
        else:
            console.log("Nao clicou em porra nenhuma")
            await api_simplifica(task['configEntrada']['urlRetorno'], "ERRO", observacao, task['configEntrada']['uuidSimplifica'], None, None)
        ...
        if faturar_pre_venda is not None:
            ...
            await api_simplifica(task['configEntrada']['urlRetorno'], "ERRO", observacao, task['configEntrada']['uuidSimplifica'], None, valor_nota)
