...
from worker_automate_hub.utils.util import add_start_on_boot_to_registry, create_worker_bat
...
def function_configure(flag: bool):
    ...
    if flag:
        ...
        q = [inquirer.Text("vault_token", "Por favor digite o token do Vault")]
        r = inquirer.prompt(q, theme=GreenPassion())
        q1 = [inquirer.List("env_list", "Selecione o ambiente", environment_names)]
        r1 = inquirer.prompt(q1, theme=GreenPassion())

        env_sel, credentials = load_environments(r1["env_list"], r["vault_token"])
        ...
        if workers == None:
            ...
            q3 = [inquirer.Confirm("reg_config", message="Adicionar configuração de inicialização aos registros do Windows?")]
            r3 = inquirer.prompt(q3, theme=GreenPassion())

            if r3["reg_config"]:
                add_start_on_boot_to_registry()

            q4 = [inquirer.Confirm("assets_config", message="Atualizar a pasta assets? (credentials.json*)")]
