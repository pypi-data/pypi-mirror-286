...
class Opendrive2Apollo(Convertor):
  ...
  def add_junction_forward_neighbors(self):
    ...
    for pb_lane in self.pb_map.lane:
      ...
      if right_forward_lane_id != "" and right_forward_lane_id not in pb_lane.right_neighbor_reverse_lane_id:
        right_count += 1

        pb_lane.right_neighbor_reverse_lane_id.add().id = right_forward_lane_id
      if left_forward_lane_id != "" and left_forward_lane_id not in pb_lane.left_neighbor_reverse_lane_id:
        left_count += 1
        pb_lane.left_neighbor_reverse_lane_id.add().id = left_forward_lane_id
