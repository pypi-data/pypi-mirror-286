...
class Pipeline:
    def __init__(self, name: str, QA_flag: str = None, slack_channel='C07CJTMQD44'):
        """
        ...
        self.name = name
        self.fusion_logger = self.create_slack_logger(slack_channel)
        self.stages = []
        self.errors = []
    ...
    def create_slack_logger(self, slack_channel):
        # Default to general chat
        try:
            slack_bot_token = self.client.query(self.translate_query('Select * from `$Bronze:LKP_SLACK_TOKEN`')).result().to_dataframe().iloc[0,0]
            return FusionLogger(slack_bot_token, slack_channel)
