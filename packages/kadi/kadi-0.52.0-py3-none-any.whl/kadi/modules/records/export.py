...
class RecordRDFGraph(RDFGraph):
    ...
    def _add_metadata(self, record, export_filter, user):
        ...
        self.add((record_ref, SDO.description, Literal(record_data["description"])))
        self.add(
            (
                record_ref,
                SDO.dateCreated,
        ...
        for file_data in record_data["files"]:
            ...
            self.add((file_ref, SDO.description, Literal(file_data["description"])))
...
class RecordROCrate(ROCrate):
    ...
    def _get_property_values(self, extras, parent_key=""):
        property_values = []
        ...
        for index, extra in enumerate(extras):
            ...
            if is_nested_type(extra["type"]):
                property_values += self._get_property_values(extra["value"], key)
            else:
                property_value = {
                    "@type": "PropertyValue",
                    "propertyID": key,
                    "additionalType": extra["type"],
                    "value": extra["value"],
                }
    ...
    def add_record_metadata(self, record, record_export_data):
        ...
        if record_data["description"]:
            record_entity["description"] = record_data["description"]
        ...
        property_values = self._get_property_values(record_data["extras"])
        ...
        for file_data in record_data["files"]:
            ...
            if file_data["description"]:
                file_entity["description"] = file_data["description"]
