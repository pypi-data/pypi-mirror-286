...
class Waveform:
    ...
    def _find_attack_endpoints(self, position_value, percent_step, method: int = 3):
        ...
        if method == 1:  # Equivalent to a value of 80%
            ...
            for p in range(len(percent_value_value)):
                idx = np.where(self.normal_signal_envelope >= percent_value_value[p])[0]
                if len(idx) > 0:
                    percent_value_position[p] = idx[0]
                else:
                    raise IndexError("locating index not working, trying a different method")
    ...
    def calculate_log_attack(self):
        ...
        try:
            start_attack_position, end_attack_position = self._find_attack_endpoints(position_value, percent_step,
                                                                                     method=3)
        except IndexError:
            start_attack_position, end_attack_position = self._find_attack_endpoints(
                position_value, percent_step,
                method=1
                )
