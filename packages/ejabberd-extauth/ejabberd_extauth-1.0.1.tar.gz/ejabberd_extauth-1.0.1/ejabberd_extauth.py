...
class ExtAuthHandler:
    ...
    def is_user(self, user: str, server: str) -> bool:
        raise NotImplementedError

    def set_pass(self, user: str, server: str, password: str) -> bool:
        raise NotImplementedError

    def try_register(self, user: str, server: str, password: str) -> bool:
        raise NotImplementedError

    def remove_user(self, user: str, server: str, password: str | None = None) -> bool:
        raise NotImplementedError
...
class ExtAuth:
    def __init__(self, handler: type[ExtAuthHandler]):
        ...
        config_file = f"/etc/ejabberd/extauth/{handler.name}.yml"
        if os.path.isfile(config_file):
            self._logger.info("Loading configuration file %s", config_file)
            with open(config_file, "r") as f:
                self._config = yaml.load(f, Loader=yaml.Loader)
        else:
            self._logger.info("Configuration file %s not found", config_file)
            self._config = {}

        if level_name := self._config.get("log_level", None):
            logging.basicConfig(level=logging.getLevelName(level_name))
            self._logger.debug("Reset root logging level to %s", level_name)
    ...
    def _read_and_handle(self):
        ...
        res_bytes = struct.pack("!h", len(res_bytes)) + res_bytes
