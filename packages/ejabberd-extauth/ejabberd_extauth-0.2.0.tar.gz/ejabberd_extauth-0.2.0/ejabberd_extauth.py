...
class ExtAuthHandler:
    ...
    def __init__(self, config: dict, logger: logging.RootLogger):
        ...
        self._logger = logger
        self._logging.debug("Handler initialized")
    ...
    def _read_and_handle():
        ...
        try:
            if command == "auth" and len(args) == 3:
                ...
                self._logging.error("Command or signature is invalid")
                res = False
        except NotImplementedError:
            self._logging.warning("Command is not implemented in handler; returning failure")
            res = False
