...
class LeonardoAI:
    ...
    def get_models(self) -> List[Dict]:
        ...
        try:
            ...
            # Debugging line to check the API response structure
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                if response.status_code == 200:
                    ...
                    if job_status['generated_image_variation_generic'] and job_status['generated_image_variation_generic'][0]['status'] == 'COMPLETE':
    ...
    def get_motion_image_url(self, motion_job_id: str, poll_interval: int = 10, timeout: int = 300) -> str:
        ...
        while time.time() - start_time < timeout:
            try:
                generation_info = self.get_single_generation(motion_job_id)
                print(f"Generation info: {generation_info}")  # Debugging line

                for image_info in generation_info:
                    print(f"Image info: {image_info}")  # Debugging line
                    if 'motionMP4URL' in image_info and image_info['motionMP4URL']:
                        return image_info['motionMP4URL']
                    elif 'status' in image_info and image_info['status'] == 'FAILED':
                        raise LeonardoAIError("Motion generation job failed.")
            except LeonardoAIError as e:
                print(f"Waiting for motion generation completion: {e}")
            except Exception as e:
                print(f"Error retrieving motion image: {e}")

            time.sleep(poll_interval)

        raise LeonardoAIError(
            f"Motion generation job {motion_job_id} did not complete within {timeout} seconds")
    ...
    def get_motion_image(self, motion_job_id: str, poll_interval: int = 10, timeout: int = 300) -> str:
        ...
        while time.time() - start_time < timeout:
            try:
                url = f"{self.base_url}/generations-motion-svd/{motion_job_id}"
                response = requests.get(url, headers=self.headers)
                if response.status_code == 200:
                    job_status = response.json()
                    print(f"Job status: {job_status}")
