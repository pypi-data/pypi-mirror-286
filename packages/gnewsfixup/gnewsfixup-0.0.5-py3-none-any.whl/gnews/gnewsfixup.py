...
class GNews:
    def __init__(self, language="en", country="US", max_results=100, period=None, start_date=None, end_date=None,
        ...
        self._exclude_websites = exclude_websites if exclude_websites and isinstance(
            exclude_websites, list) else []
    ...
    def start_date(self, start_date):
        ...
        if self._end_date:
            if start_date - self._end_date == datetime.timedelta(days=0):
                warnings.warn(
                    "The start and end dates should be at least 1 day apart, or GNews will return no results")
            elif self._end_date < start_date:
                warnings.warn(
                    "End date should be after start date, or GNews will return no results")
    ...
    def get_full_article(self, url):
        ...
        try:
            article = newspaper.Article(
                url="%s" % url, language=self._language)
            article.download()
            article.parse()
