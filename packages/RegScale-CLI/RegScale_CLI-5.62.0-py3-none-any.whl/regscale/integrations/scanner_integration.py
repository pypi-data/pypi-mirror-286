...
class ScannerIntegration(ABC):
    ...
    def get_finding_otherIdentifier(self, finding: IntegrationFinding) -> str:
        """
        ...
        if ScannerVariables.issueCreation.lower() == "perasset":
            return f"{finding.external_id}:{finding.asset_identifier}"
    ...
    def get_or_create_assessment(self, control_implementation_id: int) -> regscale_models.Assessment:
        ...
        """
        logger.info("Getting or creating assessment for control implementation %d", control_implementation_id)
    ...
    def create_new_asset(
        ...
        new_asset = regscale_models.Asset(
            name=asset.name,
            otherTrackingNumber=asset.other_tracking_number,
    ...
    def create_or_update_issue_from_finding(
        ...
        issue = regscale_models.Issue(
            ...
            assetIdentifier=(
                self.asset_map_by_identifier.get(finding.asset_identifier).name
                if finding.asset_identifier in self.asset_map_by_identifier
                else finding.asset_identifier
            )
            or finding.asset_identifier,
            securityChecks=finding.external_id,
            remediationDescription=finding.recommendation_for_mitigation,
            otherIdentifier=self.get_finding_otherIdentifier(finding),
        ...
        if ScannerVariables.issueCreation.lower() != "perasset":
            ...
            existing_issues = regscale_models.Issue.find_by_other_identifier(finding.external_id)
            delimiter = "\n"
    ...
    def handle_passing_finding(
        ...
        for issue in existing_issues:
            ...
            if (
                issue.otherIdentifier == self.get_finding_otherIdentifier(finding)
    ...
    def process_checklist(self, finding: IntegrationFinding) -> None:
        ...
        if not asset:
            self.log_error("Asset not found for identifier %s, skipping finding", finding.asset_identifier)
            return
        ...
        for finding in findings:
            if finding:
                for control_label in finding.control_labels:
                    ...
                    logger.debug(
                        "Created or Updated assessment %d for control %s",
                        assessment.id,
                        self.control_map[control_implementation_id],
                    )
    ...
    def create_vulnerability_from_finding(
        ...
        if self.regscale_version > "5.60.0" or self.regscale_version == "localdev":
    ...
    def handle_vulnerability(
        ...
        if finding.cve:
            ...
            self.create_or_update_issue_from_finding(
                title=f"Finding {finding.external_id} failed",
                parent_id=asset.id,
    ...
    def update_regscale_vulnerabilities(self, findings: List[IntegrationFinding]) -> None:
        """
        ...
        for finding in findings:
            asset = self.asset_map_by_identifier.get(finding.asset_identifier)
            if not asset:
                logger.warning("Asset not found for identifier %s, skipping finding", finding.asset_identifier)
                continue
        ...
        logger.info("Processed %d findings for vulnerabilities and issues.", len(findings))
    ...
    def close_outdated_vulnerabilities(self, current_vulnerabilities: Dict[int, Set[int]]):
        ...
        :param current_vulnerabilities: Dictionary of asset IDs to lists of current vulnerability IDs
        """
        closed_count = 0
    ...
    def close_outdated_issues(self, current_vulnerabilities: Dict[int, Set[int]]):
        """
        ...
        :param current_vulnerabilities: Dictionary mapping asset IDs to sets of current vulnerability IDs
        """
        closed_count = 0
    ...
    def process_asset_issues(self, asset_issues: List[regscale_models.Issue], current_vuln_ids: Set[int]) -> int:
        ...
        :param asset_issues: List of issues for the asset
        :param current_vuln_ids: Set of current vulnerability IDs for the asset
    ...
    def should_close_issue(self, issue: regscale_models.Issue, current_vuln_ids: Set[int]) -> bool:
        ...
        :param current_vuln_ids: Set of current vulnerability IDs
        :return: True if the issue should be closed, False otherwise
        """
        if self.regscale_version <= "5.60.0" and self.regscale_version != "localdev":
            return True
    ...
    def sync_findings(cls, plan_id: int, **kwargs: dict) -> None:
        ...
        with instance.finding_progress:
            ...
            if cls.type == ScannerIntegrationType.CHECKLIST:
                ...
                instance.update_regscale_findings(findings=findings)
                instance.update_regscale_vulnerabilities(findings=findings)
            if instance.errors:
