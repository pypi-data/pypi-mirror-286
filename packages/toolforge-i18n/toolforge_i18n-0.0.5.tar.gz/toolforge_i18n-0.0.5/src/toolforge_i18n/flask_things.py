...
from toolforge_i18n.language_info import lang_bcp47_to_mw, lang_dir, lang_fallbacks, lang_mw_to_bcp47
...
def init_html_language_codes() -> None:
    ...
    It’s maintained by message(), push_html_lang() and pop_html_lang().
    """
...
def push_html_lang(language_code: str) -> Markup:
    ...
    as it’s called by the message() function automatically.
...
def add_lang_if_needed(message: Markup, language_code: str) -> Markup:
    ...
    Note that message() calls this function automatically,
...
def assert_html_language_codes_empty(response: werkzeug.Response) -> werkzeug.Response:
    ...
    if the assertion fails, some push_html_lang() call is missing
    a corresponding pop_html_lang() call.
    """
...
def interface_language_code_from_request(translations: dict[str, dict[str, str]]) -> str:
    ...
    your implementation should generally look like this:
        ...
        from toolforge_i18n.flask_things import interface_language_code_from_request
...
def message(message_code: str, **kwargs: object) -> Markup:
    ...
    using the argument names defined in TranslationsConfig.varaibles.
