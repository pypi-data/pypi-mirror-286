...
class MGSaver:
    ...
    def get_linkDataIt(self, keyname, dataname, tqdm_print_num=1000, is_keyname_startswith=True):
    ...
    # 保存合并连接数据
    def save_linkData(self, keyname, dataname, linkname:str, tqdm_print_num=1000, is_keyname_startswith=True):
        client = self.getmg_func()
        lcet = client[self.dbname][linkname]
        lcet.drop()
        print(f'重新生成连接表-{linkname}')
        for dts in self.get_linkDataIt(keyname, dataname, tqdm_print_num=tqdm_print_num, is_keyname_startswith=is_keyname_startswith):
