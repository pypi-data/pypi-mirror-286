...
def draw_event_type_counts_by_analyst() -> px.scatter:
    stmt = """
    SELECT
        a.name as analyst,
    ...
    LEFT JOIN
        search s ON s.uid = e.search_uid
    WHERE
        e.type in ('validate', 'reject', 'advance', 'send')
    ...
    type_order = ["reject", "advance", "validate", "send"]
    df["type"] = pd.Categorical(df["type"], categories=type_order, ordered=True)
    df = df.sort_values(by=["type", "analyst"])
    fig = px.scatter(
        df,
        x="date",
        y="analyst",
        facet_col="type",
        size="event_count",
        title="Event Counts by Analyst, by Type | Trailing 7 Days",
        height=600,
    )
    return fig
