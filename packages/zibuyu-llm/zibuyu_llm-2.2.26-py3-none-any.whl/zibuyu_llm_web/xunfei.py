...
class XunFeiWeb(LLMBase):
    ...
    def ask_stream(
        ...
        :param success_callback_func：请求成功时的回调函数
        :return：生成器对象
        """
        ...
        for line in response.iter_lines():
            ...
            if self.decode(encoded_data) != 'zw':
                ...
                if callable(success_callback_func):
                    try:
                        success_callback_func(self.single_answer)
                        self.logger.error(f"请求成功回调函数执行成功！")
                    except:
                        self.logger.error(f"传入的请求成功回调函数执行失败！")
