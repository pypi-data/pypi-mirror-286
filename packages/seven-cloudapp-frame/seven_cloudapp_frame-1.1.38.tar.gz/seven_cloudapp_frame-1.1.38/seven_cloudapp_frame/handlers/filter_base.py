...
@LastEditTime: 2024-07-23 15:03:13
...
def filter_check_current_limit(handler_name=None, current_limit_count=0, limit_params_dict={}):
    ...
    def check_current(handler):
        def wrapper(self, **args):
            ...
            if safe_config.get("traffic_control_ways", 0) == 1: # 流量控制方式 0-关闭 1-开启
                ...
                if limit_count > 0:
                    if limit_type == 1:
                        ...
                        request_queue_name = f"request_queue:appid_{app_id}:handlername_{handler_name}"
                        if SafeHelper.check_current_limit_v2(request_queue_name, limit_count, request_limit_time) == True:
                            self.response_json_error("current_limit", "当前人数过多,请稍后再试")
                            return
