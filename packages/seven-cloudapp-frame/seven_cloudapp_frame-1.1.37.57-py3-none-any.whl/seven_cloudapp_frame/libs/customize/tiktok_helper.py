...
@LastEditTime: 2024-07-18 14:39:26
...
class TikTokHelper:
    ...
    def __redis_init():
        """
        :description:reidis初始化
        :return:
        :last_editors: HuangJianYi
        """
        is_cluster = False
        config_dict = share_config.get_value("platform_redis")
        if not config_dict:
            config_dict = config.get_value("redis")
        if config_dict and config_dict.get("is_cluster", False):
            is_cluster = True
            
        redis_init = SevenHelper.redis_init(config_dict=config_dict)
        return redis_init, is_cluster
    ...
    def code2_session(self, code="", anonymous_code="", app_id="", app_secret=""):
        ...
        redis_init, is_cluster = self.__redis_init()
        if is_cluster == True:
            redis_key = redis_key + ":" + config.get_value("project_name", "")
