...
@LastEditTime: 2024-07-18 11:19:51
...
class FrameBaseModel():
    ...
    def check_request_queue(self, request_queue_name, user_id, request_limit_num, request_limit_time):
        ...
        if request_limit_num > 0  and request_limit_time > 0:
            redis_init = RedisExHelper.init(config.get_value("safe_redis"))
            current_time = int(time.time())
            window_start = current_time // request_limit_time * request_limit_time
            window_start = f"{request_queue_name}:{window_start}" + ":" + config.get_value("project_name", "")
            request_count = redis_init.lpush(window_start, user_id)
            redis_init.expire(window_start, request_limit_time + 1)
            # 使用LTRIM保留最新的REQUEST_LIMIT个请求计数
            redis_init.ltrim(window_start, -request_limit_num, -1)
            if request_count > request_limit_num:
                invoke_result_data.success = False
