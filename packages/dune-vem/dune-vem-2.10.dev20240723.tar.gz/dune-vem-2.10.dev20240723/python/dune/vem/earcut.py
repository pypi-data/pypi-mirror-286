...
template <std::size_t I, typename T> struct nth {
    ...
    */
    inline static double get(const T& t) { return t[I]; };
};
...
Earcut<N>::linkedList(const Ring& points, const bool clockwise) {
    ...
    if (clockwise == (sum > 0)) {
        for (int i = 0; i < len; i++) last = insertNode(vertices + i, points[i], last);
    } else {
        for (int i = len; i-- > 0;) last = insertNode(vertices + i, points[i], last);
    }
...
"""
    ...
    def __call__(self, coords, indices):
        ...
        if self.code is None:
            self.code = algorithm.load('earcut', io.StringIO(EarCut.code), [points])
