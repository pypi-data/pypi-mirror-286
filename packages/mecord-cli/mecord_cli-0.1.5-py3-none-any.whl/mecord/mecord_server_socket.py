...
class MecordLongConnectThread(Thread):
    ...
    def receive_TaskNotify(self, body):
        ...
        logger.info(f'{self.name} === task {rsp.task_uuid} notify={rsp.ok} -c {respond_times}s')
    ...
    def GetTask(self):
        if not os.path.exists(stop_thread_file):
            ...
            req.limit = 1 if 'exector' in current_thread().name else self.executor.idlePower(self.mtype)
            ...
            if self.mtype:
                print(f"{current_thread().name} waiting next {req.limit} task, widgets: {req.widgets} -t {send_byte_time}")
            else:
                print(f"{current_thread().name} waiting next {req.limit} task -t {send_byte_time}")

    def ReplyTask(self, tasklist):
        ...
        self.send_byte(self._pb_pack("ReplyTask", req))
    ...
    def run(self):
        ...
        while self.is_running:
            ...
            if limit >= 1:
                ...
                try:
                    reqid = uuid.uuid5(uuid.NAMESPACE_DNS, self.name + (self.mtype if self.mtype == '-h' else '') + str(int(time.time()*1000)))
