...
def _connect():
    _reload = 'no'
    _config_file = wg_config_file().as_posix()
...
"""
            with open(_config_file, 'wt') as f:
                f.write(_interface)
            break
    hosts_mappings = list()
    for neighbor in neighbors:
...
"""
            # print(_peer)
            with open(_config_file, 'at') as f:
                f.write(_peer)
    _current_md5sum = _md5sum(wg_config_file)
    if _current_md5sum != _running_md5sum:
        _reload = 'yes'
        print("meshing network information changed, force reloading ...")
        # restart_wg_command = 'wg-quick down wg0; wg-quick up wg0; wg show'
        restart_wg_command = f"wg-quick down {_config_file}; wg-quick up {_config_file}; wg show"
