...
class Superposition(QtWidgets.QWidget):
	...
	def show_pdb_target(self, pdbs, newPath=True):
		...
		self.ui.referenceBrowser.setHtml(html, baseUrl=QtCore.QUrl.fromLocalFile(str(Path(__file__).resolve().parent)),)
	...
	def show_pdb(self, pdbs, ref, newPath=True):
		...
		self.ui.targetBrowser.setHtml(html, baseUrl=QtCore.QUrl.fromLocalFile(str(Path(__file__).resolve().parent)),)
