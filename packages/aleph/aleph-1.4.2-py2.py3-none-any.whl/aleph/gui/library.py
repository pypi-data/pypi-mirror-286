...
class Library(QtWidgets.QWidget):
	...
	def show_pdb(self, pdbs, template=False, newPath=True):
		...
		if not template:
			self.ui.browser.setHtml(html, baseUrl=QtCore.QUrl.fromLocalFile(str(Path(__file__).resolve().parent)),)
		else:
			self.ui.templateBrowser.setHtml(html, baseUrl=QtCore.QUrl.fromLocalFile(str(Path(__file__).resolve().parent)),)