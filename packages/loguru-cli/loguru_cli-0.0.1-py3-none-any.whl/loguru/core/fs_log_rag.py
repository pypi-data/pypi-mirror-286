...
class LoguruRAG:
    ...
    def scan(self, clean_and_rebuild: bool = False):
        if clean_and_rebuild or not os.path.exists(self._vector_store_directory):
            print("Scanning log locations to rebuild index...")
    ...
    def _load_log_file(self, log_file_path, pattern_to_split_log_lines):
        ...
        embedding_model = self.load_embedding_model(model_name=self._embedding_model_name)
    ...
    def load_embedding_model(self, model_name, normalize_embedding=True):
    ...
    def get_response(self, query, chain, stream: bool = False) -> tuple[str, list[Document]]:
    ...
    def ask(self, question: str, stream: bool = False) -> tuple[str, list[Document]]:
        ...
        chain = self.load_qa_chain(retriever, llm, prompt)
        start_time = time.time()
        response, source_docs = self.get_response(question, chain, stream=stream)
        end_time = time.time()
        time_taken = round(end_time - start_time, 2)
