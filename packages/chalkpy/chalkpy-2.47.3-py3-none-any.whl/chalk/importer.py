...
def parse_grouped_window(
    parent: Type[Features],
    feature_name: str,
    expr: Underscore,
    kind: Type[int] | Type[float],
) -> Tuple[WindowConfigResolved, None] | Tuple[None, str]:
    """
    ...
    cfg = WindowConfigResolved(
        ...
        child_namespace=joined_class.namespace,
        child_feature=aggregated_feature,
        bucket_duration_seconds=60,
        aggregation=aggregation,
        group=group_key_features,
    )
...
def _get_has_many_class(
    ...
    Returns
    -------
    Tuple[Type[Features], list[Feature], None] | Tuple[None, None, str]
        (joined_class, aggregated, None) - if the has-many class and aggregated features are found
        (None, None, error_message) - if the has-many class and aggregated features are not found
    """
    try:
        has_many_feature = getattr(parent, child_attr_name)
    except Exception:
        return None, None, None, f"could not find feature '{child_attr_name}' in the child namespace"

    underlying = unwrap_feature(has_many_feature, raise_error=False)

    if not isinstance(underlying, Feature):
        return None, None, None, f"the attribute '{child_attr_name}' is not a feature"

    if not underlying.is_has_many:
        return None, None, None, f"the attribute '{child_attr_name}' is not a has-many feature"

    joined_class = underlying.joined_class
    if joined_class is None:
        return None, None, None, f"has-many feature '{child_attr_name}' is not joined to a class"

    aggregated_values: list[Feature] = []
    for group_name in group_names:
        ...
        aggregated_values.append(joined_feature)

    if aggregated_feature_name is None:
        return joined_class, aggregated_values, None, None
    ...
    return joined_class, aggregated_values, aggregated_feature, None
...
def parse_windowed_materialization(f: Feature) -> Tuple[WindowConfigResolved, None] | Tuple[None, str]:
    ...
    joined_class, aggregations, aggregated_feature, error = _get_has_many_class(
        parent=f.features_cls,
        child_attr_name=child_attr_name,
        group_names=[aggregated_value] if aggregated_value else [],
        aggregated_feature_name=aggregated_value,
    )
    if error is not None:
        return None, error

    assert aggregations is not None  # pyright doesn't understand the return type. this is guaranteed.
    assert joined_class is not None  # pyright doesn't understand the return type. this is guaranteed.

    error = _check_types(
        feature_name=f.window_stem,
        has_many_name=child_attr_name,
        aggregation=aggregation,
        joined_feature=aggregated_feature,
...
def import_all_files(
    ...
    for _, fsb in FeatureSetBase.registry.items():
        for gbw in fsb.__chalk_group_by_materialized_windows__:
            name = gbw._name  # pyright: ignore[reportPrivateUsage]
            assert name is not None

            kind = gbw._kind  # pyright: ignore[reportPrivateUsage]
            assert kind is not None

            mat, error = parse_grouped_window(
                parent=fsb,
                feature_name=name,
                expr=gbw._expression,  # pyright: ignore[reportPrivateUsage]
                kind=kind,
            )
            ...
            if error is not None:
                ...
                lsp_error_builder.add_diagnostic(
                    message=f"The materialized feature '{name}' is incorrectly configured.",
                    label=error,
                    range=lsp_error_builder.property_value_kwarg_range(
                        feature_name=name,
                        kwarg="expression",
                    )
                    or lsp_error_builder.property_range(feature_name=name),
                    code="42",
                )
            else:
