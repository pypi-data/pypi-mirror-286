...
def _process_group_by_windowed(
    ...
    if gbw._name is None:
        gbw._name = gbw._name if gbw._name is not None else annotation_name

    gbw._namespace = namespace

    gbw._kind = float
    if gbw._dtype is not None:
        gbw._kind = pyarrow_to_primitive(gbw._dtype, name=annotation_name)
        if gbw._kind not in (int, float):
            error_builder.add_diagnostic(
                message=(
                    f"group_by_window feature '{namespace}.{annotation_name}' has an invalid dtype '{gbw._dtype}'. "
                    "Please use a numeric integer or float dtype, like 'int64' or 'float64'."
                ),
                label="invalid dtype",
                range=error_builder.property_value_kwarg_range(annotation_name, kwarg="dtype")
                or error_builder.property_value_range(annotation_name)
                or error_builder.property_range(annotation_name),
                code="16",
            )

    additional_inits.append((gbw._name, gbw._kind))
...
def _process_class(
    ...
    group_by_materialized_windows: list[GroupByWindowed] = []
    cls_annotations: Dict[str, Any] = {}
    for name, annotation in raw_cls_annotations.items():
        ...
        if isinstance(annotation, DataFrameMeta) and isinstance(windowed_group_by, GroupByWindowed):
            group_by_materialized_windows.append(windowed_group_by)
            _process_group_by_windowed(
                cls=cls,
