...
class Xray:
    ...
    def getTestPlan(key: str):
        ...
        resp = requests.post(f'{XRAY_API}/graphql',
            json={
                'query': json_data
            },
            headers={
        ...
        if resp.status_code == 200:
            ...
            print("Infelizmente ocorreu um erro obter issueID do TestPlan")
            print("Código de erro {}".format(resp.status_code))
            print(json.dumps(resp.json(), indent=4))
            print("------------------------------------------------------------------------------")
    ...
    def addTestExecutionsToTestPlan(issueId: str, testExecIssueId: str):
        ...
        if resp.status_code == 200:
            return resp.json().get('data').get('addTestExecutionsToTestPlan').get('addedTestExecutions')[0]
        else:
            print("Infelizmente ocorreu um erro na vinculação dos resultados ao TestPlan")
            print("Código de erro {}".format(resp.status_code))
            print(json.dumps(resp.json(), indent=4))
            print("------------------------------------------------------------------------------")
    ...
    def importExecutionCucumber(key: str = None):
        ...
        if key != None:
            ...
            Xray.addTestExecutionsToTestPlan(issueId, resp.json().get('id'))
        
        if resp.status_code == 200:
            print("CUCUMBER JSON: {}".format(join(Config.cucumber_path(), 'cucumber.json')))
            if Config.debug():
                print(json.dumps(resp.json(), indent=4))
