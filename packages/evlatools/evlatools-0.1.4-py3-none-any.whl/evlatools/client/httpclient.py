...
try:
    from client.media import ContentTypes
    from utils.jsonpath import JsonHandle
except ImportError:
    from ..client.media import ContentTypes
    from ..comm.jsonpath import JsonHandle
...
def open_file(filepath):
    ...
    with open(file=filepath, mode="rb") as file:
        content = file.read()
    filedata = (filename, content, mimetype)
    return filedata
...
class HTTPRequest(object):
    ...
    def __init__(self, url, method, payload=None, *args, **kwargs):
        self.url = url
        self.method = method
    ...
    def execute(self):
        try:
            ...
            if headers is None:
                response = requests.request(url=self.url, method=self.method, params=self.payload, *self.args, **self.kwagrs)
                ...
                if content_type is None:
                    ...
                    response = requests.request(url=self.url, method=self.method, json=self.payload, *self.args, **self.kwagrs)

                elif content_type == ContentTypes.APPLICATION_FORM_URLENCODED:
                    response = requests.request(url=self.url, method=self.method, data=self.payload, *self.args, **self.kwagrs)
                    ...
                    response = requests.request(url=self.url, method=self.method, data=encoder, *self.args, **self.kwagrs)
