...
def transform_enum_type(context: mypy.plugin.AnalyzeTypeContext) -> mypy.types.Type:
	...
	# and also the allowed arguments of TypeVarType in 1.11.0, supporting only TypeVarId
	# - Not reflected in Release notes
	# - Not reflected in Internal API changelog
	# - Someone's issue:https://github.com/python/mypy/issues/17516#issuecomment-2247584589
	if MYPY_VERSION >= (1, 4):
		self_tvar_type = mypy.types.TypeVarType(
			'_EnumMetaType',
			f'{meta_info.fullname}._EnumMetaType',
			mypy.types.TypeVarId(-1, namespace = 'fastenum'),
			[],
			meta_enum_instance,
			meta_enum_instance,
		)
