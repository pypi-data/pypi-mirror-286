...
def Create_Folder_for_Crypto_layer(var_exr_file_path, var_crypto_layer_name):
    ...
    file_basename = os.path.basename(var_exr_file_path)
    current_crypto_layer_folder = os.path.join(current_file_dirname,
                                               f"{Path(file_basename).stem}_{var_crypto_layer_name.decode('utf-8')}")
...
def get_args():
    """Return in dict form"""
    parser = argparse.ArgumentParser(description='Decode Cryptomattes in EXR file to PNG files.')
    parser.add_argument('--input', '-i', dest='input_path', type=str, default="sample.exr",
...
def cli():
    ...
    exr_base_name = os.path.basename(exr_file_path)
    for metadata_id, metadata_dictionary in cryptomattes.items():
