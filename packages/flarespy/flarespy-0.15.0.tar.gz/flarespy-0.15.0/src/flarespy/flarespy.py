...
class FlareLightCurve(lk.LightCurve):
    ...
    # Data preprocessing -----------------------------------------------------------------------------------------------
    def detrend(self, window_length=0.3, window_length_std=2, period_range=None, edge_cutoff=0, mask_eclipse=True):
        """
        ...
        window_length_std : float, optional
            The window length (in days) for calculating the rolling standard. Default is 2.
        ...
        mask_eclipse : bool, optional
            Whether to detect and mask the eclipses in the light curve. Default is True.
        """
        ...
        if mask_eclipse:
            ...
            self.meta["ECLIPSE_MASK"] = np.zeros_like(self.time.value, dtype=bool)
        ...
        self._standardize(window_length_std=window_length_std)
    ...
    def _standardize(self, window_length_std):
        """
        ...
        window_length_std: float
            The window length (in days) for the rolling standard.
        """

        flux_series = pd.Series(self.detrended_flux.value, index=pd.DatetimeIndex(self.time.datetime))

        rolling_window = flux_series.rolling(pd.Timedelta(window_length_std, unit="d"), center=True)
            ...
            with plt.style.context(MPLSTYLE):
                for row in self.candidate_parameters.itertuples():
                    ...
                    if candidate_type == "sso":
                        ...
                        if "energy" in self.candidate_parameters.columns:
                            if isinstance(row.energy, str):
                    ...
                    figure_path = (
                        figure_subfolder / f"{self.label.replace(' ', '')}-S{self.sector}-{row.t_peak:.7f}.png"
                    )
