...
def get_subtype(seq_id, tree):
    """
     Get the subtype of the interest's sequence ID in a tree. If the subtype of the clade in the tree is a Circulating
...
def __get_subtype_from_clade(phylo_tree, clade, id_to_search, depth_start=2, depth_limit=4):
    ...
    If no subtype is found "Not-Determined" is returned, else the majoritarian subtype is returned.
    ...
    :return: the subtypes in the other leaves' clade of interest than the leaf of interest.
    :rtype: list
    """
    ...
    for depth in range(depth_start, depth_limit+1):
        ...
        if len(max_subtypes) == 1:
            ...
            logging.info(f"Subtype of {id_to_search} found in clade [{', '.join(subtypes_list)}]")
            return max_subtype
        logging.warning(f"More than one subtype in the clade [{', '.join(subtypes_list)}] and no majoritarian subtype, "
                        f"check the upper clade.")
    logging.warning(f"Limit of depth level reached for the {id_to_search} subtype reached, subtype set as 'ND'.")
