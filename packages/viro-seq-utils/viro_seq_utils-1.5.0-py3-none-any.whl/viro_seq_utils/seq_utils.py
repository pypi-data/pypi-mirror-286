...
def get_records_wo_stops(records):
    """
    Get the records without stops codons in at least one frame even if the initial open reading frame has a STOP.
    WARNING: If one of the open reading frames contains a translation without stop,
    the initial nucleotide sequence will be added even if it is not the open reading frame without stops.
...
def net_charge(aa_record, use_h=False):
    ...
    try:
        ...
        seq = aa_record.seq.replace("-", "").upper()
