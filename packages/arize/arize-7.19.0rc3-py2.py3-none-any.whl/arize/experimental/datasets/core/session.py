...
from ..validation.errors import InvalidConfigFileError, InvalidSessionError
...
class Session:
    ...
    def _read_config(self) -> Optional[str]:
        ...
        logger.debug(
            f"Connection details not found/provided. Looking for session values from '{self.arize_profile}'"
            f" in '{file_path}'."
        )
        try:
