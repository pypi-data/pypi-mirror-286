...
class AcquireLedgerState:
    ...
    def execute(
        self, point: Point | Origin, id: Optional[Any] = None
    ) -> (bool, Point | Origin, Optional[Any]):
    ...
    def send(self, point: Point | Origin, id: Optional[Any] = None) -> None:
        """Send the request.
    ...
    @staticmethod
    def _parse_AcquireLedgerState_response(response: dict) -> (Point | Origin, Optional[Any]):
        ...
        if result := response.get("result"):
            if result.get("acquired") == "ledgerState" and result.get("point"):
                point: Point | Origin = rh.parse_PointOrOrigin(result.get("point"))
