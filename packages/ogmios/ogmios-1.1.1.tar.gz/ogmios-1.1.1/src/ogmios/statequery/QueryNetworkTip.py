...
class QueryNetworkTip:
    """Ogmios method to query the point of the network's latest block.

    NOTE: This class is not intended to be used directly. Instead, use the Client.query_network_tip
    method.
    ...
    def __init__(self, client: Client):
        self.client = client
        self.method = mm.Method.queryNetwork_tip.value

    def execute(self, id: Optional[Any] = None) -> (Point | Origin, Optional[Any]):
    ...
    def receive(self) -> (Point | Origin, Optional[Any]):
        ...
        return self._parse_QueryNetworkTip_response(response)

    @staticmethod
    def _parse_QueryNetworkTip_response(response: dict) -> (Point | Origin, Optional[Any]):
        if response.get("method") != mm.Method.queryNetwork_tip.value:
            raise InvalidMethodError(f"Incorrect method for query_network_tip response: {response}")

        if response.get("error"):
            raise ResponseError(f"Ogmios responded with error: {response}")

        if result := response.get("result"):
            point: Point | Origin = rh.parse_PointOrOrigin(result)
            id: Optional[Any] = response.get("id")
            logger.info(
                f"""Parsed query_network_tip response:
        Point = {point}
        ID = {id}"""
            )
            return point, id
        raise InvalidResponseError(f"Failed to parse query_network_tip response: {response}")
