...
from intelliw.utils.intelliwapi.context import traceid_ctx
...
async def middleware_header(request: FARequest, call_next):
    trace_id = request.headers.get('traceId', '0')
    traceid_ctx.set(trace_id)
    response = await call_next(request)
    response.headers["traceId"] = str(trace_id)
    return response
