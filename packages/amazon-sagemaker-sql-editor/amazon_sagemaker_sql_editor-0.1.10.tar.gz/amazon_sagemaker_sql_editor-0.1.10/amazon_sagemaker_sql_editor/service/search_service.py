...
class SearchService(ClassAttributeMetricsContextMixin):
    ...
    async def search(self, request: SearchRequest) -> SearchResponse:
        ...
        self.metrics_context.set_property(
            ...
            len(
                list(
                    filter(
                        lambda index: index.indexStatus == IndexStatus.Status.SUCCESS,
                        indexes,
                    )
                )
            ),
        ...
        self.metrics_context.set_property(
            ...
            len(
                list(
                    filter(
                        lambda index: index.indexStatus == IndexStatus.Status.FAILED,
                        indexes,
                    )
                )
            ),
    ...
    async def index_callback(self, response: GetSchemaResponse):
        ...
        try:
            ...
            if self._index_status.get(root_item, None) in [
                None,
