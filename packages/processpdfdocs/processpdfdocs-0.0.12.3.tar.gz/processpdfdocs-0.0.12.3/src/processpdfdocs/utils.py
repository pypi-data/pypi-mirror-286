...
prompt = """
        ...
        - "Table": Chứa một bảng (table) hay biểu mẫu (form), mẫu đơn,...
        ...
        - Loại 2: Nếu hình ảnh thuộc nhãn "Table", hãy tái cấu trúc lại bảng trong hình ảnh dưới dạng bảng markdown, đồng thời tuyệt đối không sử dụng thêm từ ngữ mô tả nào khác cho hình ảnh có chứa bảng, chỉ cần đưa ra bảng ở định dạng markdown. Với các ô trong bảng với nội dung nhiều dòng, hãy sử dụng kí tự xuống dòng của markdown như <br> để phân biệt các dòng trong ô đó.
...
def ocr_pdf_to_text_and_html(pdf_path, temp_image_converted_path, gemini_api_key):
            ...
            for i, image in enumerate(images):
                ...
                try:    
                    ...
                    for i, bbox in enumerate(figure_bboxes):
                        ...
                        input_image = {
                        'mime_type': 'image/png',
                        'data': pathlib.Path(figure_img_path).read_bytes()
                        }
                        ...
                        a_page_output.append(((x1, y1, x2, y2), response.text, 'figure'))
