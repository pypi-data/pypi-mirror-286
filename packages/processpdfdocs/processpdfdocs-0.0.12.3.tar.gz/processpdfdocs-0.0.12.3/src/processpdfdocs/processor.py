...
class Processor:
    def __init__(self, model_dir='~/.processpdfdocs/models', gemini_api_key=None):
    ...
    def process_pdf(self, pdf_path):
        ...
        if not is_text_selectable(pdf_path):
            extracted_text = ocr_pdf_to_text_and_html(pdf_path, temp_image_converted_path=self.temp_image_converted_path, gemini_api_key=self.gemini_api_key)
            texts = "\n".join(extracted_text)
        else:
