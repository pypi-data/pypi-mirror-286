...
class JobCluster:
    ...
    def run_local(self):
        ...
        while self.n_jobs > 0:
            ...
            for idx, _ in enumerate(sorted(Path(self.output_folder, 'slurm').glob('*.json.gzip'))):
                with (
                    Path(self._output_folder, 'log', f'out_{idx + 1}.log').open('w') as out,
                ):
                    env = os.environ
    ...
    def _prepare_jobs_directory(self):
        ...
        shutil.copytree(Path(inspect.getsourcefile(inspect.getmodule(JobItem))).parent, Path(json_folder, 'plus_slurm'))
