...
class Expression:
    ...
    def func(self):
        return self.engine.get_class(Constant.Expressions, self.name)().run(*[b.run() for b in self._body])
    ...
    def binary(self):
        return self.engine.get_class(Constant.Expressions, self._body[1].text)().run(*[self.body[0].run(),
                                                                                       self.body[2].run()])
