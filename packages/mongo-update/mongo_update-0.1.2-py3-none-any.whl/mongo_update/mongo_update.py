...
class DownMongo:
    ...
    def data_to_file(self, file_type, dbname, collection_name):
        ...
        if not self.start_date:
            print(f'{now}正在下载数据 ({self.host}) {self.dbname}: {self.collection_name}, 数据区间: 近 {self.days} 天\n...')
        else:
            print(f'{now}正在下载数据 ({self.host}) {self.dbname}: {self.collection_name}, 数据区间: {self.start_date} ~ {self.end_date}')
...
class UploadMongo:
    ...
    def add2mongo(self, df, dbname=None, collection_name=None):
        """
        需要检查 self.dbname 和 self.collection_name
        df: 待插入数据, dataframe 格式
        可以初始化时指定 dbname 和 collection_name 或者在这个函数指定
        """
        if dbname:
            self.dbname = dbname
        if collection_name:
        ...
        now = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S ')
        print(f'{now}正在更新mongoDB ({self.host}:{self.port}) 数据库: {self.dbname}/{self.collection_name}...')
