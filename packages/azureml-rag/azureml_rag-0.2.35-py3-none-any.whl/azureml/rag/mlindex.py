...
class MLIndex:
        ...
        with track_activity(logger, "MLIndex.as_langchain_vectorstore") as activity_logger:
            ...
            if index_kind == "acs":
                ...
                try:
                    ...
                    # AZURE_SEARCH_KEY is newer key, AZURE_AI_SEARCH_KEY is new key, but fall back to AZURE_COGNITIVE_SEARCH_KEY for backward compat.
                    if "AZURE_SEARCH_KEY" in os.environ or "AZURE_AI_SEARCH_KEY" in os.environ or "AZURE_COGNITIVE_SEARCH_KEY" in os.environ:
                        from azure.core.credentials import AzureKeyCredential

                        logger.warning(f"Failed to get credential for ACS with {e}, falling back to env vars.")
                        old_search_key = os.environ["AZURE_AI_SEARCH_KEY"] if "AZURE_AI_SEARCH_KEY" in os.environ else os.environ["AZURE_COGNITIVE_SEARCH_KEY"]
    ...
    def as_langchain_retriever(self, credential: Optional[TokenCredential] = None, **kwargs):
        ...
        elif index_kind in ["faiss", "pinecone", "milvus", "azure_cosmos_mongo_vcore", "azure_cosmos_nosql"]:
            return self.as_langchain_vectorstore(credential=credential).as_retriever(**kwargs)
        else:
            raise ValueError(f"Unknown index kind: {index_kind}")
