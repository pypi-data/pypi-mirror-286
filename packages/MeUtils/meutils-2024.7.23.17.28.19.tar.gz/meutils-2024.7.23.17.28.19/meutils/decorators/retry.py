...
def default_retry_error_callback(retry_state: RetryCallState):
    ...
    logger.debug(f"最后一次重试仍然失败调用的函数: {retry_state.__repr__()}")
    logger.debug(f"最后一次重试仍然失败调用的函数: {retry_state.outcome.result()}")

    from meutils.notice.feishu import send_message
    send_message(f"retry_state: {retry_state}")
    send_message(f"retry_state: {retry_state.__repr__()}")
