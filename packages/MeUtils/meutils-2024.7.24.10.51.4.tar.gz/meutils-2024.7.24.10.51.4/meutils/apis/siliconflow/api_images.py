...
@retrying()
async def api_create_image(request: ImageRequest, api_key: Optional[str] = None):
    ...
    async with httpx.AsyncClient(base_url=base_url, headers=headers, timeout=100) as client:
        response = await client.post(f"stabilityai/{model}/text-to-image", json=payload)
...
if __name__ == '__main__':
    ...
    request = ImageRequest(prompt="画条狗")
    arun(
        api_create_image(
            request
        )
