...
def CreateLocalData(struct, fname):
    ...
    df.to_csv(f"{fname}.{extension}", index=False)

    return extension


def CreateLocalPhot(struct, fname):
    extension = CreateLocalData(struct, fname)
    return extension
...
def CreateLocalBulkphot(struct, fname):
    ...
    with open(f"{fname}.{extension}", "w") as file:
        for survey in data:
...
def CreateLocalImage(struct, fname):
    ...
    extension = "fits"
    hdu.writeto(f"{fname}.{extension}", overwrite=True)
...
def generate_local_file(struct, name):
    ...
    if not name:
        ...
        fname = f"{name}_ATK{ftype}"

    extension = globals()[f"CreateLocal{ftype.capitalize()}"](struct, fname)
