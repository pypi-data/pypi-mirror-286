    ...
    async def _consume_message(self, message: aio_pika.IncomingMessage):
        ...
        with catch_error():
            ...
            if correlation_id in self._unary_response:
                ...
                if message.body[:2] == ERROR_HEAD:
                    future.set_exception(RpcServerError(message.body[2:].decode()))
                else:
                    future.set_result(Utils.msgpack_decode(message.body))

            elif correlation_id in self._stream_response:

                future = asyncio.Future()

                if message.body == STREAM_EOF:
                    future.set_exception(EOFError())
                elif message.body[:2] == ERROR_HEAD:
                    future.set_exception(RpcServerError(message.body[2:].decode()))
                else:
                    future.set_result(Utils.msgpack_decode(message.body))
                ...
                raise Exception(f'unexpected message: {correlation_id=}')
