...
def convert_to_tensorrt_llm_config(
    ...
    if first_attention_decoder_config.moe_num_experts:
        # Export old MOE config fields
        config["moe_num_experts"] = first_attention_decoder_config.moe_num_experts
        config["moe_top_k"] = first_attention_decoder_config.moe_top_k
        config["moe_tp_mode"] = 2  # ParallelismMode.TENSOR_PARALLEL
        config["normalization_mode"] = 1  # ExpertScaleNormalizationMode.RENORMALIZE

        # Export new MOE config fields
        config["moe_config"] = {
            "num_experts": first_attention_decoder_config.moe_num_experts,
            "top_k": first_attention_decoder_config.moe_top_k,
            "tp_mode": 2,  # ParallelismMode.TENSOR_PARALLEL
