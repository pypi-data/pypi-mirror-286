...
# TODO use "self.announce()" in Command classes rather than "print()".
...
def read_repo_file(filename):
    ...
    """
    filename = Path(__file__).resolve().parent.joinpath(filename)
...
class BuildManPagesCommand(Command):
    ...
    def run():
        ...
        run_external(("gzip", "--verbose", "--keep", "--best", "--force", *files))
...
class TwineCheckCommand(Command):
    ...
    def run():
        ...
        run_twine(["check", "dist/*"]) or fail("Could not check packages in dist/*")
...
class PublishCommand(Command):
    ...
    user_options = [
        (
            "username=",
            "u",
            "The username to the PyPi repository. "
            "Defaults to the $PYPI_USERNAME env variable.",
        ),
        (
            "password=",
            "p",
            "The password to the PyPi repository. "
    ...
    def finalize_options(self):
        ...
        self._username = (
            self.username if self.username else getenv("PYPI_USERNAME", None)
        )
        if not self._username:
            fail("Must specify username, either --username or $PYPI_USERNAME")

        self._password = (
            self.password if self.password else getenv("PYPI_PASSWORD", None)
        )
...
setup(
    ...
    scripts=["bin/xdg-env-completion.sh"],
