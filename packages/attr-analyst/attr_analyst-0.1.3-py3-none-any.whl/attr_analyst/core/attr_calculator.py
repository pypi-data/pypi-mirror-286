...
def check_data_availibility(
    ...
    # 检查label_name是否存在于target_df和compare_df中
    ...
    # 检查dimension_columns是否在target_df和compare_df中都包含
    ...
    # 检查target_df和compare_df的column名称是否相同
...
def calculate_attr(
    ...
    # 1. 根据dimension_columns的值进行target与compare dataframe的join合并
...
def cal_attr_multiply(
    ...
    if attr_df[label_name_target] == attr_df[label_name_compare]:
        attr_df[label_l_name] = 0
    else:
        attr_df[label_l_name] = (attr_df[label_name_target] - attr_df[label_name_compare]) / (np.log(attr_df[label_name_target]) - np.log(attr_df[label_name_compare]))
