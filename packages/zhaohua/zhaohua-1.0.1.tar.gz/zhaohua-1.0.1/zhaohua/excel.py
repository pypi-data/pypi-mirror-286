...
# - write_mode: 写入模式。当数据为一维列表时，'row'表示按行写入，任意其它值表示按列写入。默认为'row'。
...
def save_to_excel(data, file_path, start_cell='end', write_mode='row'):
    """
    将一维或二维数组写入到Excel文件。
    ...
    write_mode (str): 写入模式。当数据为一维列表时，'row'表示按行写入，任意其它值表示按列写入。默认为'row'。
    """
    # 检查文件路径是否为空
    if not file_path:
