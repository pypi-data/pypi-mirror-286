...
class FirestoreCollection:
    ...
    def list_dicts_paginated(self, pagination: Pagination) -> PaginatedList:
        ...
        # check for next_cursor or previous_cursor
        if pagination.next_cursor:
            print(pagination.next_cursor)
            next_doc = self.collection.document(pagination.next_cursor).get()
            if next_doc:
                ref = ref.start_after(next_doc)
        elif pagination.previous_cursor:
            print(pagination.previous_cursor)
            prev_doc = self.collection.document(pagination.previous_cursor).get()
            if prev_doc:
                ref = ref.end_at(prev_doc)

        # check for limit
        if pagination.limit:
            ref = ref.limit(pagination.limit)
