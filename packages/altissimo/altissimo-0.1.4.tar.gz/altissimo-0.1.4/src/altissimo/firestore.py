...
class FirestoreCollection:
    ...
    def __previous_page_query(self, pagination):
        ...
        previous_doc = self.collection.document(pagination.previous_cursor).get()
        if not previous_doc:
            return self.__default_page_query(pagination)
        direction = firestore.Query.DESCENDING
        if pagination.direction == "descending":
        ...
        # get next page of results
        ref = ref.start_after(previous_doc).limit(pagination.limit + 1)
