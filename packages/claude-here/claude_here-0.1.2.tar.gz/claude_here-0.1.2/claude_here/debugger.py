...
class DebugContext(_coconut.typing.NamedTuple("DebugContext", [("name", 'str'), ("frame_info", '_coconut.typing.Any'), ("source_lines", '_coconut.typing.Sequence[str]'), ("extra_info", 'dict[str, str]')])):  #24 (line in Coconut source)
    ...
    def raw_context(self):  #44 (line in Coconut source)
        return "".join(self.context_lines)  #44 (line in Coconut source)
...
def collect_stack_info(stack_level=1):  #58 (line in Coconut source)
    """Collect information about the callee site for sending to Claude."""  #59 (line in Coconut source)
    cur_frame = inspect.currentframe()  #60 (line in Coconut source)
    outer_frame = reduce(lambda frame, _: frame.f_back, range(stack_level), cur_frame)  #61 (line in Coconut source)

    frame_info = inspect.getframeinfo(outer_frame)  #67 (line in Coconut source)
    try:  #68 (line in Coconut source)
        source_lines, source_lineno = inspect.getsourcelines(outer_frame)  #69 (line in Coconut source)
    except OSError:  #70 (line in Coconut source)
        source_lines = []  #71 (line in Coconut source)

    return DebugContext(name="breakpoint", frame_info=frame_info, source_lines=source_lines, extra_info=_coconut.dict((("locals", format_vars(outer_frame.f_locals)), ("globals", format_vars(outer_frame.f_globals)))))  #73 (line in Coconut source)



def filter_traceback(orig_tb):  #84 (line in Coconut source)
    """Filter out traceback frames from claude_here."""  #85 (line in Coconut source)
    new_tb_top = orig_tb  #86 (line in Coconut source)
    while new_tb_top is not None and new_tb_top.tb_frame.f_globals.get("__package__") == "claude_here":  #87 (line in Coconut source)
        new_tb_top = new_tb_top.tb_next  #88 (line in Coconut source)

    if new_tb_top is not None:  #90 (line in Coconut source)
        tb_cursor_plus_1 = new_tb_top  #91 (line in Coconut source)
        tb_cursor = tb_cursor_plus_1.tb_next  #92 (line in Coconut source)
        while tb_cursor is not None:  #93 (line in Coconut source)
            tb_cursor_minus_1 = tb_cursor.tb_next  #94 (line in Coconut source)
            if tb_cursor.tb_frame.f_globals.get("__package__") == "claude_here":  #95 (line in Coconut source)
                tb_cursor_plus_1.tb_next = tb_cursor_minus_1  #96 (line in Coconut source)
                tb_cursor_plus_1, tb_cursor = (tb_cursor_plus_1, tb_cursor_minus_1)  #97 (line in Coconut source)
            else:  #101 (line in Coconut source)
                tb_cursor_plus_1, tb_cursor = (tb_cursor, tb_cursor_minus_1)  #102 (line in Coconut source)

    return new_tb_top  #107 (line in Coconut source)



def collect_exc_info(exc_type, exc_val, exc_tb):  #110 (line in Coconut source)
    """Collect information about the given exception for sending to Claude."""  #111 (line in Coconut source)
    filtered_tb = filter_traceback(exc_tb)  #112 (line in Coconut source)

    frame_info = inspect.getframeinfo(filtered_tb.tb_frame)  #114 (line in Coconut source)
    source_lines, source_lineno = inspect.getsourcelines(filtered_tb)  #115 (line in Coconut source)
    pretty_tb = ((("".join)(traceback.format_exception(exc_type, exc_val, filtered_tb))).rstrip())  #116 (line in Coconut source)

    return DebugContext(name="exception", frame_info=frame_info, source_lines=source_lines, extra_info=_coconut.dict((("traceback", pretty_tb),)))  #122 (line in Coconut source)
