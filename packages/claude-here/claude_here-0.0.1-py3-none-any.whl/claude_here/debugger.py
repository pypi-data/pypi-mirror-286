...
class DebugContext(_coconut.typing.NamedTuple("DebugContext", [("name", 'str'), ("frame_info", '_coconut.typing.Any'), ("source_lines", '_coconut.typing.Sequence[str]'), ("extra_info", 'dict[str, str]')])):  #10 (line in Coconut source)
...
IGNORED_VARS = set(dir(builtins)) | {"claude_here"}  #33 (line in Coconut source)
...
def filter_traceback(orig_tb):  #68 (line in Coconut source)
    ...
    while new_tb_top is not None and new_tb_top.tb_frame.f_globals.get("__package__") == "claude_here":  #71 (line in Coconut source)
        new_tb_top = new_tb_top.tb_next  #72 (line in Coconut source)

    if new_tb_top is not None:  #74 (line in Coconut source)
        tb_cursor_plus_1 = new_tb_top  #75 (line in Coconut source)
        tb_cursor = tb_cursor_plus_1.tb_next  #76 (line in Coconut source)
        while tb_cursor is not None:  #77 (line in Coconut source)
            tb_cursor_minus_1 = tb_cursor.tb_next  #78 (line in Coconut source)
            if tb_cursor.tb_frame.f_globals.get("__package__") == "claude_here":  #79 (line in Coconut source)
                tb_cursor_plus_1.tb_next = tb_cursor_minus_1  #80 (line in Coconut source)
                tb_cursor_plus_1, tb_cursor = (tb_cursor_plus_1, tb_cursor_minus_1)  #81 (line in Coconut source)
            else:  #85 (line in Coconut source)
                tb_cursor_plus_1, tb_cursor = (tb_cursor, tb_cursor_minus_1)  #86 (line in Coconut source)

    return new_tb_top  #91 (line in Coconut source)



def collect_exc_info(exc_type, exc_val, exc_tb):  #94 (line in Coconut source)
    """Collect information about the given exception for sending to Claude."""  #95 (line in Coconut source)
    filtered_tb = filter_traceback(exc_tb)  #96 (line in Coconut source)

    frame_info = inspect.getframeinfo(filtered_tb.tb_frame)  #98 (line in Coconut source)
    source_lines, source_lineno = inspect.getsourcelines(filtered_tb)  #99 (line in Coconut source)
    pretty_tb = ("\n".join)(traceback.format_exception(exc_type, exc_val, filtered_tb))  #100 (line in Coconut source)

    return DebugContext(name="exception", frame_info=frame_info, source_lines=source_lines, extra_info=_coconut.dict((("traceback", pretty_tb),)))  #102 (line in Coconut source)
