...
_coconut_header_info = ('3.1.1-post_dev2', '39', True)
...
def collect_frame_info(debug_context_name, frame, extra_info=None):  #102 (line in Coconut source)
    ...
    return DebugContext(name=debug_context_name, frame_info=frame_info, source_type=source_type, raw_source=raw_source, extra_info=((lambda _coconut_x: {} if _coconut_x is None else _coconut_x)(extra_info)) | {"locals": format_vars(frame.f_locals, max_size=get_verbosity() * 50, max_len=get_verbosity() * 1000), "globals": format_vars(frame.f_globals, max_size=get_verbosity() * 5, max_len=get_verbosity() * 100)})  #107 (line in Coconut source)
...
def collect_exc_info(exc_type, exc_val, exc_tb):  #155 (line in Coconut source)
    ...
    extra_info = {"traceback": (("".join)(traceback.format_exception(exc_type, exc_val, filtered_tb)))}  #159 (line in Coconut source)
