...
BASE_PROMPT = """Your task is to analyze the provided source code, breakpoint contexts, and/or unhandled exceptions to identify issues and provide helpful debugging suggestions.

You are being called by the `claude_here` debugging library, which the user is using to ask you for debugging help; any cases where you see `import claude_here` or `breakpoint()` are the user asking you for help. It is expected that `claude_here` will be imported and not used.

Carefully examine the attached source code, breakpoint contexts, and/or unhandled exceptions. Your goal is to identify the root cause of any errors or issues and suggest solutions to fix them. Follow these steps:
...
Remember to be thorough in your analysis, clear in your explanations, and helpful in your suggestions. Your goal is to assist the user in resolving their Python code issues effectively."""  #42 (line in Coconut source)
...
def assemble_prompt(_coconut_match_first_arg=_coconut_sentinel, *_coconut_match_args, **_coconut_match_kwargs):  #49 (line in Coconut source)
    ...
    if not _coconut_match_check_2:  #65 (line in Coconut source)
        ...
        if _coconut_match_check_2:  #65 (line in Coconut source)
                ...
                return "\n```python\n{_coconut_format_0}\n```\n".format(_coconut_format_0=(code.rstrip()))  #66 (line in Coconut source)
...
def generate_prompt(all_debug_context, max_context_items):  #69 (line in Coconut source)
    ...
    prompt_cmpts = []  #71 (line in Coconut source)

    for filepath, debug_contexts in all_debug_context.items():  #73 (line in Coconut source)
        coconut_filepath = os.path.splitext(filepath)[0] + ".coco"  #74 (line in Coconut source)
        if os.path.exists(coconut_filepath):  #75 (line in Coconut source)
            filepath = coconut_filepath  #76 (line in Coconut source)

        with open(filepath, "r") as fobj:  #78 (line in Coconut source)
            source_code = fobj.read()  #79 (line in Coconut source)
        filename = os.path.basename(filepath)  #80 (line in Coconut source)
        file_prompt_cmpts = [_coconut.dict((("source_code", source_code),)),]  #81 (line in Coconut source)

        seen_code = {source_code}  #85 (line in Coconut source)
        for ctx in _coconut_iter_getitem(debug_contexts, _coconut.slice(-max_context_items, None)):  #86 (line in Coconut source)
            info_dict = _coconut.dict()  #87 (line in Coconut source)
            if ctx.raw_source not in seen_code:  #88 (line in Coconut source)
                info_dict["surrounding_code"] = ctx.raw_source  #89 (line in Coconut source)
                seen_code.add(ctx.raw_source)  #90 (line in Coconut source)
            info_dict |= _coconut.dict((("executing_function", ctx.function), ("executing_line", ctx.raw_context), ("line_number", (str)(ctx.lineno))))  #91 (line in Coconut source)
        ...
        prompt_cmpts += [_coconut.dict(((filename, (SeparatedBy)("\n\n", file_prompt_cmpts)),)),]  #99 (line in Coconut source)

    attachment = assemble_prompt((SeparatedBy)("\n\n", prompt_cmpts))  #101 (line in Coconut source)
    return (_coconut_mk_anon_namedtuple(('prompt', 'attachment'))(BASE_PROMPT, attachment))  #102 (line in Coconut source)
