...
def breakpointhook(just_gather_info=False, **kwargs):  #8 (line in Coconut source)
...
EXTRA_RECURSION_LIMIT = 100  #16 (line in Coconut source)


def excepthook(exc_type, exc_val, exc_tb):  #19 (line in Coconut source)
    """Claude exception handler."""  #20 (line in Coconut source)
# if we're recovering from a RecursionError, we'll need some extra stack
    sys.setrecursionlimit(sys.getrecursionlimit() + EXTRA_RECURSION_LIMIT)  #22 (line in Coconut source)
    collect_exc_info(exc_type, exc_val, exc_tb)  #23 (line in Coconut source)
    traceback.print_exception(exc_type, exc_val, exc_tb)  #24 (line in Coconut source)
    launch_claude()  #25 (line in Coconut source)
