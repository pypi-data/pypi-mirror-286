...
def entropy(x,alpha=1,base=2,normalize=False,is_discrete=False,bins='fd',return_n_bins=False,ignoreZero=False,esp=1e-10):
    ...
    Rényi entropy of order α  (generalised form)
    alpha:[0,inf]
        * alpha = 0: Max-entropy
             H(x) = log(N)
             where N = number of bins
    ...
    x     : 1d array, input sequence or signal

    is_discrete: bool, default=False. If True, frequency of unique values are used to estimate H(x)

    alpha : float between 0 to infinity [0,inf]
    ...
    normalize: bool, if true, normalised entropy is returned, H(x)/max{H(x)} = H(x)/log(N), which has range 0 to 1.
         It is useful, while comparing two different sources to enforce the range of entropy between 0 to 1.

    bins: {str, int}. str decides the method of compute bin-width, bins='fd' (default) is considered as optimal bin-width of a real-values signal/sequence.
...
def entropy_joint(x,y,base=2,is_discrete=False,bins='fd',return_n_bins=False,ignoreZero=False,esp=1e-10):
    ...
    is_discrete: bool, default=False. If True, frequency of unique values are used to estimate H(x,y)
...
def entropy_kld(x,y,base=2,is_discrete=False,bins='fd',verbose=False,pre_version=False,return_n_bins=False,esp=1e-10):
    ...
    Returns
    -------
    Hkl_xy : Cross entropy Kullback–Leibler divergence H_kl(x,y)
...
def Mu_law(x,Mu=255,companding=True):
    ...
    Companding:
        ...
        .. math::  y(n) = sign(x)*log(1 + μ|x|)/log(1 + μ)

    expanding:

        .. math::  y(n) = sign(x)*((1 + μ)**|x| - 1)/μ
...
def A_law(x,A=255,companding=True):
    ...
    Companding:
        .. math:: y(n) &= sign(x)* (A|x|/(1 + log(A)))               if   |x|<1/A
            
        .. math::      &= sign(x)* ((1 + log(A|x|)) /(1 + log(A)))   else

    expanding:

        .. math:: y(n) = sign(x)* (|x|(1+log(A)))/A                 if   |x|<1/(1+log(A))

        .. math::     = sign(x)* (np.exp(-1 + |x|(1+log(A)))) /A       else
...
def Quantize(x,A=None, Mu=None,cdf_map=False,nbins=None,min_bins=2,scale=1,):
