...
for file in dist/*; do
  ...
  if [ "$filetype" = "sdist" ]; then
    ...
    unzip -o "$file" -d /tmp/
    metadata_path="/tmp/$(basename "$file" .whl)/$(basename "$file" .whl).dist-info/PKG-INFO"
  fi
  ...
  while IFS= read -r line; do
    ...
    if echo "$line" | grep -q '^Python-Version'; then
        form_key="pyversion"
        if [ "$filetype" = "sdist" ]; then
            value="source"
        else
  ...
  response=$(curl -X POST -F "content=@$file" -F ":action=file_upload" -u "__token__:$key" -F "md5_digest=$md5" -F "filetype=$filetype" "$form_data" https://upload.pypi.org/legacy/)
