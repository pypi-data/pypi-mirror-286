...
class GitlabConnection:
    ...
    def update(self, endpoint: str, params: dict = dict(), payload: dict = dict(), timeout=10) -> dict:
        ...
        # attempt request to service
        try:
            result = requests.put(url, json=payload, cert=(os.environ["REQUESTS_CLIENT_CERT"], os.environ["REQUESTS_CLIENT_KEY"]), timeout=timeout) if (CERT_IS_PRESENT and KEY_IS_PRESENT) else requests.post(url, json=payload, timeout=timeout)
        except Exception as e:
            raise e
