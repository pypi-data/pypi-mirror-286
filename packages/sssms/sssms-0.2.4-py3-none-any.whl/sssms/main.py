...
def cli(ctx, noproxy):
    ...
    if not noproxy and cfg and 'proxy' in cfg and ctx.invoked_subcommand != 'init':
...
def conf(**kwargs):
    ...
    actions = {
        'delete': lambda: compose(
            lambda _: config.delete(),
            partial(console.print, style="green")
        )("Configuration file deleted successfully."),
        'path': lambda: console.print(f"Configuration file path: {config.__showpath__()}", style="cyan"),
        'show': lambda: compose(
            config.load,
            maskinfo,
            yaml.dump,
            partial(Panel, title="Current Configuration", expand=False),
            console.print
        )(),
    }

    if any(map(itemgetter(1), filter(lambda x: x[0] in actions and x[1], kwargs.items()))):
        return next(action() for key, action in actions.items() if kwargs.get(key))

    updates = {f"credentials.{k}": v for k, v in {'email': kwargs['email'], 'password': kwargs['password']}.items() if v} | \
              {f"encryption.secret_key": kwargs['secret']} if kwargs['secret'] else {} | \
              {f"cache.{k}": v for k, v in {'lookups': kwargs['cachelookups'], 'path': kwargs['cachepath']}.items() if v is not None} | \
              {f"smtp.server": kwargs['server']} if kwargs['server'] else {}

    if updates:
        try:
            compose(
                partial(reduce, lambda acc, item: updatenest(acc, *item), updates.items()),
                config.update,
                lambda _: console.print("Configuration updated successfully.", style="green")
            )({})
        except Exception as e:
