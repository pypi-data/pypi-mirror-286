...
def load(path:str=None):
    ...
    if os.path.exists(path):
        ...
        if 'credentials' in content and 'password' in content['credentials']:
            ...
            content['credentials']['password'] = fernet.decrypt(content['credentials']['password'].encode()).decode()
        return content
...
def update(path:str=None, **kwargs):
    ...
    if os.path.exists(path):
        ...
        if 'email' in kwargs:
            content['credentials']['email'] = kwargs['email']
        if 'password' in kwargs:
            key = content['encryption']['key']
            key += '=' * (-len(key) % 4)
            fernet = Fernet(key.encode())
            content['credentials']['password'] = fernet.encrypt(kwargs['password'].encode()).decode()
        if 'secret' in kwargs:
            new_key, new_salt = __key__(kwargs['secret'])
            content['encryption']['key'] = new_key.decode()
            content['encryption']['salt'] = new_salt
        if 'cachelookups' in kwargs:
            content['cache']['lookups'] = kwargs['cachelookups']
        if 'cachepath' in kwargs:
            content['cache']['path'] = kwargs['cachepath']
        if 'server' in kwargs:
            content['smtp']['server'] = kwargs['server']

        with open(path, 'w') as f:
            yaml.dump(content, f)
        print(f"Config file updated at {path}")
    else:
        raise FileNotFoundError('Config file not found')
...
def delete(path:str=None):
    ...
    if os.path.exists(path):
        os.remove(path)
        print(f"Config file deleted from {path}")
    else:
        raise FileNotFoundError('Config file not found')
