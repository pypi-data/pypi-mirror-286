...
class ApplicationContext:
    def __init__(self, app_name, input_dir, input_file_mapping = {}):
...
def run_grid_search(tune_config):
    ...
    return sweeper.sweep_params(access_token=get_access_token(), job_name=job_name,
                             ...
                             input_file_mapping=tune_config.app_context.input_file_mapping,
                             queue_name=tune_config.execution_context.queue,
                             cpu_count=tune_config.execution_context.cpu)
...
def get_remote_data_dirs(job_name, working_dir, config_file_location = 'settings.ini', cluster_scratch = "/scratch/"):
    all_experiments = sweeper.get_all_experiments(get_access_token(), job_name, working_dir, config_file_location)
    data_dirs = []
    for exp in all_experiments:
        if len(exp.processes) > 0:
            data_dirs.append(cluster_scratch + exp.processes[0].processId)
        else:
            data_dirs.append("Error")
    return data_dirs
