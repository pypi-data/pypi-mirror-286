...
class PostgresDB:
    ...
    def _build_sql_prompt(self, query: str) -> str:
        ...
        prompt += f"""
            ...
            - When doing computation, cast values to the appropriate type. For example, CAST(rushing_rush_yds AS NUMERIC) before using the ROUND function.
            - When rounding numbers, use the ROUND function with explicit type casting, e.g., ROUND(CAST(column_name AS NUMERIC), 2).
