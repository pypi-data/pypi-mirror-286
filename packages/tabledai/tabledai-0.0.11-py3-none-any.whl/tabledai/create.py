...
class PostgresDB:
    ...
    def _create_or_update_table(self, table_name: str, file_path: str) -> None:
        ...
        if not non_duplicate_data.empty:
            ...
            logging.info(f"No new data to insert for table '{table_name}'. All records are duplicates.")
            logging.info(f"-" * 100)
    ...
    def add_foreign_key(self, table_name: str, field: str, reference_table: str, reference_field: str) -> None:
        with self.engine.connect() as con:
            # if no reference table or field is provided, check for existing restraint and if it exists, drop it
            try:
            ...
            try:
                if all:
                    ...
                    for fk in foreign_keys:
                        alter_table_query = f"ALTER TABLE {table_name} DROP CONSTRAINT {fk[0]}"
    ...
    @disable_logging
    def _generative_openai_call(self, prompt: str) -> None:
        try:
            response = client.chat.completions.create(
                model="gpt-4o",
                messages=[
    ...
    def query(self, query: str, generative: bool = False) -> dict:
        result = {'sql': None, 'data': pd.DataFrame(), 'generation': None}
        ...
        if sql:
            ...
            logging.info(f"_" * 100)
            result['sql'] = sql
            result_df = self._query_db(sql)

            if not result_df.empty:
                logging.info(f"Query executed successfully.")
                logging.info(f"_" * 100)
                result['data'] = result_df.to_dict()

                if generative:
                    generative_prompt = self._build_generative_prompt(query, result_df)
                    generative_result = self._generative_openai_call(generative_prompt)
                    result['generation'] = generative_result
            else:
