...
class PostgresDB:
    ...
    def _sql_openai_call(self, prompt: str) -> str:
        ...
        if isinstance(eval(result), dict) and 'sql' in eval(result):
            return eval(result).get('sql')
        return None
    ...
    def query(self, query: str, generative: bool = False) -> dict:
        ...
        if not db_result_dict.get('success'):
            ...
            retry_prompt = (
                ...
                f"This SQL failed to execute due to the following error: '{db_result_dict.get('data')}'. Please try again, taking into account the error message."
            )
