...
from zippy_form.serializers import FormSerializer, FormStepSerializer, MapFieldToFormStepSerializer, UpdateFieldSettingsSerializer, \
    ReOrderFieldSerializer, AccountSerializer, WebhookSerializer, FormPaymentSettingsSerializer, AccountProfileUpdateSerializer, AccountPaymentUpdateSerializer, FormSettingsSerializer
...
def get_form_settings_details(req, form_id):
    ...
    if gsheet_enabled:
        try:
            google_credentials = GoogleCredentials.objects.filter(account=account_id).get()
...
def update_form_settings_details(req, form_id):
    ...
    if form_settings:
        ...
        if gsheet_enabled and recaptcha_enabled:
...
def create_update_gsheet(req, form, gsheet_type=GSHEET_TYPES[0][0], sheet_name=None):
    """
    Create or Update GSheet based on the type selected & auto sync active fields
    """
