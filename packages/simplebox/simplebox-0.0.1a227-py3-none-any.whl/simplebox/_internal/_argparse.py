...
This can be used to ensure the order is deterministic for hashing or tests. It has a substantial performance penalty and is not recommended in general.default False.""")
        ...
        self.__parser.add_argument("--autoname", action="store_true", default=False,
                                   help="If the name of the json field is not specified and the autoname is True, the python attribute name will be tried as the json field name, and the private attribute will try to be replaced with a public attribute (only if serialized), JsonField and JsonSerializable used. default False.")
