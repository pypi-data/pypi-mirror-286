...
class RestOptions(Dictionary):
    ...
    def __init__(self, params: dict = None, data: list or dict or complexjson.JsonSerializable = None,
                 ...
                 json: list or dict or complexjson.JsonSerializable = None, restful: dict = None, **kwargs):
        super().__init__()

        self.update(params=params, data=complexjson.parser(data), headers=headers, cookies=cookies, files=files,
                    auth=auth, timeout=timeout, allow_redirects=allow_redirects, proxies=proxies, hooks=hooks,
                    stream=stream, verify=verify, cert=cert, json=complexjson.parser(json), restful=restful, **kwargs)
