...
class App(object):
    def __init__(self):
        ...
        if (currentPlatform == OS.MAC.value) or (currentPlatform == OS.WINDOWS.value):
            load_simba_fonts()
            self.regular_font, self.header_font = ('Poppins Regular', 12), ('Poppins Regular', 14, "bold")
        else:
            self.regular_font, self.header_font = get_linux_fonts()
        ...
        file_menu.add_command(label="Create a new project", compound="left", image=self.menu_icons["create"]["img"], command=lambda: ProjectCreatorPopUp(), font=self.regular_font)
        file_menu.add_command(label="Load project", compound="left", image=self.menu_icons["load"]["img"], command=lambda: LoadProjectPopUp(), font=self.regular_font)
        file_menu.add_separator()
        file_menu.add_command(label="Restart", compound="left", image=self.menu_icons["restart"]["img"], command=lambda: self.restart(), font=self.regular_font)
        file_menu.add_separator()
        file_menu.add_command(label="Exit", compound="left", image=self.menu_icons["exit"]["img"], command=self.root.destroy, font=self.regular_font)

        batch_process_menu = Menu(menu)
        menu.add_cascade(label="Process Videos", menu=batch_process_menu)
        batch_process_menu.add_command(label="Batch pre-process videos", compound="left", image=self.menu_icons["factory"]["img"], command=lambda: BatchPreProcessPopUp(), font=self.regular_font)

        video_process_menu = Menu(menu)
        fps_menu = Menu(video_process_menu)
        fps_menu.add_command(label="Change FPS for single video", command=ChangeFpsSingleVideoPopUp, font=self.regular_font)
        fps_menu.add_command(label="Change FPS for multiple videos", command=ChangeFpsMultipleVideosPopUp, font=self.regular_font)
        fps_menu.add_command(label="Up-sample fps with interpolation", command=UpsampleVideosPopUp, font=self.regular_font)

        menu.add_cascade(label="Tools", menu=video_process_menu)
        video_process_menu.add_cascade(label="Change FPS...", compound="left", image=self.menu_icons["fps"]["img"], menu=fps_menu, font=self.regular_font)

        clip_video_menu = Menu(menu)
        clip_video_menu.add_command(label="Clip single video", command=ClipVideoPopUp, font=self.regular_font)
        clip_video_menu.add_command(label="Clip multiple videos", command=InitiateClipMultipleVideosByTimestampsPopUp, font=self.regular_font)

        clip_video_menu.add_command(label="Clip video into multiple videos", command=MultiShortenPopUp, font=self.regular_font)
        clip_video_menu.add_command(label="Clip single video by frame numbers", command=ClipSingleVideoByFrameNumbers, font=self.regular_font)
        clip_video_menu.add_command(label="Clip multiple videos by frame numbers", command=InitiateClipMultipleVideosByFrameNumbersPopUp, font=self.regular_font)

        video_process_menu.add_cascade(label="Clip videos...", compound="left", image=self.menu_icons["clip"]["img"], menu=clip_video_menu, font=self.regular_font)

        crop_video_menu = Menu(menu)
        crop_video_menu.add_command(label="Crop videos", compound="left", image=self.menu_icons["crop"]["img"], command=CropVideoPopUp, font=self.regular_font)
        crop_video_menu.add_command(label="Crop videos (circles)", compound="left", image=self.menu_icons["circle"]["img"], command=CropVideoCirclesPopUp, font=self.regular_font)
        crop_video_menu.add_command(label="Crop videos (polygons)", compound="left", image=self.menu_icons["polygon"]["img"], command=CropVideoPolygonsPopUp, font=self.regular_font)
        crop_video_menu.add_command(label="Multi-crop", compound="left", image=self.menu_icons["crop"]["img"], command=MultiCropPopUp, font=self.regular_font)
        video_process_menu.add_cascade(label="Crop videos...", compound="left", image=self.menu_icons["crop"]["img"], menu=crop_video_menu, font=self.regular_font)

        format_menu = Menu(video_process_menu)
        img_format_menu = Menu(format_menu)
        video_format_menu = Menu(format_menu)

        img_format_menu.add_command(label="Convert images to PNG", command=Convert2PNGPopUp, font=self.regular_font)
        img_format_menu.add_command(label="Convert images  to JPEG", command=Convert2jpegPopUp, font=self.regular_font)
        img_format_menu.add_command(label="Convert images to BMP", command=Convert2bmpPopUp, font=self.regular_font)
        img_format_menu.add_command(label="Convert images  to TIFF", command=Convert2TIFFPopUp, font=self.regular_font)
        img_format_menu.add_command(label="Convert images  to WEBP", command=Convert2WEBPPopUp, font=self.regular_font)
        video_format_menu.add_command(label="Convert videos to MP4", command=Convert2MP4PopUp, font=self.regular_font)
        video_format_menu.add_command(label="Convert videos to AVI", command=Convert2AVIPopUp, font=self.regular_font)
        video_format_menu.add_command(label="Convert videos to WEBM", command=Convert2WEBMPopUp, font=self.regular_font)
        video_format_menu.add_command(label="Convert videos to MOV", command=Convert2MOVPopUp, font=self.regular_font)
        format_menu.add_cascade(label="Convert image file formats...", compound="left", menu=img_format_menu, font=self.regular_font)
        format_menu.add_cascade(label="Change video file formats...", compound="left", menu=video_format_menu, font=self.regular_font)
        video_process_menu.add_cascade(label="Convert file formats...", compound="left", image=self.menu_icons["convert"]["img"], menu=format_menu, font=self.regular_font)

        rm_clr_menu = Menu(video_process_menu)
        rm_clr_menu.add_command(label="Convert to grayscale", compound="left", image=self.menu_icons["greyscale"]["img"], command=lambda: GreyscaleSingleVideoPopUp(), font=self.regular_font)
        rm_clr_menu.add_command(label="Convert to black and white", compound="left", image=self.menu_icons["bw"]["img"], command=Convert2BlackWhitePopUp, font=self.regular_font)
        rm_clr_menu.add_command(label="CLAHE enhance videos", compound="left", image=self.menu_icons["clahe"]["img"], command=CLAHEPopUp, font=self.regular_font)
        rm_clr_menu.add_command(label="Interactively CLAHE enhance videos", compound="left", image=self.menu_icons["clahe"]["img"], command=InteractiveClahePopUp, font=self.regular_font)
        video_process_menu.add_cascade(label="Remove color from videos...", compound="left", image=self.menu_icons["clahe"]["img"], menu=rm_clr_menu, font=self.regular_font)

        concatenate_menu = Menu(video_process_menu)
        concatenate_menu.add_command(label="Concatenate two videos", compound="left", image=self.menu_icons["concat"]["img"], command=ConcatenatingVideosPopUp, font=self.regular_font)
        concatenate_menu.add_command(label="Concatenate multiple videos", compound="left", image=self.menu_icons["concat_videos"]["img"], command=lambda: ConcatenatorPopUp(config_path=None), font=self.regular_font)
        video_process_menu.add_cascade(label="Concatenate (stack) videos...", compound="left", image=self.menu_icons["concat"]["img"], menu=concatenate_menu, font=self.regular_font)
        video_process_menu.add_command(label="Convert ROI definitions", compound="left", image=self.menu_icons["roi"]["img"], command=lambda: ConvertROIDefinitionsPopUp(), font=self.regular_font)
        convert_data_menu = Menu(video_process_menu)
        convert_data_menu.add_command(label="Convert CSV to parquet", command=Csv2ParquetPopUp, font=self.regular_font)
        convert_data_menu.add_command(label="Convert parquet o CSV", command=Parquet2CsvPopUp, font=self.regular_font)

        video_process_menu.add_cascade(label="Convert working file type...", compound="left", image=self.menu_icons["change"]["img"], menu=convert_data_menu, font=self.regular_font)

        video_process_menu.add_command(label="Create path plot", compound="left", image=self.menu_icons["path"]["img"], command=MakePathPlotPopUp, font=self.regular_font)

        downsample_video_menu = Menu(video_process_menu)
        downsample_video_menu.add_command(label="Down-sample single video", command=DownsampleSingleVideoPopUp, font=self.regular_font)
        downsample_video_menu.add_command(label="Down-sample multiple videos", command=DownsampleMultipleVideosPopUp, font=self.regular_font)
        video_process_menu.add_cascade(label="Down-sample video...", compound="left", image=self.menu_icons["sample"]["img"], menu=downsample_video_menu, font=self.regular_font)
        video_process_menu.add_cascade(label="Drop body-parts from tracking data", compound="left", image=self.menu_icons["trash"]["img"], command=DropTrackingDataPopUp, font=self.regular_font)
        extract_frames_menu = Menu(video_process_menu, font=self.regular_font)
        extract_frames_menu.add_command(label="Extract defined frames", command=ExtractSpecificFramesPopUp, font=self.regular_font)
        extract_frames_menu.add_command(label="Extract frames", command=ExtractAllFramesPopUp, font=self.regular_font)
        extract_frames_menu.add_command(label="Extract frames from seq files", command=ExtractSEQFramesPopUp, font=self.regular_font)
        video_process_menu.add_cascade(label="Extract frames...", compound="left", image=self.menu_icons["frames"]["img"], menu=extract_frames_menu, font=self.regular_font)

        video_process_menu.add_command(label="Create GIFs", compound="left", image=self.menu_icons["gif"]["img"], command=CreateGIFPopUP, font=self.regular_font)

        video_process_menu.add_command(label="Get metric conversion factor (pixels/millimeter)", compound="left", image=self.menu_icons["calipher"]["img"], command=CalculatePixelsPerMMInVideoPopUp, font=self.regular_font)
        video_process_menu.add_command(label="Change video brightness / contrast", compound="left", image=self.menu_icons["brightness"]["img"], command=BrightnessContrastPopUp, font=self.regular_font)
        video_process_menu.add_command(label="Merge frames to video", compound="left", image=self.menu_icons["merge"]["img"], command=MergeFrames2VideoPopUp, font=self.regular_font)
        video_process_menu.add_command(label="Print classifier info...", compound="left", image=self.menu_icons["print"]["img"], command=PrintModelInfoPopUp, font=self.regular_font)

        video_process_menu.add_cascade(label="Reorganize Tracking Data", compound="left", image=self.menu_icons["reorganize"]["img"], command=PoseReorganizerPopUp, font=self.regular_font)

        rotate_menu = Menu(menu)
        rotate_menu.add_command(label="Rotate videos", command=RotateVideoSetDegreesPopUp, font=self.regular_font)
        rotate_menu.add_command(label="Interactively rotate videos", command=VideoRotatorPopUp, font=self.regular_font)
        rotate_menu.add_command(label="Flip videos", command=FlipVideosPopUp, font=self.regular_font)
        rotate_menu.add_command(label="Reverse videos", command=ReverseVideoPopUp, font=self.regular_font)
        video_process_menu.add_cascade(label="Rotate / flip / reverse videos...", compound="left", image=self.menu_icons["rotate"]["img"], menu=rotate_menu, font=self.regular_font)

        superimpose_menu = Menu(menu)
        superimpose_menu.add_command(label="Superimpose frame numbers", command=SuperImposeFrameCountPopUp, font=self.regular_font)
        superimpose_menu.add_command(label="Superimpose watermark", command=SuperimposeWatermarkPopUp, font=self.regular_font)
        superimpose_menu.add_command(label="Superimpose timer", command=SuperimposeTimerPopUp, font=self.regular_font)
        superimpose_menu.add_command(label="Superimpose progress-bar", command=SuperimposeProgressBarPopUp, font=self.regular_font)
        superimpose_menu.add_command(label="Superimpose video on video", command=SuperimposeVideoPopUp, font=self.regular_font)
        superimpose_menu.add_command(label="Superimpose video names", command=SuperimposeVideoNamesPopUp, font=self.regular_font)
        superimpose_menu.add_command(label="Superimpose free-text", command=SuperimposeTextPopUp, font=self.regular_font)
        video_process_menu.add_cascade(label="Superimpose on videos...", compound="left", image=self.menu_icons["superimpose"]["img"], menu=superimpose_menu, font=self.regular_font)

        temporal_join_videos = Menu(menu)
        temporal_join_videos.add_command(label="Temporal join all videos in directory", command=VideoTemporalJoinPopUp, font=self.regular_font)
        temporal_join_videos.add_command(label="Temporal join selected videos", command=ManualTemporalJoinPopUp, font=self.regular_font)
        video_process_menu.add_cascade(label="Temporal join videos...", compound="left", image=self.menu_icons["stopwatch"]["img"], menu=temporal_join_videos, font=self.regular_font)
        video_process_menu.add_command(label="Box blur videos", compound="left", image=self.menu_icons["blur"]["img"], command=BoxBlurPopUp, font=self.regular_font)
        video_process_menu.add_command(label="Cross-fade videos", compound="left", image=self.menu_icons["crossfade"]["img"], command=CrossfadeVideosPopUp, font=self.regular_font)
        video_process_menu.add_command(label="Create average frames from videos", compound="left", image=self.menu_icons["average"]["img"], command=CreateAverageFramePopUp, font=self.regular_font)
        video_process_menu.add_command(label="Video background remover...", compound="left", image=self.menu_icons["remove_bg"]["img"], command=BackgroundRemoverPopUp, font=self.regular_font)
        video_process_menu.add_command(label="Visualize pose-estimation in folder...", compound="left", image=self.menu_icons["visualize"]["img"], command=VisualizePoseInFolderPopUp, font=self.regular_font)
        help_menu = Menu(menu)
        menu.add_cascade(label="Help", menu=help_menu)

        links_menu = Menu(help_menu)
        links_menu.add_command(label="Download weights", command=lambda: webbrowser.open_new(str(r"https://osf.io/sr3ck/")), font=self.regular_font)
        links_menu.add_command(label="Download classifiers", command=lambda: webbrowser.open_new(str(r"https://osf.io/kwge8/")), font=self.regular_font)
        links_menu.add_command(label="Ex. feature list", command=lambda: webbrowser.open_new(str(r"https://github.com/sgoldenlab/simba/blob/master/misc/Feature_description.csv")), font=self.regular_font)
        links_menu.add_command(label="SimBA github", command=lambda: webbrowser.open_new(str(r"https://github.com/sgoldenlab/simba")), font=self.regular_font)
        links_menu.add_command(label="Gitter Chatroom", command=lambda: webbrowser.open_new(str(r"https://gitter.im/SimBA-Resource/community")), font=self.regular_font)
        links_menu.add_command(label="Install FFmpeg", command=lambda: webbrowser.open_new(str(r"https://m.wikihow.com/Install-FFmpeg-on-Windows")), font=self.regular_font)
        links_menu.add_command(label="Install graphviz", command=lambda: webbrowser.open_new(str(r"https://bobswift.atlassian.net/wiki/spaces/GVIZ/pages/20971549/How+to+install+Graphviz+software")), font=self.regular_font)
        links_menu.add_command(label="SimBA API", command=lambda: webbrowser.open_new(str(r"https://simba-uw-tf-dev.readthedocs.io/")), font=self.regular_font)
        help_menu.add_cascade(label="Links", menu=links_menu, compound="left", image=self.menu_icons["link"]["img"], font=self.regular_font)
        help_menu.add_command(label="About", compound="left", image=self.menu_icons["about"]["img"], command=AboutSimBAPopUp, font=self.regular_font)

        self.frame = Frame(background, bd=2, relief=SUNKEN, width=750, height=300)
        self.r_click_menu = Menu(self.root, tearoff=0)
        self.r_click_menu.add_command(label="Copy selection", command=lambda: self.copy_selection_to_clipboard(), font=self.regular_font)
        self.r_click_menu.add_command(label="Copy all", command=lambda: self.copy_all_to_clipboard(), font=self.regular_font)
        self.r_click_menu.add_command(label="Paste", command=lambda: self.paste_to_txt(), font=self.regular_font)
        self.r_click_menu.add_separator()
        self.r_click_menu.add_command(label="Clear", command=lambda: self.clean_txt(), font=self.regular_font)
