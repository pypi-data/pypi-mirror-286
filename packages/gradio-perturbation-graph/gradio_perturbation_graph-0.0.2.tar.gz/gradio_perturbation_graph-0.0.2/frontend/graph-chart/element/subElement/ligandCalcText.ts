...
import {
    ...
    LIGAND_WIDTH_HALF,
    LIGAND_TEXT_STYLE,
    LIGAND_CALC_TEXT_STYLE,
    LIGAND_EXP_CALC_DIS,
    LIGAND_EXP_CALC_LINEHEIGHT,
} from '../../constant';
import { formulaList, skew, superscriptOffset, subscriptOffset } from '../../helper';

class LigandCalcTextElement extends LigandSubElement {
    type = 'ligandCalcText';
    el = new Group({});
    ...
    moveTo({ x, y, originX, originY, calc_data, exp, calc_error, info }: MoveToParams) {
        ...
        const formula = {
            ...formulaList[key],
            text: `${calc_data}${info?.includes('predDGErr') && calc_error ? `±${calc_error}` : ''}`,
        }
        if (calc_error) {
            formula.style.padding[3] = 5;
        } else {
            formula.style.padding[3] = 30;
        }
        const position ={ x: -LIGAND_WIDTH_HALF, y: LIGAND_WIDTH_HALF - lineHeight - (twoLines ? lineHeight : 0)};
        const vecX = { x: 1, y: 0 };
        const vecY = { x: 0, y: 3 };
        if (info?.includes(key) && calc_data) {
...
export default LigandCalcTextElement;
