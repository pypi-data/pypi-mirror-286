...
def fetch_from_external_src(spark, secrets, src_db_name, target_db_name, table_name, timestmp_col):
  ...
  max_updated_prev_minute = last_time_for(spark, target_db_name, table_name, timestmp_col, 1)
  query = f"""
...
def external_src_incremental_load(spark, secrets, src_db_name, target_db_name, table_name):
  print("Loading data from external source...")
  df = fetch_from_external_src(spark, secrets, src_db_name, target_db_name, table_name, 'updatedAt')
