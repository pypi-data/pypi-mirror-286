...
class Unit_Registry:
    ...
    def is_valid_unit(self, unit_str):
        ur = self.ur
        try:
            ur(unit_str)
            return True
    ...
    def getUnit(self, string, ur=None):
        """
        ...
        if string is None:
            ...
            string = (
                string.replace("$", "USD").replace("€", "EUR").replace("%", "percent")
            )
        return ur(string)

    def conversionFactor(self, unitFrom, unitTo, context=None):
        """
