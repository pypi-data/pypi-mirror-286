...
class OnPremNodeBase(BaseModel):
    ...
    def __init__(self, **data: Any):
        ...
        if data.get("role", None) not in [role.name for role in OnPremVMRole]:
            if data.get("role", None) in [VMRole.CONTROLLER.name, VMRole.INFERENCE_CONTROLLER.name]:
...
class InferenceControllerOnPremConfig(BaseModel):
    use_ssl: bool = True
    on_prem_node_id: Optional[str] = None # if not provided, all cpu-only nodes are considered candidates
...
class InferenceDeploymentIn(BaseModel):
    ...
    def __init__(self, **data: Any):
        ...
        if not data.get("controller_cloud_config", None) and not data.get("controller_on_prem_config", None):
            raise ValueError("controller_cloud_config or controller_on_prem_config must be provided")
        
        # Validate that only one of controller_cloud_config or controller_on_prem_config is provided
        if data.get("controller_cloud_config", None) and data.get("controller_on_prem_config", None):
            raise ValueError("controller_cloud_config and controller_on_prem_config cannot be provided together")
...
class InferenceDeploymentDB(InferenceDeploymentIn):
    ...
    def __init__(self, **data: Any):
        ...
        if not data.get("controller_cloud_config", None) and not data.get("controller_on_prem_config", None):
            data["controller_cloud_config"] = InferenceControllerCloudConfig(cloud_provider=ProviderEnum.AWS, region="us-east-1").model_dump(mode="json")
...
class OpenaiWandbIntegration(BaseModel): # Not used, implemented for completeness of openai api
    project: str
    name: Optional[str] = None
...
class InferenceDeploymentGatewayCloudChoice(BaseModel): # TODO: Define this class properly
    pass
