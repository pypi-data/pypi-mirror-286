...
from django_keycloak_lma.models import (
    Client,
    OpenIdConnectProfile,
    RemoteClient
)
import django_keycloak_lma.services.permissions
import django_keycloak_lma.services.realm
import django_keycloak_lma.services.uma
...
def synchronize_permissions(modeladmin, request, queryset):
    for realm in queryset:
        try:
            django_keycloak_lma.services.permissions.synchronize(
                client=realm.client)
...
def synchronize_resources(modeladmin, request, queryset):
    for realm in queryset:
        try:
            django_keycloak_lma.services.uma.synchronize_client(
                client=realm.client)
