...
from keycloak.models import (
    Client,
    OpenIdConnectProfile,
    RemoteClient
)
import keycloak.services.permissions
import keycloak.services.realm
import keycloak.services.uma


def refresh_open_id_connect_well_known(modeladmin, request, queryset):
    for realm in queryset:
        keycloak.services.realm.refresh_well_known_oidc(realm=realm)
...
def refresh_certs(modeladmin, request, queryset):
    for realm in queryset:
        keycloak.services.realm.refresh_certs(realm=realm)
    modeladmin.message_user(
        request=request,
...
def synchronize_permissions(modeladmin, request, queryset):
    for realm in queryset:
        try:
            keycloak.services.permissions.synchronize(
                client=realm.client)
...
def synchronize_resources(modeladmin, request, queryset):
    for realm in queryset:
        try:
            keycloak.services.uma.synchronize_client(
                client=realm.client)
