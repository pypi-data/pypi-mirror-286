...
from keycloak.remote_user import KeycloakRemoteUser
...
def get_or_create_from_id_token(client, id_token, check_client=False):
    """
    Get or create OpenID Connect profile from given id_token.

    :param keycloak.models.Client client:
    :param str id_token:
    :param check_client:
    :rtype: keycloak.models.OpenIdConnectProfile
    """
    issuer = keycloak.services.realm.get_issuer(client.realm)
    ...
    if check_client:
        if client.client_id != id_token_object['azp']:
            raise exceptions.Conflict(detail=messages.get_message(messages.MISMATCH_CLIENT_ERROR))

    with transaction.atomic():
        oidc_profile = update_or_create_user_and_oidc_profile(
            client=client, id_token_object=id_token_object)

        oidc_profile.access_token = id_token
        oidc_profile.expires_before = pytz.UTC.localize(datetime.fromtimestamp(id_token_object['exp']))
...
def get_oidc_profile_by_token(client, id_token_object):
    ...
    try:
        ...
        raise exceptions.NotFound(detail=messages.obj_id_not_found(id_token_object['sub'], messages.get_message(messages.OIDC_PROFILE_NOT_FOUND)))
...
def update_tokens(token_model, token_response, initiate_time):
    ...
    :param keycloak.models.TokenModelAbstract token_model:
...
def get_active_access_token(oidc_profile):
    ...
    :param keycloak.models.KeycloakOpenIDProfile oidc_profile:
    :rtype: string
    :raise: keycloak.services.exceptions.TokensExpired
    """
    initiate_time = timezone.now()
...
def refresh_token(client, refresh_token_obj):
    try:
        ...
        raise exceptions.AuthenticationFailed(detail=messages.get_message(messages.REFRESH_TOKEN_ERROR))
...
def token_introspect(client, token):
    response = client.openid_api_client.token_introspect(token)
    return response
