...
from django_keycloak_lma.factories import OpenIdConnectProfileFactory
from django_keycloak_lma.tests.mixins import MockTestCaseMixin
from django_keycloak_lma.auth.backends import KeycloakAuthorizationBase
...
class BackendsKeycloakAuthorizationBaseHasPermTestCase(
    ...
    def setUp(self):
        ...
        self.setup_mock(
            'django_keycloak_lma.services.oidc_profile.get_entitlement',
            return_value={
                'authorization': {
                    'permissions': [
                        {
