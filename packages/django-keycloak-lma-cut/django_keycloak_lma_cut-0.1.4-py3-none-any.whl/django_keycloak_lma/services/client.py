...
def get_keycloak_id(client):
    ...
    :param django_keycloak_lma.models.Realm realm:
    :return:
    """
...
def get_uma1_client(client):
    """
    :type client: django_keycloak_lma.models.Client
...
def get_service_account_profile(client):
    ...
    oidc_profile = django_keycloak_lma.services.oidc_profile._update_or_create(
        client=client,
...
def get_access_token(client):
    ...
    try:
        return django_keycloak_lma.services.oidc_profile.get_active_access_token(
            oidc_profile=oidc_profile)
    except TokensExpired:
        token_reponse, initiate_time = get_new_access_token(client=client)
        oidc_profile = django_keycloak_lma.services.oidc_profile.update_tokens(
