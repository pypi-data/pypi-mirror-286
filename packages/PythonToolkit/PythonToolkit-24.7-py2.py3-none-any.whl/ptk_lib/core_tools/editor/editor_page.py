...
class CellDialog(wx.Dialog):
    def __init__(self, parent, id=-1):
        ...
        butsizer.Add( self.ok_but, 0, wx.ALL,5)
        butsizer.Add( self.cancel_but, 0, wx.ALL,5)
...
class EditorPage(wx.stc.StyledTextCtrl):
    ...
    def GetCurrentCell(self):
        ...
        #wxpython 4.1 changed api to return tuple
        if (cell_start == -1) or (cell_start == (-1,-1)):
            return None

        cell_end   =  self.FindText(pos, self.GetLength(),'#%%',flags=0)
        #wxpython 4.1 changed api to return tuple
        if (cell_end == -1) or (cell_end == (-1,-1)):
            cell_end = self.GetLength()
        else:
            cell_end = cell_end[0]

        cell = self.GetTextRange(cell_start[0], cell_end)
        return cell
