...
class Engine(MBClient):
    ...
    def msg_register_task(self, msg):
        ...
        s = msg.data[0]
        #execute code in empty globals and use _tasks as local
        exec( s, {}, self._tasks ) #store directly into here
        return True
    ...
    def msg_add_builtin(self, msg):
        ...
        name, s = msg.get_data()
        code = compile(s, 'PTK builtin','single') # marshal.loads(s)
        #give builtin command access to user dict and store directly into l
        l = {}
        exec( s, self._userdict, l) 
        #add to builtins
        builtins.__dict__[name] = l[name]
        return True
