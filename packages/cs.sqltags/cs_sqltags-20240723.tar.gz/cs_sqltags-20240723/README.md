...
## Class `BaseSQLTagsCommand(cs.cmdutils.BaseCommand, SQLTagsCommandsMixin)`
...
*`BaseSQLTagsCommand.apply_opt(self, opt, val)`*:
Apply a command line option.

*`BaseSQLTagsCommand.parse_categories(categories)`*:
Extract "category" words from the `str` `categories`,
return a list of category names.

Splits on commas, strips leading and trailing whitespace, downcases.

*`BaseSQLTagsCommand.parse_tagset_criterion(arg, tag_based_test_class=None)`*:
...
## Class `PolyValue(PolyValue, PolyValued)`
...
## `prefix2like(prefix: str, esc='\\') -> str`

Convert a prefix string to an SQL LIKE pattern.

## Class `SQLParameters(SQLParameters)`
...
## Class `SQLTagBasedTest(cs.tagset.TagBasedTest, SQTCriterion)`
...
## Class `SQLTags(cs.tagset.BaseTagSets, cs.deco.Promotable)`
...
*`SQLTags.default_factory(self, name: Optional[str] = None, *, unixtime=None, tags=None, skip_refresh=False)`*:
...
*`SQLTags.items(self, *, prefix=None)`*:
...
## Class `SQLTagSet(cs.obj.SingletonMixin, cs.tagset.TagSet)`
...
## Class `SQLTagsORM(cs.sqlalchemy_utils.ORM, cs.dateutils.UNIXTimeMixin)`
