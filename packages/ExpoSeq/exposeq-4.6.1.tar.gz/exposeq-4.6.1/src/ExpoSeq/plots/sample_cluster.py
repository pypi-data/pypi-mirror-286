...
class PrepareData:
    ...
    def get_top_10_percent(self, group):
        ...
        return sorted_group[cumulative_sum <= self.summed_clonefraction][
            : self.max_num_reads
        ]
...
class ClusterExperiment:
    def __init__(
        ...
        sequencing_report,
        region_string,
        summed_clonefraction,
        max_num_reads,
        ax=None,
        edge_color="peachpuff",
        max_weight_lines=100,
    ):
        report = sequencing_report
        self.region_string = region_string
        report = self.prepare_data(
            sequencing_report, summed_clonefraction, max_num_reads
        )
        self.G = nx.Graph()
    ...
    def prepare_data(sequencing_report, summed_clonefraction, max_num_reads):
        report = PrepareData(summed_clonefraction, max_num_reads).cleaning(
            sequencing_report
        )
        return report
                    ...
                    if distance != 0:
                        if distance > 5:
                            self.G.add_edge(
                                seq1,
                                seq2,
                                weight=int(max_weight_lines * 1 / (distance**2)),
                            )
        ...
        for index, g in enumerate(self.G):
            for i in report[self.region_string]:
                if i == g:
                    node = math.sqrt(
                        report.loc[
                            report[self.region_string] == i, "cloneFraction"
                        ].values[0]
                    )
    ...
    def create_colors(self, report):
        ...
        sample_colors = {
            sample: color
            for sample, color in zip(report["Experiment"].unique(), palette)
        }
    ...
    def draw_plot(self, nodesize, node_colors, ax, edge_color):
        ...
        nx.draw_networkx_nodes(
            self.G, pos, node_size=nodesize, node_color=node_colors, ax=ax
        )
