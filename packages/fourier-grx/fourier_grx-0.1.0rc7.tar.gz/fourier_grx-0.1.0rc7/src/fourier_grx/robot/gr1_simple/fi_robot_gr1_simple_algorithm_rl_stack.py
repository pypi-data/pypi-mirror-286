...
class RobotGR1SimpleAlgorithmRLStackControlModel(RobotGR1SimpleAlgorithmBasicControlModel):
    def __init__(
        self,
        dt: float = 0.01,
        decimation: int = 1,
    ...
    def init_actor(self):
        ...
        self.nn_actor_input_length = self.num_actor_obs * self.num_actor_obs_stack
    ...
    def load_actor(self):
        if self.nn_actor is None:
            if self.actor_model_file_path is None:
                model_file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "rl_stack.pt")
            else:
                model_file_path = self.actor_model_file_path

            try:
                self.nn_actor = torch.jit.load(model_file_path, map_location=torch.device("cpu"))
    ...
    def actor_obs(self):
        ...
        self.nn_actor_input = torch.cat((self.nn_actor_input[:, self.num_actor_obs :], obs_buf), dim=-1).float()
    ...
    def actor_output(self):
        self.target_joint_position_offset = self.nn_actor_output * self.nn_actor_output_scale
