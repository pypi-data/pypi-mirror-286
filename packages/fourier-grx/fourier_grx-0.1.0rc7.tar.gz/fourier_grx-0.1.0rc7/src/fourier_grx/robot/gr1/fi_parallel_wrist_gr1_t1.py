...
class ParallelWrist:
    ...
    def fk_nnfit(self, joint_position_1, joint_position_2):
        ...
        for k in range(2):
            ...
            for i in range(20):
                d += c_a[k + (i << 1)] * b[i]
            wrist_position[k] = ((xp1[k] + d) - -1.0) \
                                / (-0.54567132576597 * float(k) + 1.63702291013563) \
    ...
    def forward(
        ...
        # G_matrix
        G_matrix = np.array(
            [[0, 0, 0, np.cos(wrist_position_roll), 0, -np.sin(wrist_position_roll)],
             [0, 0, 0, 0, 1, 0]]
        ).T
    ...
    def inverse(
        ...
        # G_matrix
        G_matrix = np.array(
            [[0, 0, 0, np.cos(wrist_position_pitch), 0, -np.sin(wrist_position_pitch)],
             [0, 0, 0, 0, 1, 0]]
        ).T
...
if __name__ == "__main__":
    """
    testing demo for inverse solution
    """

    # leftwrist = ParallelWrist('left')

    # counter = 0
    # samplings = 500
    # period = 5 # unit: sec
    # t_samplings = np.arange(samplings) * 0.01

    # wrist_roll_spline = 10 * np.sin(2 * np.pi * t_samplings / period) # unit: deg
    ...
    #     wrist_pitch = 0
    #     wrist_roll = wrist_roll_spline[counter]

    #     motor1, motor2, _, _, _, _ = leftwrist.inverse(wrist_roll, wrist_pitch) # 0.1 ms

    #     print(f'motor1: {motor1}')
    #     print(f'motor2: {motor2}')
    ...
    #     print(f"Elapsed time with high precision: {elapsed_time} seconds")
