...
class Visualizer_s(Visualizer):
    ...
    # def group_plot_bar(self, col_group: list = ["display type", "size"], col_plot: str = "price",
    #                    ylabel_mark: str = "%", figsize=(10, 6), save_plot_name=None):
    #
    #     df = self.dc.get_price_df()
    #     col_group_str = '&'.join(col_group)
    #
    #     grouped_mean = df.groupby(col_group)[col_plot].mean().sort_values(ascending=False)
    #     grouped_max = df.groupby(col_group)[col_plot].max().sort_values(ascending=False)
    #     grouped_min = df.groupby(col_group)[col_plot].min().sort_values(ascending=False)
    #     plt.figure(figsize=figsize)
    #
    #     grouped_mean.plot(kind="bar")
    #     grouped_max.plot(kind="bar")
    #     grouped_min.plot(kind="bar")
    #     plt.ylabel(f"{col_plot}({ylabel_mark})")
    #     sns.despine()
    #
    #     if save_plot_name is None:
