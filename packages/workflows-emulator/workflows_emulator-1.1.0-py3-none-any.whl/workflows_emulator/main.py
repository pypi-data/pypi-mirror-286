...
WORKFLOW_SCHEMA = load_config('workflows_emulator/workflow-schema.json')
...
def execute_workflow(config: dict, params: dict) -> any:
    main_config = config.pop('main')
    # we need the whole config inside the prams to run try/retry "predicate"
...
def execute_subworkflow(workflow: dict, params: dict, context: dict = {}) -> any:
    ...
    logger.info(f"Running subworkflow:\n{' '*8}context -> "
                f"{json.dumps(context)}\n{' '*8}params -> {json.dumps(solved_params)}")
    ...
    for retry_num in range(retry_config['max_retries'] + 1):
        try:
            ...
            # we run out of time but the issue was not fixed
                ...
                logger.error(f"Retry -> timeout. Max delay {max_delay} "
                             f"reached.")
                raise err
            ...
            if predicate is not None:
                ...
                # if the handling was successful it returned True
                if isinstance(run_result, bool) and run_result:
                    break
    else:
        # if retries run out, but not the max_delay and the predicate didn't
        # fix it
        logger.info(f"Retry: retries run out")
        raise raised_err
    return context, None, None
...
def execute_step_call_connector(rendered_step: dict) -> dict:
    ...
    if not os.path.exists(service_discovery_file_path):
        raise ValueError(f"Service {service_name} with version {version} discovery file not found.")

    service_discovery_config = load_config(service_discovery_file_path)
    ...
    service_path = f'{service_name}/{version}/'
