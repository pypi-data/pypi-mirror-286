...
class MessageHandler():
    ...
    def _parseMessage(self, IrcMessage: str) -> None:
    ...
    def _parseMessageEmotes(self) -> None:
    ...
    def _populateMessageChannel(self):
        self.message.channel = self.message.params[0] if len(self.message.params) > 0 else None  
    def _populateMessageText(self):
        self.message.text = self.message.params[1] if len(self.message.params) > 1 else None

    def _populateMessageId(self):
        self.message.id = self.message.tags.get("id")

    def _populateMessageMsgId(self):
        self.message.msgId = self.message.tags.get("msg-id")

    def _populateMessageUsername(self):
        self.message.username = self.message.tags.get("display-name")
