...
class TCI:
    ...
    def __init__(self, settings=TCISettings()):
        ...
        self._user: str = settings.user or ""
        self._password: str = settings.password or ""
        self._caprequest: str = settings.caprequest or ""
        self._ircClient: IrcController = self._getIrcClient(settings.server, settings.port, ssl=settings.SSL)
        ...
        self._COMMANDS: COMMANDS = MessageHandler.COMMANDS
    ...
    def event(self) -> EventHandler:
        """ returns event handeler pointer"""
        return self._event
    ...
    def start(self)->None:
        ...
        """
        self._run = True
        self.task = self._loop.create_task(self._runClient())                

    def disconnect(self):
        """
        disconnect _summary_
    ...
    def _getIrcClient(self, url: str, port: int, ssl) -> IrcController:
        client = IrcController(url, port, SSL=ssl or False)
        client.onDisconnected = self.disconnect
        return client
    
    async def _connect(self):
        if not self._ircClient.isConnected():
            await self._ircClient.connect()
    ...
    async def _runClient(self)->None:
        ...
        while self._run:
            try:
                data = await self._ircClient.receive()
                if data:
    ...
    async def _sendMsgQ(self)->None:
        ...
        while not self._sendQ.empty() and not self._sendTokens.isEmpty:       
          try:
              self._sendTokens.use()
              data = self._sendQ.get_nowait()
              await self._ircClient.send(data)
