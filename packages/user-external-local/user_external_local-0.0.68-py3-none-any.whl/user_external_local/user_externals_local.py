...
class UserExternalsLocal(GenericCRUD, metaclass = MetaLogger, object=object_init):
    ...
    @staticmethod
    def get_recipient_user_dict(system_id:int, recipient: Recipient) -> dict:
        ...
        if recipient.__user_id:
            custom_condition += f" AND user_id = {recipient.__user_id}"
        if recipient.__profile_id:
            custom_condition += f" AND profile_id = {recipient.__profile_id}"
        if recipient.__person_id:
            custom_condition += f" AND person_id = {recipient.__person_id}"
        if recipient.__telephone_number:
            custom_condition += f" AND `phone.full_number_normalized` = {recipient.__telephone_number}"
