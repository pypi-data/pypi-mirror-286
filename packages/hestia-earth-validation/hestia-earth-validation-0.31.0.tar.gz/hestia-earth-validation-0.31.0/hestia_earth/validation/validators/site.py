...
def validate_pond_measurements(site: dict):
    site_type = SiteSiteType.POND.value
    measurements = site.get('measurements', [])
    term_ids = ['salineWater', 'freshWater', 'brackishWater', 'waterSalinity']
    has_measurement = any([find_term_match(measurements, term_id) for term_id in term_ids])
    return site.get('siteType') != site_type or has_measurement or {
        'level': 'error',
...
def validate_site(site: dict, node_map: dict = {}):
    ...
    return [
        ...
        validate_pond_measurements(site)
    ] + flatten(
        ([
