...
def test_validate_site_valid(*args):
    with open(f"{fixtures_folder}/valid.json") as f:
        node = json.load(f)
    assert validate_site(node) == [True] * 36
...
def test_validate_pond_measurements_valid():
    # site is not pond => valid
    node = {'siteType': SiteSiteType.CROPLAND.value}
    assert validate_pond_measurements(node) is True
...
def test_validate_pond_measurements_invalid():
    with open(f"{fixtures_folder}/pond-measurements/invalid.json") as f:
        node = json.load(f)
    assert validate_pond_measurements(node) == {
        'level': 'error',
