...
from labelsmith.utils.metrics import ShyftMetrics
import appdirs
import configparser
...
class ShyftGUI:
        ...
        for id in sorted_keys:
            ...
            self.tree.insert(
                ...
                values=(
                    ...
                    shift.get("Tasks completed", "N/A"),  # Add this new column
            ...
            try:
                ...
                try:
                    total_tasks_completed = sum(
                        int(shift.get("Tasks completed", 0)) for shift in self.data["data"].values()
                    )
                    logger.debug(f"Total tasks completed: {total_tasks_completed}")
                except ValueError as e:
                    logger.error(f"Error calculating total tasks completed: {str(e)}")
                ...
                totals_tree.insert("", "end", values=("Total Tasks Completed", total_tasks_completed))
    ...
    def submit_action(self):
        try:
            ...
            try:
                ...
                tasks_completed = int(new_data.get("Tasks completed", 0))  # Use get() with a default of 0
            except ValueError:
                messagebox.showerror(
                    "Error",
                    "Invalid input for 'Hourly rate' or 'Tasks completed'. Please enter numerical values.",
                )
                return

            gross_pay = hourly_rate * duration_hrs
            new_data["Gross pay"] = "{:.2f}".format(gross_pay)
            new_data["Tasks completed"] = str(tasks_completed)  # Ensure it's stored as a string
            ...
            messagebox.showinfo("Success", f"Shift logged successfully. {tasks_completed} tasks completed.")
            self.root.focus_force()
            logger.info(f"Shift logged successfully. {tasks_completed} tasks completed.")
        except ValueError as e:
    ...
    def manual_entry(self, event=None):
        ...
        self.fields = [
            ...
            "Out (hh:mm)",
            "Hourly rate",
            "Tasks completed",  # Add this new field
        ]
        ...
        for field in self.fields:
            ...
            # Set a default value for "Tasks completed"
            if field == "Tasks completed":
                entry.insert(0, "0")  # Default to 0 if not specified
    ...
    def attempt_task(self, shared_data):
        ...
        if hasattr(justification_window, 'result') and justification_window.result is not None:
            ...
            # Add task duration to the result
    ...
    def log_shift(self):
        if self.timer_window and tk.Toplevel.winfo_exists(self.timer_window.root):
            ...
            tasks_completed = len(self.collected_data) if self.collected_data else 0

            # Create a dictionary to store task durations
            task_durations = {}
            for i, task_data in enumerate(self.collected_data, start=1):
                task_durations[str(i)] = {"Duration (hh:mm)": task_data['Task Duration']}
            ...
            with lock:
                self.data["data"][formatted_id] = {
                    ...
                    "Model ID": self.collected_data[0]["Model ID"] if self.collected_data else "",
                    "Project ID": self.collected_data[0]["Project ID"] if self.collected_data else "",
                    "In (hh:mm)": (datetime.now() - elapsed_time).strftime("%H:%M"),
                    "Out (hh:mm)": datetime.now().strftime("%H:%M"),
                    "Duration (hrs)": f"{duration_hrs:.2f}",
                    "Hourly rate": f"{self.collected_data[0]['Hourly Rate of Pay']:.2f}" if self.collected_data else "0.00",
                    "Gross pay": f"{gross_pay:.2f}",
                    "Tasks completed": tasks_completed,
                    "Task durations": task_durations  # Add the task durations to the shift data
                }
                self.save_data()
    ...
    def setup_file_menu(self):
        ...
        self.file_menu.add_command(label="Plot productivity", command=self.plot_productivity_default)
        self.file_menu.add_command(label="Plot productivity...", command=self.plot_productivity_custom)
        # Add other file menu items here...
        self.menu_bar.add_cascade(label="File", menu=self.file_menu)

    def plot_productivity_default(self):
        self.generate_plot(metric='time')

    def plot_productivity_custom(self):
        config = self.show_plot_config_dialog()
        if config:
            self.generate_plot(**config)

    def show_plot_config_dialog(self):
        dialog = tk.Toplevel(self.root)
        dialog.title("Plot Configuration")
        dialog.geometry("350x250")  # Increased size to fit all elements comfortably
        dialog.resizable(False, False)

        # Create a main frame to hold all widgets
        main_frame = tk.Frame(dialog, padx=10, pady=5)
        main_frame.pack(fill=tk.BOTH, expand=True)
        ...
        # Metric Selection
        tk.Label(main_frame, text="Metric:").grid(row=0, column=0, padx=(0,5), pady=5, sticky="w")
        metric_var = tk.StringVar(value="tasks")
        tk.Radiobutton(main_frame, text="Tasks Completed", variable=metric_var, value="tasks").grid(row=0, column=1, sticky="w")
        tk.Radiobutton(main_frame, text="Time per Task", variable=metric_var, value="time").grid(row=1, column=1, sticky="w")

        # Window Size
        tk.Label(main_frame, text="Window (number of days):").grid(row=2, column=0, padx=(0,5), pady=5, sticky="w")
        window_size = tk.Entry(main_frame)
        window_size.insert(0, "7")
        window_size.grid(row=2, column=1, padx=(0,5), pady=5, sticky="ew")

        # Start Date
        tk.Label(main_frame, text="Start Date (YYYY-MM-DD):").grid(row=3, column=0, padx=(0,5), pady=5, sticky="w")
        start_date = tk.Entry(main_frame)
        start_date.grid(row=3, column=1, padx=(0,5), pady=5, sticky="ew")

        # End Date
        tk.Label(main_frame, text="End Date (YYYY-MM-DD):").grid(row=4, column=0, padx=(0,5), pady=5, sticky="w")
        end_date = tk.Entry(main_frame)
        end_date.grid(row=4, column=1, padx=(0,5), pady=5, sticky="ew")

        # Save Filename
        tk.Label(main_frame, text="Save Filename:").grid(row=5, column=0, padx=(0,5), pady=5, sticky="w")
        save_filename = tk.Entry(main_frame)
        save_filename.insert(0, "productivity.html")
        save_filename.grid(row=5, column=1, padx=(0,5), pady=5, sticky="ew")

        config = {}

        def on_submit():
            config['metric'] = metric_var.get()
            config['window'] = int(window_size.get())
            config['start_date'] = start_date.get() if start_date.get() else None
            config['end_date'] = end_date.get() if end_date.get() else None
            config['save_path'] = save_filename.get()
            dialog.destroy()

        # Create a frame for the submit button to center it
    ...
    def generate_plot(self, metric='tasks', window=7, start_date=None, end_date=None, save_path=f"{app_data_dir}/productivity.html"):
        try:
            metrics = ShyftMetrics(self.data_file_path)
            plot_path = metrics.plot_interactive_trend(
                save_path=save_path,
                window=window,
            ...
            messagebox.showinfo("Plot Created", f"Interactive plot saved and opened: {plot_path}")
        except Exception as e:
