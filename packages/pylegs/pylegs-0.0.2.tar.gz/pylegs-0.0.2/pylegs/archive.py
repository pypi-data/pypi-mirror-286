...
from pylegs.io import PathLike, parallel_function_executor, read_table
    ...
    if compute_mag:
      for band in compute_mag:
        ...
        if f'flux_{b}' in table.colnames:
          ...
          table[f'mag_{b}'] = 22.5 - 2.5 * np.log10(flux, where=flux > 0)
...
class BaseArchiveMatcher:
  ...
  def _get_pix_bounding_box(self, margin: float = 0.0) -> Tuple[Tuple[float, float], Tuple[float, float]]:
    ...
    coord = SkyCoord(ra=[ra_lim[0], ra_lim[1]], dec=list(dec_lim), unit=u.deg)
  ...
  def match(
    ...
    if concat:
      concat.write(output_path, overwrite=True)
    return concat
  ...
  def plot(
    ...
    # plot brick names
    if labels:
      x_arr, y_arr = self.wcs.world_to_pixel(SkyCoord(ra=df.ra.values, dec=df.dec.values, unit=u.deg))
    ...
    if xticks is not None:
      ax.coords[0].set_ticks(xticks * u.deg)
    if yticks is not None:
      ax.coords[1].set_ticks(yticks * u.deg)
    
    # configure axis units
    ax.coords[0].set_format_unit(ra_unit)
    ax.coords[1].set_format_unit(dec_unit)
...
class PolygonMatcher(BaseArchiveMatcher):
  ...
  def wcs(self) -> WCS:
    if self._wcs is None:
      ...
      self._wcs.wcs.crpix = [ra_min + abs(ra_max-ra_min)/2, dec_min + abs(dec_max-dec_min)/2]
      self._wcs.wcs.cdelt = [-1, 1]
      self._wcs.wcs.crval = [ra_min + abs(ra_max-ra_min)/2, dec_min + abs(dec_max-dec_min)/2] 
  ...
  def intersection_df(self):
    if self._intersection_df is None:
      ...
      for i, row in df.iterrows():
        B = np.array(radec_to_vector([row.ra1, row.ra2, row.ra2, row.ra1], [row.dec1, row.dec1, row.dec2, row.dec2])).T
        inside = radec_to_vector(row.ra, row.dec)
        brick_pol = SingleSphericalPolygon(B, inside)
  ...
  def _compute_brick_mask(self, coords, ra1, ra2, dec1, dec2):
    v = np.array(radec_to_vector([ra1, ra2, ra2, ra1], [dec1, dec1, dec2, dec2])).T
    inside = radec_to_vector(ra1 + abs(ra2-ra1)/2, dec1 + abs(dec2-dec1)/2)
    brick_pol = SingleSphericalPolygon(v, inside)
  ...
  def _plot_search_shape(self, ax = None, transform = None):
    ...
    poly = Polygon(points, edgecolor='tab:red', linewidth=2, alpha=0.6, transform=transform)
    ax.add_patch(poly)
  
  

class RectangleMatcher(PolygonMatcher):
  def __init__(self, ra1, ra2, dec1, dec2):
    coords = SkyCoord(ra=[ra1, ra2, ra2, ra1], dec=[dec1, dec1, dec2, dec2], unit=u.deg)
...
class RadialMatcher(BaseArchiveMatcher):
  ...
  def wcs(self) -> WCS:
    if self._wcs is None:
      ...
      self._wcs = WCS(naxis=2)
      # self._wcs.wcs.crpix = [ra_min + abs(ra_max-ra_min)/2, dec_min + abs(dec_max-dec_min)/2]
      # self._wcs.wcs.crpix = [self.ra, self.dec]
      self._wcs.wcs.cdelt = [-1, 1]
      # self._wcs.wcs.crval = [ra_min + abs(ra_max-ra_min)/2, dec_min + abs(dec_max-dec_min)/2] 
  ...
  def intersection_df(self):
    if self._intersection_df is None:
      ...
      center = SkyCoord(ra=self.ra, dec=self.dec, unit=u.deg, frame='icrs')
      bricks_centers = SkyCoord(ra=ra, dec=dec, unit=u.deg, frame='icrs')
      mask = center.separation(bricks_centers) < (self.radius*u.deg + 1.5*u.deg)
      ...
      for i, p in enumerate(points):
        ...
        point_intersection_mask[:, i] = center.separation(s) < self.radius * u.deg
        # point_intersection_mask[:, i] = heaversine(self.ra, self.dec, s.ra.deg, s.dec.deg)*u.deg < self.radius * u.deg
        # point_intersection_mask[:, i] = f2(self.ra, self.dec, s.ra.deg, s.dec.deg)*u.deg < self.radius*u.deg
  ...
  def _compute_brick_mask(self, coords: SkyCoord, **kwargs):
    return coords.separation(SkyCoord(ra=self.ra, dec=self.dec, unit=u.deg)) < self.radius*u.deg
...
class CrossMatcher(BaseArchiveMatcher):
  def __init__(
    ...
    if ra is not None and dec is not None:
      self.coords = SkyCoord(ra=ra, dec=dec, unit=u.deg)
    else:
      self.coords = coords
    self._ra_lim = None
    self._dec_lim = None
...
if __name__ == '__main__':
  RadialMatcher(120.2939, 40.7714, 12).plot(labels=False)
