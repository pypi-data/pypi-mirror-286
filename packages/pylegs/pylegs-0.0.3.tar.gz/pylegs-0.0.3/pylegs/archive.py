...
class BaseArchiveMatcher:
  ...
  def plot(
    ...
    xpad, ypad = _sanitize_quantity((xpad, ypad), u.deg, convert=True)
    ...
    xlim = _sanitize_quantity(xlim, u.deg, convert=True)
    ylim = _sanitize_quantity(ylim, u.deg, convert=True)
    ...
    if xticks is not None:
      ax.coords[0].set_ticks(_sanitize_quantity(xticks, u.deg, convert=True))
    if yticks is not None:
      ax.coords[1].set_ticks(_sanitize_quantity(yticks, u.deg, convert=True))
...
class PolygonMatcher(BaseArchiveMatcher):
  def __init__(
    ...
    if isinstance(coords, SkyCoord):
      ...
      ra, dec = _sanitize_quantity((ra, dec), u.deg)
...
class RectangleMatcher(PolygonMatcher):
  def __init__(
    ...
    ra1, ra2, dec1, dec2 = _sanitize_quantity((ra1, ra2, dec1, dec2), u.deg)
...
class RadialMatcher(BaseArchiveMatcher):
  def __init__(
    ...
    self.radius = _sanitize_quantity(radius, u.deg)
    self._ra_lim = None
    self._dec_lim = None
...
class CrossMatcher(BaseArchiveMatcher):
  def __init__(
    ...
    if isinstance(coords, SkyCoord):
      ...
      raise ValueError('Either `coords` or `ra` and `dec` must be passed')
    self.radius = _sanitize_quantity(radius)
    self._ra_lim = None
    self._dec_lim = None
  ...
  def from_table(
    ...
    ra_col, dec_col = guess_coords_columns(table, ra_col, dec_col)
