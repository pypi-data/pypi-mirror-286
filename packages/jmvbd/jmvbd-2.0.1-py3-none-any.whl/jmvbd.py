...
db_path = os.path.join(appdata_path, 'jmvbd', 'processed_folders.db')
api_key_path = os.path.join(appdata_path, 'jmvbd', 'apikey.txt')
...
def get_movie_id(title):
    ...
    if response.status_code == 200:
        ...
        logger.error(f"Failed to fetch data from TMDB: {response.status_code} {response.reason}")
    return None
...
def get_trailer_url(movie_id):
    ...
    if response.status_code == 200:
        ...
        if 'results' in data and data['results']:
            ...
            logger.info(f"No trailer found for movie ID: {movie_id}")
    else:
        logger.error(f"Failed to fetch video data from TMDB: {response.status_code} {response.reason}")
    return None
