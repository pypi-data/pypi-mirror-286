...
class CameraMotionDetectionDelegate(metaclass=ABCMeta):
    @abstractmethod
    async def detectVideo(self, video_path: str) -> list[Detection]: ...


CameraMotionDetectionDelegateMethodNames = Literal["detectVideo"]


class CameraObjectDetectionDelegate(metaclass=ABCMeta):
    @abstractmethod
    async def detectImage(self, image: bytes, metadata: "ImageMetadata") -> list[Detection]: ...
