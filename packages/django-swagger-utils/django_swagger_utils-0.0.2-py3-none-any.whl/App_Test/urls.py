...
with open(os.path.join('App_Test/api_specs', 'api_spec.json'), 'r') as file:
    content = file.read()
    ...
    for url_path, operation_details in app_urls.items():
        for method, details in operation_details.items():
            if 'operationId' in details:
                ...
                full_module_path = f'App_Test.views.{operation_id}.{operation_id}'
                try:
                    module = importlib.import_module(full_module_path)
                    view_func = getattr(module, operation_id, None)
                    url_patterns.append(re_path(f'api/App_Test{url_path}', view_func, name=operation_id))
