...
class ProducerConsumer:
    ...
    def __init__(self, num_consumers: int, engine: ChatEngine, json: bool, logger: str) -> None:
        """
        ...
        if self._json:
            self._sampling_params = self._chat_engine.json_output()
        else:
            self._sampling_params = self._chat_engine.create_sampling_params()
        ...
        while True:
            ...
            try:
                ...
                _stream = await self._chat_engine.engine.add_request(
                    uuid.uuid4().hex, _prompt, self._sampling_params
                )
