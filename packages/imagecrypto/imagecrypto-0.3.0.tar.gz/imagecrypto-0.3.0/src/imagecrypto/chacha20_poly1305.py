...
def encrypt_image(image_path, ecdhe_private_key, ecdhe_public_key):
    ...
    nonce = os.urandom(12)
    encrypted_data = chacha.encrypt(nonce, flattened.tobytes(), None)
...
def get_times(image_path):
    ecdhe_private_key = generate_ecdhe_key()
    ecdhe_public_key = ecdhe_private_key.public_key()
    encrypted_image, encrypted_data, nonce, shape, encryption_time = encrypt_image(image_path, ecdhe_private_key, ecdhe_public_key)
...
def display_images(original_path, encrypted_path, decrypted_path):
    ...
    fig.suptitle('ChaCha20-Poly1305 Encryption/Decryption', fontsize=16)
