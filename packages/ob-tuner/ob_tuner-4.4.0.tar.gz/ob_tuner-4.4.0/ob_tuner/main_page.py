...
with gr.Blocks(analytics_enabled=False, theme='freddyaboulton/dracula_revamped') as main_block:
        ...
        with gr.Row() as chat_row:
            ...
            with gr.Column(scale=1) as context_container:
                ...
                with gr.Row():
                    ...
                    voice_enabled = gr.Checkbox(label="Speak", show_label=True, scale=1)
    ...
    # Chat Button
    chat_button.click(fn=chat, inputs=[msg, chatbot, profile_name, session_state, client_id, context], outputs=[msg, chatbot, session_state, context]).then(alert_on_actions, inputs=[session_state], outputs=[session_state, events])
    ...
    # chat_button.click(alert_on_actions, inputs=[session_state], outputs=[session_state, events])

    msg.submit(chat, [msg, chatbot, profile_name, session_state, client_id, context], [msg, chatbot, session_state, context]).then(alert_on_actions, inputs=[session_state], outputs=[session_state, events])
    ...
    # msg.submit(alert_on_actions, inputs=[session_state], outputs=[session_state, events])
    ...
    reset_agent.click(alert_on_actions, inputs=[session_state], outputs=[session_state, events])
