...
def speak(voice_enabled, text, session_state):
    ...
    myobj = gTTS(text=thing_to_say, lang=language, slow=False)

    # delete previous file if exists
    if os.path.exists(f"/tmp/{session_id}.mp3"):
        os.remove(f"/tmp/{session_id}.mp3")
    try:
        myobj.save(f"/tmp/{session_id}.mp3")
        file = f"/tmp/{session_id}.mp3"
        logger.info(f"Saving audio file {file}")
    except Exception as e:
        myobj.save(f"{session_id}.mp3")
        file = f"{session_id}.mp3"
        logger.warning(f"Saving audio file {file}")

    if voice_enabled:
        show_warning = not session_state.get("audio_warning_triggered")
        audio = gr.Audio(file, autoplay=True, scale=1)
        if show_warning:
