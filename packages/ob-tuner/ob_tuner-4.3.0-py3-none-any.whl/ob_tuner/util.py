...
def react_to_message(chatbot, session_state, profile_name, client_id, like_data: gr.LikeData):
    ...
    try:
        ...
        logger.info(f"Dumping file on local filesystem...")
        gr.Warning(f"Feedback saved to local filesystem... something went wrong... Thank you!")

        with open(f"reactions/{profile_name}/{client_id}/{session_id}.json", "w") as file:
            json.dump(item, file)
        with open(f"reactions/{profile_name}/{client_id}/{session_id}.json", "r") as file:
