...
class FeatureView(WrappedOrmObject[FeatureViewID, orm.FeatureView]):
    ...
    def output_edge_tables(self) -> list[Table]:
        ...
        for path in paths_between_outputs:
            ...
            for rel in path_itr:
                if not table.op_graph.schema.has_column(rel.new_column_name):
                ...
                rhs_table = rel.to_feature_view_source.table
                if not rhs_table.schema.has_column(rel.new_column_name):
                    rhs_table = rhs_table.rename_columns(
