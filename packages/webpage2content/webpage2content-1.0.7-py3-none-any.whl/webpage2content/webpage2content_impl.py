...
def _call_gpt(
    ...
    while True:
        if logger:
            logger.debug("webpage2content._call_gpt calling chat completion")
        ...
        if logger:
            logger.debug(
                f"webpage2content._call_gpt got answer of length {len(answer)}, appending to full answer currently at length {len(answer_full)}"
            )
