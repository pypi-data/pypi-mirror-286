...
def get_df_schema(df: pd.DataFrame) -> List[Field]:
    """
    ...
    for field in pa.Schema.from_pandas(df):
        if pa.types.is_string(field.type):
            res.append(Field(field.name, String))
        elif pa.types.is_integer(field.type):
            res.append(Field(field.name, Int64))
        elif pa.types.is_floating(field.type):
            res.append(Field(field.name, Float64))
        elif pa.types.is_boolean(field.type):
            res.append(Field(field.name, Bool))
        elif pa.types.is_timestamp(field.type):
            res.append(Field(field.name, Timestamp))
        else:
            raise ValueError(f"Unsupported type {field.type} for {field.name}")
    return res
