    ...
    if os.path.isfile(".env"):
        ...
        if concierge_root:
            ...
            if os.path.exists(os.path.join(files(), "..", "volumes")):
                ...
                print("This is probably the result of an issue caused by a previous version which was ignoring the user's settings and incorrectly creating the Concierge volumes inside the package install directory.")
                proceed = False
                while not proceed:
            ...
            if new_version != existing_version:
                ...
                start_index = find_index(
                    scripts,
                    lambda s: (not existing_version or Version(s["version"]) > Version(existing_version)) # if there's no existing version we start from zero
...
def docker_compose_helper(environment, compute_method, dir, rebuild=False):
    ...
    filename += ".yml"
    if dir != os.getcwd():
        shutil.copytree(os.path.join(dir, "docker_compose_dependencies"), os.path.join(os.getcwd(), "docker_compose_dependencies"), dirs_exist_ok=True)
        shutil.copyfile(os.path.join(dir, filename), os.path.join(os.getcwd(), filename))
