...
class Notifier:
    def __init__(self):
        ...
        self.tokens_file = os.path.join('.', 'tokens.txt')
        self.settings_file = os.path.join('.', 'notification_setting.txt')
    ...
    def debug_settings(self):
        ...
        token = self.tokens.get(self.user, 'トークンが見つかりません')
        masked_token = f"{token[:4]}{'*' * (len(token) - 8)}{token[-4:]}"
        print(f"ユーザーのトークン: {masked_token}")
