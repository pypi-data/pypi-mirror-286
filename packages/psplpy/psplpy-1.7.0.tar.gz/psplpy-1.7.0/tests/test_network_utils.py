...
def test_mp_http():
    ...
    s = TestServer()
    threading.Thread(target=s.run_server).start()
    time.sleep(1)
    c = MpHttpClient()
    result = c.batch([{'content': 1}, {'content': 2}, {'content': 3}])
    assert result == [{'result': {'content': 1, '_process_id': 0}, 'num': 1},
                      {'result': {'content': 2, '_process_id': 1}, 'num': 1},
                      {'result': {'content': 3, '_process_id': 2}, 'num': 1}], result
    s.close_server()
