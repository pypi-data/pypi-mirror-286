...
def surface_reflectance(refl_toa_image, meteo_coll):
    ...
    # CGM - The NED asset is deprecated, testing out NASADEM asset
    ...
    # Note, the Band 1 coefficients were made up by CGM and are not based on any real analysis
    # Coeff        Band1      Band2      Band3      Band4      Band5     Band6      Band7
    c1 = ee.Image([0.847,     0.987,     2.148,     0.942,     0.248,    0.260,     0.315])
    c2 = ee.Image([-0.001343, -0.000727, -0.000199, -0.000261, -0.00041, -0.001084, -0.000975])
    c3 = ee.Image([0.000610,  0.000037,  0.000058,  0.000406,  0.000563, 0.000675,  0.004012])
    c4 = ee.Image([0.0869,    0.0869,    0.0464,    0.0928,    0.2256,   0.0632,    0.0116])
    c5 = ee.Image([0.0609,    0.0788,    -1.0962,   0.1125,    0.7991,   0.7549,    0.6906])
    cb = ee.Image([0.720,     0.640,     0.310,     0.286,     0.189,    0.274,     -0.186])
