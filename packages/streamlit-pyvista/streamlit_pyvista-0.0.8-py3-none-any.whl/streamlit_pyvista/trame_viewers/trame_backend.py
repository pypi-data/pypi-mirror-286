...
class TrameBackend(ABC):
    ...
    def __init__(self, plotter: Optional[pv.Plotter] = None, server: Optional[Server] = None,
        ...
        threading.Timer(5 * ONE_MINUTE, self.stop).start()
    ...
    def _update_viewer_for_new_meshes(self):
        ...
        self._replace_mesh_displayed(self.mesh_array[0])
        ...
        self.state.options_warp = self.mesh_array[0].point_data.keys().copy()
    ...
    async def stop(self):
        ...
        `shutdown_event` is set. Otherwise, a delayed call to `stop` is scheduled using `threading.Timer`
        and `asyncio.get_running_loop().call_soon(asyncio.create_task, self.stop())`.
        """
        if self.client_counter == 0:
            ...
            threading.Timer(2 * ONE_MINUTE,
                            lambda: asyncio.get_running_loop().call_soon(asyncio.create_task, self.stop())).start()
