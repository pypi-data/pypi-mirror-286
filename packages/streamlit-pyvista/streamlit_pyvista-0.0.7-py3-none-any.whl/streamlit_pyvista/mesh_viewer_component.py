...
class MeshViewerComponent:
    ...
    def __init__(self, mesh_path: str | list[str] = None,
                 server_manager_url: str = "http://127.0.0.1:8080",
    ...
    def _run_server_manager(self):
        ...
        try:
            subprocess.run(["python3", self.manager.get_launch_path()],
                           ...
                           # Comment the line above to debug this subprocess call
                           check=True)
        except subprocess.CalledProcessError as e:
            root_logger.debug(f"Tried to launch the Server Manager but got an unexpected error:\n{e}")

    def set_mesh(self, meshes: list[str] | str = None):
        """ 
    ...
    def show(self):
        ...
        if host and not is_localhost(host) and is_localhost(self.server_url):
            root_logger.debug(
                f"The host that the iframe should have is {host} and it actually has {self.endpoints[ServerMessageInterface.Keys.Host]}")
