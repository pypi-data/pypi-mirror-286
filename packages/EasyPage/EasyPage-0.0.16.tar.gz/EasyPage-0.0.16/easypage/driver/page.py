...
class PageDriver(Driver):
    ...
    def query_selector(
        ...
        ele = Element(node_id=new_node['nodeId'], page=self)
        if not ele or ele.tag_name is None:
            if raise_err:
                raise errors.ElementNotFound(selector)
            return
    ...
    def query_selector_all(
        ...
        for nn in new_nodes['nodeIds']:
            ele = Element(node_id=nn, page=self)
            if not ele or ele.tag_name is None:
                continue
            elements.append(ele)
        return elements
    ...
    def add_script_to_evaluate_on_new_document(
        ...
        :return:
        """
        return self.cdp.page.add_script_to_evaluate_on_new_document(
            source=source,
    ...
    def iframe_get(self, ele: "Element") -> Union["Element", None]:
        """
        普通查询到的 iframe 到 真正的 iframe 的转换

        :param ele:
        :return:
        """
        # 获取 frameId
        status, frame_detail = self.cdp.dom.describe_node(nodeId=ele.node_id)
        if not frame_detail or not status:
            return
        frame_id = frame_detail['node']['frameId']
    ...
    def wait_for_selector(
        ...
        start_time = time.time()
        while time.time() - start_time < timeout:
            ele = self.query_selector(selector=selector, node_id=node_id, wait=False)
    ...
    def wait_for_selectors(
        ...
        while time.time() - start_time < timeout:
            for s in selectors:
                ele = self.query_selector(selector=s, node_id=node_id, wait=False)
...
class Element(ActionChain):
    ...
    def shadow_root(self) -> Union['Element', None]:
        """
        返回 shadow-root 暂时不区分特性
        shadow-root 是一个与 Web 组件,与主文档的 DOM 树是隔离
        ...
        ele = Element(node_id=node['node']['shadowRoots'][0]['nodeId'], page=page)
        if not ele or ele.tag_name is None:
            return
