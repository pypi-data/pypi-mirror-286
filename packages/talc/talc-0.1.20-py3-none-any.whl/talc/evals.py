...
from talc.synthetic import Document, QuestionGenerationConfig, SimpleQuestionGeneratorArgs, HallucinationInducingQuestionGeneratorArgs
...
class KnowledgeBaseUploadArgs(BaseModel):
    """Arguments for uploading a knowledge base."""
...
class EvalsClient:
    ...
    def upload_knowledge_base(
        self, kb: KnowledgeBase | list[Document], name: str
    ) -> KnowledgeBase:
        """Upload a knowledge base to the server."""
        if isinstance(kb, KnowledgeBase):
            data = [document.model_dump() for document in kb.documents]
        elif isinstance(kb, list) and isinstance(kb[0], Document):
            data = [document.model_dump() for document in kb]
        else:
            raise ValueError(
                "Invalid knowledge base format. Only KnowledgeBase or list[Document] is supported."
            )
