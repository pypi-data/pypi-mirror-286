...
class IRCApp(SimpleIRCClient):
    ...
    async def _intercept_part(self, message):
        ...
        if message.startswith("/part"):
            ...
            #self.app.channel_list = [x.channel_name for x in await self.app.channel_ops.get_channels()]
            self.app.notify(str(self.app.channel_list))
            return True
        return False 
    ...
    def on_join(self, connection, event):
        ...
        try:
            if sender not in self.user_list:
                self.app.channel_list.add_leaf(sender)
        except:
            pass
