...
then
    ...
    echo -e "${success_color}INFO:${reset_style} ${field_color}server_index:${reset_style} ${server_index}"
elif [[ -e "${argrelay_dir}/var/argrelay_client.server_index" ]]
then
    echo -e "${failure_color}ERROR:${reset_style} ${field_color}server_index:${reset_style} ${failure_message}# \`@/var/argrelay_client.server_index\` is not a file"
    exit 1
else
    echo -e "${warning_color}WARN:${reset_style} ${field_color}server_index:${reset_style} ${warning_message}# \`@/var/argrelay_client.server_index\` has not been created yet${reset_style}"
    server_index="0"
fi
...
echo -e "${success_color}INFO:${reset_style} ${field_color}server_url[${server_index}]:${reset_style} http://${server_host_name}:${server_port_number}"
