...
class FluoTomoScan:
    ...
    skip_angle_inds: Union[Sequence[int], NDArray, None] = None
    ...
    def detect_pixel_size(self):
        ...
        if len(h5_files) > 1:
            ...
            sample_name = h5_files[0].split("_XRF_")[0]
            entry_name = (
                "entry0000: "
                + sample_name
                + " - "
        ...
        with h5py.File(os.path.join(pixel_size_path, h5_files[0]), "r") as f:
            self.pixel_size = f[entry_name]["FLUO"]["pixelSize"][()] * 1e-6
