...
def get_product(url,categories_class_name):
    global counter
    ...
    try:
        ...
        response = requests.get(new_url, impersonate='chrome120')
        soup = BeautifulSoup(response.text, 'html.parser')
        product_dict['Main_category'], product_dict['Sub_category1'], product_dict['Sub_category2'] = get_categories(soup,categories_class_name)
    ...
    try:
        if response.status_code != 200:
            raise Exception('Error status code different than 200')
        try:
