...
from tortoise import Model, fields, connections
from tortoise.converters import encoders, escape_str
...
class DBModel(Model):
    ...
    def get_sheet_name(cls, tb_suffix: str = None):
        ...
        return f"{or_name}{tb_suffix}" if tb_suffix else or_name, or_name
    ...
    def check_val(val, field, model):
        ...
        v = column.default
        if (v is not None) and isinstance(v, (str, UUID)):
            return escape_str(v)
        return v
    ...
    def gen_insertup_sql(
            cls,
            darr: Union[list[dict], tuple[dict]],
        ...
        sql = F"INSERT INTO `{cls.sheet_name(suffix=suffix)}`({','.join(all_field)}) VALUES "
        v_list = []
        for item in darr:
