...
class AutoPrompt(PromptOp, ABC):
    ...
    def __init__(self, spg_type_names: List[SPGTypeName], **kwargs):
        super().__init__()
        self.spg_types = []
        project_id = kwargs.get("project_id", None)
        schema_session = SchemaClient(project_id=project_id).create_session()
        if hasattr(self, "spg_type_name") or not spg_type_names:
