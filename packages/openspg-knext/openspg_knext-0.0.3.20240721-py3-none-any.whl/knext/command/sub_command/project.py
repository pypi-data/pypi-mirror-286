...
from knext.common.template import copytree, render_template, remove_files_except
...
@click.option("--tmpl", help="Template of project.")
@click.option(
    "--prj_path",
    help="If set, project will be created according to config file of this path.",
)
def create_project(
    name: str, namespace: str, desc: Optional[str], tmpl: Optional[str], prj_path: Optional[str]
):
    """
    ...
    remove_files_except(project_dir.resolve()/"schema", tmpl_file, f"{namespace.lower()}.schema.tmpl")
    project = client.project_create_post(project_create_request=project_create_request)
