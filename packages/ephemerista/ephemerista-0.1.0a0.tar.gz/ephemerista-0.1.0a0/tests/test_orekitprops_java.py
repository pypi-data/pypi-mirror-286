...
from ephemerista.bodies import Planet, Satellite, Sun
...
@pytest.mark.parametrize("third_bodies", [[], [Sun(), Satellite(name="luna"), Planet(name="jupiter")]])
def test_comparison_java(maven_package, py_prop_class, enable_srp, enable_drag, grav_degree_order, third_bodies):  # noqa: ARG001
    py_prop = py_prop_class(
        enable_srp=enable_srp, enable_drag=enable_drag, grav_degree_order=grav_degree_order, third_bodies=third_bodies
    )
    ...
    traj_python = py_prop.propagate(state_init=state_init, time=time_list)
    cart_list_python = traj_python.cartesian_states

    tpv_icrf_init = cartesian_to_tpv(state_init.to_cartesian())
    stop_conds = []
    time_end_abs = time_to_abs_date(time_end)
    tpv_list = java_prop.propagate(tpv_icrf_init, time_j2000_list, time_end_abs, stop_conds)
