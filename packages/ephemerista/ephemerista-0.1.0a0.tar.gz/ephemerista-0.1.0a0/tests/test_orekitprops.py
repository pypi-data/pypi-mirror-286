...
from ephemerista.bodies import MinorBody, Planet, Satellite, Sun
...
def test_propagate_same_time(maven_package, orekit_prop):  # noqa: ARG001
    prop = orekit_prop()
    ...
    c0 = Cartesian.from_rv(time, r, v)
    c1 = prop.propagate(state_init=c0, time=time)
...
@pytest.mark.parametrize("orekit_prop", OREKIT_PROP_CLASSES)
def test_propagate_single_time(maven_package, orekit_prop):  # noqa: ARG001
    prop = orekit_prop()
    ...
    c0 = Cartesian.from_rv(time_start, r, v)
    c2 = prop.propagate(state_init=c0, time=time_end)
    delta_pos = c2.position - c0.position

    delta_t: TimeDelta = time_end - time_start
    assert np.linalg.norm(delta_pos) == approx(v_norm * delta_t.to_decimal_seconds(), abs=1e-2)  # 10 meters tolerance


@pytest.mark.parametrize("orekit_prop", OREKIT_PROP_CLASSES)
def test_propagate_single_time_and_back(maven_package, orekit_prop):  # noqa: ARG001
    prop = orekit_prop()
    time_start = Time.from_iso("TDB", "2016-05-30T12:00:00")
    time_end = Time.from_iso("TDB", "2016-05-30T12:00:01")
    ...
    c0 = Cartesian.from_rv(time_start, r, v)
    c2 = prop.propagate(state_init=c0, time=time_end)
    c3 = prop.propagate(state_init=c2, time=time_start)
...
@pytest.mark.parametrize("orekit_prop", OREKIT_PROP_CLASSES)
def test_propagate_multiple_times(maven_package, orekit_prop):  # noqa: ARG001
    prop = orekit_prop()
    time_start = Time.from_iso("TDB", "2016-05-30T12:00:00")
    time_list = [
        time_start,
    ...
    c0 = Cartesian.from_rv(time_start, r, v)
    traj = prop.propagate(state_init=c0, time=time_list)
    ...
    for i in range(0, len(time_list)):
        ...
        delta_pos = c2.position - c0.position
        delta_t: TimeDelta = time_tai - time_start
...
@pytest.mark.parametrize("orekit_prop", OREKIT_PROP_CLASSES)
def test_no_degree_order(maven_package, orekit_prop):  # noqa: ARG001
    prop = orekit_prop(grav_degree_order=None)
...
@pytest.mark.parametrize("orekit_prop", OREKIT_PROP_CLASSES)
def test_all_valid_third_bodies(maven_package, orekit_prop):  # noqa: ARG001
    third_bodies = [
        Sun(),
        Satellite(name="luna"),
        Planet(name="mercury"),
    ...
    prop = orekit_prop(third_bodies=third_bodies)
    ...
    prop.propagate(state_init=c0, time=time_end)
...
@pytest.mark.parametrize("orekit_prop", OREKIT_PROP_CLASSES)
def test_invalid_third_bodies(maven_package, orekit_prop):  # noqa: ARG001
    # We try to define a third body attractor not supported by Orekit
    third_bodies = [MinorBody(name="ceres")]
    with pytest.raises(ValueError):
        _prop = orekit_prop(third_bodies=third_bodies)


@pytest.mark.parametrize("orekit_prop", OREKIT_PROP_CLASSES)
def test_srp(maven_package, orekit_prop):  # noqa: ARG001
    prop = orekit_prop(enable_srp=True)
...
@pytest.mark.parametrize("orekit_prop", OREKIT_PROP_CLASSES)
def test_drag(maven_package, orekit_prop):  # noqa: ARG001
    prop = orekit_prop(enable_drag=True)
...
def test_apside_stop(maven_package, orekit_prop, peri_or_apoapsis: StoppingEvent):  # noqa: ARG001
    ...
    assert (
        len(prop._orekit_prop.getEventsDetectors()) == 0
    )  # The Orekit EventDetector is added upon calling the propagate method

    traj = prop.propagate(state_init=state_init, time=time_list, stop_conds=[peri_or_apoapsis])

    assert (
        len(prop._orekit_prop.getEventsDetectors()) == 1
    )  # The Orekit EventDetector is added upon calling the propagate method
