...
class ExtractTime:
    ...
    @staticmethod
    def trigonometric_features(data, return_df: bool = True):
        dict_ = {'month': 12, 'day': 30}
        sin_f = [(f'{i}_sin', ExtractTime.sin_transformer(v), [i]) for i, v in dict_.items()]
        cos_f = [(f'{i}_cos', ExtractTime.cos_transformer(v), [i]) for i, v in dict_.items()]
