...
class ShellyPro3EM(JShelly):
    ...
    def publish_active_power(self, ts, em):
        ...
        self.mqtt_client.publish(self.power_topic, json.dumps(msg), 1, True)
    ...
    def on_emdata_0(self, ts, em):
        ...
        try:
            self.write(point)
            self.debug(f"Total energy consumed {em["total_act"]}, exported {em["total_act_ret"]}")
        except Exception as e:
            self.error(f"Writing to influx failed {str(e)}")
    ...
    def to_dict(self):
        data = super().to_dict()
        data["_shellypro3em"] = {"shelly_topic": self.shelly_topic, "power_topic": self.power_topic}
        return data
