...
class DynamicGenerator:
    ...
    details on arrival and departure rates and illness periods for
    ...
    def __init__(
        ...
        recipient_illness_function: Callable[[Recipient], float],
        ...
        ndd_illness_function: Callable[[Donor], float],
    ):
        ...
        :param recipient_attrition_function: A function that takes a Recipient
            ...
            period.
        :param recipient_illness_function: A function that takes a Recipient and
        ...
        :param ndd_attrition_function: A function that takes a non-directed
            donor and returns the chance that this donor will leave in any period.
        :param recipient_illness_function: A function that takes a non-directed
        ...
        self._recipient_illness_function = recipient_illness_function
    ...
    def _gen_participants(
        ...
        :param illness_function: A function that takes as input a participant, and returns the chance that they are ill in any given period.
        """
        periods = len(arrival_counts)
        illnesses: dict[str, list[int]] = {}
        arrivals: dict[str, int] = {}
        departures: dict[str, int] = {}

        def _get_illness_departure(
            participant: ParticipantType,
            illness_function: Callable[[ParticipantType], float],
            attrition_function: Callable[[ParticipantType], float],
        ) -> tuple[list[int], int]:
            """Determine when the given participant arrives, leaves, and is ill.

            :param participant: The participant in question
            :param illness_function: A function that takes as input a
                participant, and returns the chance they are ill in any given
                period.
            ...
            for r_period in range(period, periods):
                if np.random.random() < illness_function(participant):
                    illnesses.append(r_period)
                if np.random.random() < attrition_function(participant):
                    departure = r_period
                    break
            return illnesses, departure
        ...
        for period, count in enumerate(arrival_counts):
            for _ in range(count):
                try:
                    ...
                    # No one left to add
                    return arrivals, departures, illnesses
                # Arriving at this period
                arrivals[participant.id] = period
                illnesses[participant.id], departures[participant.id] = (
                    _get_illness_departure(
                        participant, illness_function, attrition_function
                    )
                )
        return arrivals, departures, illnesses
    ...
    def generate(
        ...
        recipient_arrivals, recipient_departures, recipient_illnesses = (
            self._gen_participants(
                dynamic_instance.allRecipients(),
                recip_arrival_counts,
                self._recipient_illness_function,
        ...
        dynamic_instance.recipient_illnesses = recipient_illnesses
        # Work out when NDDs leave, and when they are ill
        ndd_arrivals, ndd_departures, ndd_illnesses = self._gen_participants(
            instance.allNDDs(),
            ndd_arrival_counts,
            self._ndd_illness_function,
            self._ndd_attrition_function,
        )
        dynamic_instance.ndd_arrivals = ndd_arrivals
        dynamic_instance.ndd_departures = ndd_departures
        dynamic_instance.ndd_illnesses = ndd_illnesses
