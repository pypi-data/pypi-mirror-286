    ...
    def writeFileJson(self, filename: str, write_recipients: bool = False) -> None:
        ...
        def mkdonor(donor: Donor):
            ...
            if (
                ...
                result["illnesses"] = self.ndd_illnesses[donor.id]
            return result
        ...
        def mkrecipient(recipient: Recipient):
            ...
            if (
                ...
                result["illnesses"] = self.recipient_illnesses[recipient.id]
            return result
...
class DynamicInstance(Instance):
    ...
    :param recipient_departures: A mapping from recipient IDs to the period in
        which they depart
    :param recipient_illnesses: A mapping from recipient IDs to a list of periods
    ...
    :param ndd_departures: A mapping from non-directed donor IDs to the period in
        which they depart
    :param ndd_illnesses: A mapping from non-directed donor IDs to a list of periods
    ...
    recipient_illnesses: MutableMapping[str, list[int]] = field(default_factory=dict)
    ...
    ndd_illnesses: MutableMapping[str, list[int]] = field(default_factory=dict)
    failing_transplants: list[Transplant] = field(default_factory=list)
    transplant_periods: MutableMapping[str, int] = field(default_factory=dict)
    ...
    def is_ill(self, participant: Donor | Recipient, period: int) -> bool:
        ...
        if isinstance(participant, Recipient):
            return period in self.recipient_illnesses[participant.id]
        return period in self.ndd_illnesses[participant.id]
