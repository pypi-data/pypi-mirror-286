...
def main():
    ...
    build_db_parser.add_argument("--db-name", dest="db_name", help="DB name of yoursulf")
    build_db_parser.add_argument("--add-to-library", dest="fasta_file", help="Add FILE to library(required for kmer database)")
    ...
    build_db_parser.add_argument('--bypass-tax', action='store_true', help='Bypass the download_taxonomy step')
    ...
    combined_help_message = '''usage: hostpurge [-h] {run,build-db,qc} ...
...
--------------------------------------
    ...
    usage: [-h] --mode {a,b,c,d}
               [-i1 INPUT_1.fq] [-i2 INPUT_2.fq] [-o1 OTUPUT_1.fq] [-o2 OTUPUT_2.fq] [-t THREADS]
...
[--kmer_db KMER_DB] [--taxid TAXID]#Only for kmer database
    ...
    optional arguments:
      ...
      --taxid TAXID         Taxonomy ID of reads to extract (Only for kmer, model b, c, d) 
...
--------------------------------------
    ...
    usage:   [-h] [--db-type {kmer, alignment}] [-t THREADS]
                [--db-name DB_NAME] 
                [--add-to-library REFERENCE_GENOME.fasta(Only for kmer database)
                [--input-fasta REFERENCE_GENOME.fasta] [-o OUTPUT_PREFIX]

    optional arguments:
      -h, --help            show this help message and exit
      --db-type {kmer, alignment}    Type of database to build: kmer or alignment
      -t THREADS            Number of threads to use (default: 1)
      --db-name DB_NAME     DB name of yourself
      --add-to-library REFERENCE_GENOME.fasta(Please update your FASTA file headers to include the prefix in the following format: >GenomeID|kraken:taxid|39947. For more details, please refer to our GitHub help document.)
      --input-fasta REFERENCE_GENOME.fasta    Input reference genome FASTA file (required for alignment database)
