...
class DgnCode(ABC):
    ...
    def calc_compound_section(self, material: Material, geometry: Geometry, opt: PMOptions):
        pm = PM3DCurve(material, geometry, opt)
...
@auto_schema
def calc_cracked_properties(material: Material, geometry: Geometry, lcb: Lcb) -> list[ResultCrackedProperties]:
    sect = MSection(material, geometry)
    comp_sect = sect.calc_compound_section()
    ret = []
    for lcom in lcb.uls.lcoms:
        result = comp_sect.calculate_cracked_properties(m_ext=lcom.f.Mx)
        ret.append(ResultCrackedProperties(d_nr=result.d_nc, i_cr=result.e_ixx_c_cr))

    return ret

@auto_schema
def calc_uncracked_stress(material: Material, geometry: Geometry, lcb: Lcb):
    sect = MSection(material, geometry)
    comp_sect = sect.calc_compound_section()
    ret = []
    for lcom in lcb.uls.lcoms:
        if type(comp_sect) is ConcreteSection:
            result = comp_sect.calculate_uncracked_stress(n=lcom.f.Nz, m_x=lcom.f.Mx, m_y=lcom.f.My)
        else:
            result = comp_sect.calculate_uncracked_stress(n=lcom.f.Nz, m=lcom.f.Mx)
...
@auto_schema
def calc_moment_curvature(material: Material, geometry: Geometry):
    sect = MSection(material, geometry)
    comp_sect = sect.calc_compound_section()
