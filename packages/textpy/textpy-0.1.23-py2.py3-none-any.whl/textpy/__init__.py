'''
# textpy
Reads a python module and statically analyzes it. This works well with Jupyter extensions in VScode,
and will have better performance when the module files are formatted with *PEP-8*.
...
def print_my_book(book: MyBook) -> None:
    ...
    print(book.content)
```
Run the following codes to find all the occurrences of some pattern (for example, "MyBook") in
`myfile.py`:
```py
>>> import textpy as tx
...
be either `PyDir` or `PyFile`, both subclasses of `PyText`, depending on the path type.
...
```
In addition, the `.findall()` method has some optional parameters to customize the matching pattern,
including `whole_word=`, `case_sensitive=`, and `regex=`.
```py
