...
BEGIN
    ...
    IF POSITION(':' IN _fields) > 0 THEN
        ...
        _nested_fields := string_to_array(_fields, ',');
        FOREACH _nested_field_origin IN ARRAY _nested_fields
        LOOP
            ...
            _nested_field = string_to_array(_nested_field[2].substr(1, length(_nested_field[2])-1), ';');
            _nested_field_model := _nested_field[1];
            -- fields are the rest of the array
            _nested_field_fields := _nested_field[2:].join(',');
            ...
            fields = REPLACE(fields, _nested_field_origin, '');
        END LOOP;
    ...
    IF array_length(_nested_queries, 1) > 0 THEN
        fields := fields || ',' || _nested_queries.join(',');
    END IF;
