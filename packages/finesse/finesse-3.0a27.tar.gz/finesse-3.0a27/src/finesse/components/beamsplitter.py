...
class Beamsplitter(Surface):
    ...
    def _plane_of_incidence(self, direction):
        ...
        if self.plane == PlaneOfIncidence.xz:
            ...
            raise ValueError(
                f"PlaneOfIncidence must be one of {tuple(_ for _ in PlaneOfIncidence.__members__.keys())}"
            )
        ...
        if self.interaction_type(from_node, to_node) == InteractionType.REFLECTION:
            ...
            if from_node.port is self.p3 or from_node.port is self.p4:
                M_sym = abcd.beamsplitter_refl(
                    ...
                    nr1=nri,
                    nr2=nro,
                    direction=self._plane_of_incidence(direction),
                )
            else:
                M_sym = abcd.beamsplitter_refl(
                    Rc,
