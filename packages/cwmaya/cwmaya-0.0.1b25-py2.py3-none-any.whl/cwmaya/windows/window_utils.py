...
def show_in_vscode(data, **kw):
    ...
    if not code_path:
        raise RuntimeError("Visual Studio Code executable not found in PATH.")
    
    process = subprocess.Popen([code_path, "-"], stdin=subprocess.PIPE)
...
def show_file_in_vscode(filepath, **kw):
    ...
    process = subprocess.Popen([code_path, filepath])
    process.communicate()
