...
def initialize(longPrefix, shortPrefix, outputPlug):
    ...
    # make an output plug so we can easily get the b64
    result["outputPayload"] = attrs.makeStringAttribute(
        f"{longPrefix}OutScriptPayload",
        f"{shortPrefix}opl",
        hidden=False,
        writable=False,
        keyable=False,
    ...
    om.MPxNode.addAttribute(result["outputPayload"])
    ...
    for attr in top_level_attrs:
        ...
        om.MPxNode.attributeAffects(result[attr], result["outputPayload"])
...
def computePythonScript(script_values, context=None):
    ...
    executor_script = f"pyPayloadExecutor_{script}"
    ...
    #     f'"{scene}"',
    #     "-script",
    #     f'"{executor_script}.mel"',
    #     "-command",
    #     f'"{executor_script}(\\\"{b64_payload}\\\")"',
    # ]
