...
class ToolsMenuGroup(object):
    ...
    def create_desktop_app_section(self):
        ...
        pm.menuItem(label="Open app", command=pm.Callback(desktop_app_helpers.open_app))
        ...
        pm.menuItem(
            label="Sync coredata",
            command=pm.Callback(self.on_send_coredata),
        )
    ...
    def create_script_section(self):
        pm.setParent(self.tools_menu, menu=True)
        pm.menuItem(divider=True, dividerLabel="Scripts")

        self.generate_scripts_menu = pm.menuItem(
            label="Generate scripts", command=pm.Callback(self.on_generate_scripts)
        )
    ...
    def on_send_coredata(self):
        response = desktop_app_helpers.send_coredata()
        if response.ok:
            desktop_app_helpers.navigate("projects")
        else:
            pm.error(f"Error syncing coredata: {response.text}")

    def on_generate_scripts(self):
        scripts = assets.scrape_py_executors(self.dialog.node)
