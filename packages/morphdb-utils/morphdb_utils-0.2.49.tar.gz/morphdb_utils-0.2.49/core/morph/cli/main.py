...
def move_resource(
    ...
    from morph.task.resource import MoveResourceTask

    task = MoveResourceTask(ctx.obj["flags"])
    results = task.run()
    return results, True
...
def remove_resource(
    ...
    from morph.task.resource import RemoveResourceTask

    task = RemoveResourceTask(ctx.obj["flags"])
    results = task.run()
    return results, True
